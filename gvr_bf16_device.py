...
# ---------------------------------------------------------------------------
# bf16-native global loads (bf16 arm only). Every loaded value is widened to
# fp32 in registers -- bf16 -> fp32 is exact (bits << 16), so fkey/invkey and
# all ladder math downstream are unchanged.
# ---------------------------------------------------------------------------
@dsl_user_op
def _ld_g_nc_v2_b32(gaddr, *, loc=None, ip=None):
    """Pinned `ld.global.nc.v2.b32` (8-byte line = 4 bf16) -> two Uint32.

    The asm boundary pins the two-scalar-b32 shape the same way
    _ld_g_nc_v4_f32 pins the fp32 arm's four-scalar shape."""
    ret = llvm.inline_asm(
        llvm.StructType.get_literal([T.i32(), T.i32()]),
        [gaddr.ir_value(loc=loc, ip=ip)],
...
def ld_g_bf16x4(copy_atom, base_addr, v_idx, frag):
    """bf16 twin of ld_g_f32x4: 4 bf16 (8 B) at vector #v_idx widened to fp32.

    base_addr: Int64 byte address; frag: (4,) f32 fragment. copy_atom is
    ignored (call-site signature parity with the fp32 arm)."""
    lo, hi = _ld_g_nc_v2_b32(base_addr + cutlass.Int64(v_idx) * cutlass.Int64(8))
    frag[0] = C.f32_of_u32(lo << cutlass.Uint32(16))
    frag[1] = C.f32_of_u32(lo & cutlass.Uint32(0xFFFF0000))
    frag[2] = C.f32_of_u32(hi << cutlass.Uint32(16))
    frag[3] = C.f32_of_u32(hi & cutlass.Uint32(0xFFFF0000))


def ld_g_bf16x4_raw(base_addr, v_idx, frag):
    """Load four bf16 values (8 B) while retaining two packed u32 carriers."""
    lo, hi = _ld_g_nc_v2_b32(base_addr + cutlass.Int64(v_idx) * cutlass.Int64(8))
    frag[0] = lo
    frag[1] = hi


def bf16x4_raw_lane(frag, q: cutlass.Constexpr):
    """Widen constexpr lane ``q`` from a packed four-bf16 packet."""
    word = frag[q // 2]
    if cutlass.const_expr((q & 1) == 0):
        return C.f32_of_u32(word << cutlass.Uint32(16))
    return C.f32_of_u32(word & cutlass.Uint32(0xFFFF0000))


@dsl_user_op
def _ld_g_nc_u16(gaddr, *, loc=None, ip=None):
    """Scalar `ld.global.nc.u16` zero-extended into one 32-bit register."""
...
def ldg_bf16(base_addr, idx):
    """bf16 twin of ldg_f32: scalar read-only 2 B gather widened to fp32."""
    u = _ld_g_nc_u16(base_addr + cutlass.Int64(idx) * cutlass.Int64(2))
    return C.f32_of_u32(u << cutlass.Uint32(16))


def _ldg_bf16_rs(base_addr, idx, sc2):
    """bf16 twin of _ldg_f32_rs: the byte stride (2) rides a caller-held
    register loaded from smem (same anti-remat rationale)."""
...
def _ld_g_nc_v4_b32(gaddr, *, loc=None, ip=None):
    """Pinned `ld.global.nc.v4.b32` (16-byte line = 8 bf16) -> four Uint32.

    Same pinned-asm rationale as _ld_g_nc_v4_f32: keep the four-scalar-b32
    shape so NVVM cannot rewrite adjacent loads into b64 register pairs."""
    ret = llvm.inline_asm(
        llvm.StructType.get_literal([T.i32(), T.i32(), T.i32(), T.i32()]),
        [gaddr.ir_value(loc=loc, ip=ip)],
    ...
    return (
        cutlass.Uint32(llvm.extractvalue(T.i32(), ret, [0])),
        cutlass.Uint32(llvm.extractvalue(T.i32(), ret, [1])),
        cutlass.Uint32(llvm.extractvalue(T.i32(), ret, [2])),
        cutlass.Uint32(llvm.extractvalue(T.i32(), ret, [3])),
    )


def ld_g_bf16x8(copy_atom, base_addr, v_idx, frag):
    """8 bf16 (16 B) at 8-element vector #v_idx, widened to fp32 in frag[0..7].

    base_addr: Int64 byte address; frag: (8,) f32 fragment. copy_atom ignored
    (call-site signature parity with the fp32 arm)."""
    w0, w1, w2, w3 = _ld_g_nc_v4_b32(base_addr + cutlass.Int64(v_idx) * cutlass.Int64(16))
    frag[0] = C.f32_of_u32(w0 << cutlass.Uint32(16))
    frag[1] = C.f32_of_u32(w0 & cutlass.Uint32(0xFFFF0000))
    frag[2] = C.f32_of_u32(w1 << cutlass.Uint32(16))
    frag[3] = C.f32_of_u32(w1 & cutlass.Uint32(0xFFFF0000))
    frag[4] = C.f32_of_u32(w2 << cutlass.Uint32(16))
    frag[5] = C.f32_of_u32(w2 & cutlass.Uint32(0xFFFF0000))
    frag[6] = C.f32_of_u32(w3 << cutlass.Uint32(16))
    frag[7] = C.f32_of_u32(w3 & cutlass.Uint32(0xFFFF0000))


def ld_g_bf16x8_pair(copy_atom, base_addr, p4, fa, fb):
    """One 16 B bf16 load covering the fp32 arm's PAIR of adjacent float4
    sample loads: elements [4*p4, 4*p4 + 8) -> fa[0..3], fb[0..3].

    p4 is the fp32 arm's (even) 4-element vector index; byte offset p4 * 8 is
    16-byte aligned because p4 is even at every call site."""
    w0, w1, w2, w3 = _ld_g_nc_v4_b32(base_addr + cutlass.Int64(p4) * cutlass.Int64(8))
    fa[0] = C.f32_of_u32(w0 << cutlass.Uint32(16))
    fa[1] = C.f32_of_u32(w0 & cutlass.Uint32(0xFFFF0000))
    fa[2] = C.f32_of_u32(w1 << cutlass.Uint32(16))
    fa[3] = C.f32_of_u32(w1 & cutlass.Uint32(0xFFFF0000))
    fb[0] = C.f32_of_u32(w2 << cutlass.Uint32(16))
    fb[1] = C.f32_of_u32(w2 & cutlass.Uint32(0xFFFF0000))
    fb[2] = C.f32_of_u32(w3 << cutlass.Uint32(16))
    fb[3] = C.f32_of_u32(w3 & cutlass.Uint32(0xFFFF0000))


@cute.jit
def gather_hint_bf16(
    x_addr, p_addr, k, n, tidx, s_wmn, s_wmx, blk: cutlass.Constexpr, kpt: cutlass.Constexpr
):
    """bf16 twin of gather_hint (2-byte logits gathers; same 2 barriers)."""
    NW = blk // 32
    lane = tidx & cutlass.Int32(31)
    pvs = []
    for t in cutlass.range_constexpr(kpt):
...
class GvrMainKernel:
    ...
    def __init__(
        ...
        self.pf = self.pfd > 0
        # Packed-carrier P3 for the compact 1024-thread SPLIT mains that
        # carry the b=16 long-row cells (v32 128k/256k, pro 512k): each
        # four-bf16 packet stays two u32 carriers until the classify
        # compare, mirroring the wide module's measured R=9 packed-P3 win.
        # Fully-prefetched engines only (pfd == u), never kbig-agnostic
        # routes outside r_const 8/9, never prefill.
        self.p3c = (
            ...
            and (
                (
                    ...
                    and self.kpt >= 2
                    and self.pfd == self.u
                    and self.r_const in (8, 9)
                )
                or (not self.split and self.u == 8 and self.blk in (256, 512, 1024))
            )
        )
    ...
    def kern(
        ...
        if cutlass.const_expr(self.dtype == cutlass.Float32):
            ...
            if cutlass.const_expr(self.prefill):
                ...
                x_addr = _pin_i64(
                    logits.iterator.toint()
                    + (row64 * cutlass.Int64(npad) + cutlass.Int64(col0)) * cutlass.Int64(2)
                )
            else:
                x_addr = _pin_i64(
            ...
            if shas != cutlass.Int32(0):
                p4 = tidx * SS2 * cutlass.Int32(2)
                if cutlass.const_expr((not self.kbig) or self.split):
                    C.ld_g_bf16x8_pair(atom128, x_addr, p4, fsa, fsb)
                else:
                    C.ld_g_bf16x4(atom128, x_addr, p4, fsa)
                    C.ld_g_bf16x4(atom128, x_addr, p4 + cutlass.Int32(1), fsb)
                if cutlass.const_expr(self.prefill):
                    # only thread 0's float4 0 holds the <=3 lead lanes; substitute
                    # lane 3 (a -inf would drive f2s_rz to INT_MIN and index the
                    # sample histogram out of bounds).
            ...
            while j < SMP:
                p4 = j * SS2 * cutlass.Int32(2)
                if cutlass.const_expr((not self.kbig) or self.split):
                    C.ld_g_bf16x8_pair(atom128, x_addr, p4, fma_, fmb_)
                else:
                    C.ld_g_bf16x4(atom128, x_addr, p4, fma_)
                    C.ld_g_bf16x4(atom128, x_addr, p4 + cutlass.Int32(1), fmb_)
                for t in cutlass.range_constexpr(4):
                    smn = C.fmin_f32(smn, fma_[t])
                    smx = C.fmax_f32(smx, fma_[t])
                for t in cutlass.range_constexpr(4):
            ...
            if cutlass.const_expr(self.prefill):
                ...
                lim4 = (npad >> cutlass.Int32(2)) - cutlass.Int32(1)
            pf = [cute.make_rmem_tensor((4,), cutlass.Float32) for _ in range(max(PFD, 1))]
            if cutlass.const_expr(P3C):
                pf = [cute.make_rmem_tensor((2,), cutlass.Uint32) for _ in range(max(PFD, 1))]
            if cutlass.const_expr(self.pf):
                fullsl = cutlass.Int32(0)
                if (c1 - c0) >= cutlass.Int32(BLK * U):
                    fullsl = cutlass.Int32(1)
                if fullsl != cutlass.Int32(0):  # prime, full slice
                    for uu in cutlass.range_constexpr(PFD):
                        if cutlass.const_expr(P3C):
                            C.ld_g_bf16x4_raw(x_addr, c0 + tidx + cutlass.Int32(uu * BLK), pf[uu])
                        else:
                            C.ld_g_bf16x4(atom128, x_addr, c0 + tidx + cutlass.Int32(uu * BLK), pf[uu])
                else:  # clamped prime
                    for uu in cutlass.range_constexpr(PFD):
                        i_ = c0 + tidx + cutlass.Int32(uu * BLK)
                        ic = i_
                        if ic >= c1:
                            ic = lim4
                        if cutlass.const_expr(P3C):
                            C.ld_g_bf16x4_raw(x_addr, ic, pf[uu])
                        else:
                            C.ld_g_bf16x4(atom128, x_addr, ic, pf[uu])
                # asm prefetch site #1: gate (c1-c0)>=2*BLK*U && SMP>=160
                g1 = cutlass.Int32(0)
                if (c1 - c0) >= cutlass.Int32(2 * BLK * U):
                    if SMP >= cutlass.Int32(160):
            ...
            if cutlass.const_expr(P3C):
                fr = [
                    cute.make_rmem_tensor((2,), cutlass.Uint32)
                    for _ in range(max(U - PFD, 1))
                ]
            att = cutlass.Int32(0)
            running = cutlass.Int32(1)
            while running != cutlass.Int32(0):
                        ...
                        if cutlass.const_expr(self.pf):
                            ...
                            if fullsl != cutlass.Int32(0):
                                for uu in cutlass.range_constexpr(PFD):
                                    if cutlass.const_expr(P3C):
                                        C.ld_g_bf16x4_raw(
                                            x_addr, c0 + tidx + cutlass.Int32(uu * BLK), pf[uu]
                                        )
                                    else:
                                        C.ld_g_bf16x4(
                                            atom128, x_addr, c0 + tidx + cutlass.Int32(uu * BLK), pf[uu]
                ...
                while it < nIt:
                    ...
                    if isfull != cutlass.Int32(0):  # full body
                        for uu in cutlass.range_constexpr(PFD, U):
                            if cutlass.const_expr(P3C):
                                C.ld_g_bf16x4_raw(x_addr, i0 + cutlass.Int32(uu * BLK), fr[uu - PFD])
                            else:
                                C.ld_g_bf16x4(atom128, x_addr, i0 + cutlass.Int32(uu * BLK), fr[uu - PFD])
                        for uu in cutlass.range_constexpr(U):
                            if cutlass.const_expr(uu < PFD):
                                vv = pf[uu]
                            else:
                            ...
                            if cutlass.const_expr(P3C):
                                for q in cutlass.range_constexpr(4):
                                    vq = C.bf16x4_raw_lane(vv, q)
                                    M = M | (cutlass.Int32(vq >= TF) << cutlass.Int32(uu * 4 + q))
                            else:
                                for q in cutlass.range_constexpr(4):
                                    M = M | (cutlass.Int32(vv[q] >= TF) << cutlass.Int32(uu * 4 + q))
                    else:  # partial body
                        for uu in cutlass.range_constexpr(PFD, U):
                            ...
                            if ic >= c1:
                                ic = lim4  # clamped address
                            if cutlass.const_expr(P3C):
                                C.ld_g_bf16x4_raw(x_addr, ic, fr[uu - PFD])
                            else:
                                C.ld_g_bf16x4(atom128, x_addr, ic, fr[uu - PFD])
                        for uu in cutlass.range_constexpr(U):
                            if cutlass.const_expr(uu < PFD):
                                vv = pf[uu]
                            else:
                    ...
                    if cutlass.const_expr(self.pf):
                        ...
                        if hasnext != cutlass.Int32(0):
                            ...
                            if infull != cutlass.Int32(0):
                                for uu in cutlass.range_constexpr(PFD):
                                    if cutlass.const_expr(P3C):
                                        C.ld_g_bf16x4_raw(x_addr, j0 + cutlass.Int32(uu * BLK), pf[uu])
                                    else:
                                        C.ld_g_bf16x4(atom128, x_addr, j0 + cutlass.Int32(uu * BLK), pf[uu])
                            else:
                                for uu in cutlass.range_constexpr(PFD):
                                    j_ = j0 + cutlass.Int32(uu * BLK)
                                    jc = j_
                                    if jc >= c1:
                                        jc = lim4
                                    if cutlass.const_expr(P3C):
                                        C.ld_g_bf16x4_raw(x_addr, jc, pf[uu])
                                    else:
                                        C.ld_g_bf16x4(atom128, x_addr, jc, pf[uu])
                    # warp-aggregated reservation
                    cnt = cutlass.Int32(C.popc(M))
                    inc = C.warp_incl_scan_add(cnt, lane)
                    bpos = cutlass.Int32(0)
                    ...
                    if M != cutlass.Int32(0):
                        ...
                        if cutlass.const_expr(self.vstg and self.blk == 512):
                            xv = _ldg_bf16_rs(x_addr, idx, x4_pin)
                        else:
                            xv = C.ldg_bf16(x_addr, idx)
                        while M != cutlass.Int32(0):
                            bp2 = C.ffs_m1(M)
                            M = M & (M - cutlass.Int32(1))
                            idx2 = (
                            ...
                            if cutlass.const_expr(self.vstg and self.blk == 512):
                                xv2 = _ldg_bf16_rs(x_addr, idx2, x4_pin)
                            else:
                                xv2 = C.ldg_bf16(x_addr, idx2)
                            pos = self._emitc(
                                xv, idx, pos, TF, SC, hb_pin, cb2_pin, s_hist, s_cbuf, s_cbuf2
                            )
                            idx = idx2
                                    ...
                                    if whole == cutlass.Int32(0):
                                        ...
                                        if q2 < cutlass.Int32(CMPB):
                                            if cutlass.const_expr(self.vstg):
                                                ...
                                                kk = C.fkey(C.ldg_bf16(x_addr, idv))
                                            s_ck64[q2] = (
                                                cutlass.Uint64(kk) << cutlass.Uint64(32)
                                            ) | cutlass.Uint64(cutlass.Uint32(idv))
                            i = i + cutlass.Int32(BLK)
                            ...
                            if need2 == m2:
                                ...
                                while i < listN:
                                    ...
                                    if cutlass.const_expr(self.vstg):
                                        ...
                                        uq = C.fkey(C.ldg_bf16(x_addr, id0))
                                    if uq >= cutlass.Uint32(rlo):
                                        if uq <= cutlass.Uint32(rhi):
                                            du = (uq - cutlass.Uint32(rlo)) >> sh2u
                                            if du > cutlass.Uint32(NBS - 1):
                        ...
                        while it < it2:
                            ...
                            if i < listN:
                                ...
                                if cutlass.const_expr(self.vstg):
                                    ...
                                    uq = C.fkey(C.ldg_bf16(x_addr, idv))
                                iu = cutlass.Int64(uq)
                                if iu > ethr:
                                    p1 = cutlass.Int32(1)
                                if tie_m != cutlass.Int32(0):
...
def get_compiled(
    ...
    logits_fake = _crt.make_fake_compact_tensor(
        dtype, (r0, c0), stride_order=(1, 0), assumed_align=16
    )
    if prefill:
...
def run(logits, pre_idx, n: int, out, ws):
    ...
    try:
        from . import gvr_bf16_host as ct_dispatch
    except ImportError:
        import gvr_bf16_host as ct_dispatch
    b, npad = logits.shape
    k = pre_idx.shape[1]
    ...
    def kern(
        ...
        if cutlass.const_expr(self.dtype == cutlass.Float32):
            ...
            if short == cutlass.Int32(0):
                ...
                # ---- row load: exact-fit peel + float4[VPT] register batch.
                # The multi-packet tile benefits from issuing every 8B packet
                # before any in-register widening (memory-level parallelism);
                # R7 extends this from VPT=2 to VPT=4 (the B=128 reg cells
                # v32_16k/pro_64k/flash_64k).  VPT=1 has a single packet
                # (nothing to batch) and degenerate tiles keep the peel path.
                frags = [cute.make_rmem_tensor((4,), cutlass.Float32) for _ in range(VPT)]
                if cutlass.const_expr(VPT not in (2, 4) or self.deg):
                    atom128 = g2r_atom_f32(128, invariant=True)
                    if n4 >= cutlass.Int32(self.blk * self.vpt):  # block-uniform peel
                        for u in cutlass.range_constexpr(VPT):
                            ld_g_bf16x4(
                    ...
                    for u in cutlass.range_constexpr(VPT):
                        ...
                        ic = i
                        if ic >= n4:
                            ic = n4 - cutlass.Int32(1)
                        raws.append(
                            _ld_g_nc_v2_b32(
                                x_addr + cutlass.Int64(ic) * cutlass.Int64(8)
                            )
                        )
                    for u in cutlass.range_constexpr(VPT):
                        rawlo, rawhi = raws[u]
                        frags[u][0] = f32_of_u32(rawlo << cutlass.Uint32(16))
                        frags[u][1] = f32_of_u32(rawlo & cutlass.Uint32(0xFFFF0000))
                        frags[u][2] = f32_of_u32(rawhi << cutlass.Uint32(16))
                        frags[u][3] = f32_of_u32(rawhi & cutlass.Uint32(0xFFFF0000))
                    for u in cutlass.range_constexpr(VPT):
                        i = tid + cutlass.Int32(u * self.blk)
                        if i >= n4:  # -INFINITY fill
                            for q in cutlass.range_constexpr(4):
...
def get_compiled__reg(
    ...
    key = (tuple(tpl), bool(pdl), bool(varlen), int(next_n), int(cr_shift), bool(hint_free), str(dtype))
    ...
    if compiled is None:
        ...
        lg_fake = _crt.make_fake_compact_tensor(
            dtype, (nb_, nc_), stride_order=(1, 0), assumed_align=16
        )
...
def reg_topk(logits, pre_idx, n, out, rd=None):
    ...
    if rd is None:
        try:
            from .gvr_bf16_host import route
        except ImportError:
            from gvr_bf16_host import route
    ...
    try:
        from .gvr_bf16_host import _dummy_kv
    except ImportError:
        from gvr_bf16_host import _dummy_kv
...
class GvrClusKernel:
    ...
    def kern(
        ...
        if cutlass.const_expr(self.dtype == cutlass.Float32):
            ...
            short = cutlass.Int32(0)
            prow = row
            sre = cutlass.Int32(0)  # short-row emit cursor (pre-bound; rule 17)
            if cutlass.const_expr(self.varlen):
                kq = cutlass.Int32(pre_idx.shape[1])
                req = row // cutlass.Int32(self.next_n)
                rr = row % cutlass.Int32(self.next_n)
...
def _val__regclus_raw(frags, s: int):
    """val[s] accessor over the packed u32x2[VPT] carrier batch (constexpr s)."""
    word = frags[s // 4][(s % 4) // 2]
    if cutlass.const_expr((s & 1) == 0):
        return f32_of_u32(word << cutlass.Uint32(16))
    return f32_of_u32(word & cutlass.Uint32(0xFFFF0000))
...
class GvrRegClusKernel:
    ...
    def __init__(
        ...
        assert vpt in (1, 2, 3, 4, 5) and cs in (2, 4, 8)
        self.blk = blk
        self.vpt = vpt
        self.cs = cs
        self.pdl = bool(pdl)
        ...
        self.hint_free = bool(hint_free)
        # Packed row carriers for the exact v32 64K/B16 reg_clus engine.
        self.rc_p3 = (
            self.dtype == cutlass.BFloat16
            and self.varlen
            and self.cr_shift == 0
    ...
    def kern(
        ...
        if cutlass.const_expr(self.dtype == cutlass.Float32):
            ...
            if short == cutlass.Int32(0):
                ...
                n4 = n >> cutlass.Int32(2)
                if n4 > cutlass.Int32(self.cs * self.blk * self.vpt):
                    n4 = cutlass.Int32(self.cs * self.blk * self.vpt)
                ntail = n - (n4 << cutlass.Int32(2))
                base4 = rank * cutlass.Int32(self.span)
                tix = (n4 << cutlass.Int32(2)) + tid  # CUDA `tidx`
                ...
                if cutlass.const_expr(self.hint_free):
                    if tid < k:
                        pv0 = tid
                    if tid + cutlass.Int32(BLK) < k:
                        pv0b = tid + cutlass.Int32(BLK)
                else:
                    if tid < k:
                        pv0 = ld_g_i32(p_addr, tid)
                ...
                # first, then -INFINITY-fill missed slots.
                frags = [cute.make_rmem_tensor((4,), cutlass.Float32) for _ in range(VPT)]
                if cutlass.const_expr(self.rc_p3):
                    frags = [cute.make_rmem_tensor((2,), cutlass.Uint32) for _ in range(VPT)]
                raws = []
                for u in cutlass.range_constexpr(VPT):
                    i = base4 + tid + cutlass.Int32(u * self.blk)
                    ic = i
                    if ic >= n4:
                        ic = n4 - cutlass.Int32(1)
                    raws.append(
                        _ld_g_nc_v2_b32(x_addr + cutlass.Int64(ic) * cutlass.Int64(8))
                    )
                for u in cutlass.range_constexpr(VPT):
                    rawlo, rawhi = raws[u]
                    if cutlass.const_expr(self.rc_p3):
                ...
                for u in cutlass.range_constexpr(VPT):
                    ...
                    if i >= n4:  # -INFINITY fill
                        if cutlass.const_expr(self.rc_p3):
                            frags[u][0] = cutlass.Uint32(0xFF80FF80)
                            frags[u][1] = cutlass.Uint32(0xFF80FF80)
                        else:
                            for z in cutlass.range_constexpr(4):
                                frags[u][z] = cutlass.Float32(_NEG_INF__regclus)
                # tail element: rank 0 only
                ...
                lmax = cutlass.Uint32(0)
                if cutlass.Uint32(pv0) < cutlass.Uint32(n):
                    uk = fkey(ldg_bf16(x_addr, pv0))  # __ldg(X+pv0)
                    lmin = uk
                    lmax = uk
                if cutlass.Uint32(pv0b) < cutlass.Uint32(n):
                    uk = fkey(ldg_bf16(x_addr, pv0b))
                    if uk < lmin:
                        lmin = uk
                    if uk > lmax:
                        lmax = uk
                ...
                for s in cutlass.range_constexpr(S):
                    if cutlass.const_expr(self.rc_p3):
                        qv = _fmaf__regclus(_val__regclus_raw(frags, s), SC, CQ)
                    else:
                        qv = _fmaf__regclus(_val__regclus(frags, s), SC, CQ)
                    bn = _umin_u32__regclus(f2u_rz(qv), cutlass.Uint32(NB__regclus - 1))
                    atomic_add_cta(s_hist.iterator + cutlass.Int32(bn), cutlass.Int32(1))
                ...
                if degen == cutlass.Int32(0):
                    ...
                    for s in cutlass.range_constexpr(S):
                        if cutlass.const_expr(self.rc_p3):
                            qv = _fmaf__regclus(
                                _val__regclus_raw(frags, s), SC, CQ
                            )  # bit-identical
                        else:
                            qv = _fmaf__regclus(
                                _val__regclus(frags, s), SC, CQ
                            )  # bit-identical
                        if qv >= LOQ:
                            bn = _umin_u32__regclus(f2u_rz(qv), cutlass.Uint32(NB__regclus - 1))
                            p = atomic_add_cta(s_mrg.iterator + cutlass.Int32(bn), cutlass.Int32(1))
                            ...
                            if p < lim1:
                                ...
                                if whole == cutlass.Int32(0):
                                    ...
                                    if cutlass.const_expr(self.rc_p3):
                                        _st_shared_cluster_i32(
                                            _mapa_shared_cluster_addr(ck_addr + j, rnk),
                                            fkey(_val__regclus_raw(frags, s)),
                                        )
                                    else:
                                        _st_shared_cluster_i32(
                                            _mapa_shared_cluster_addr(ck_addr + j, rnk),
