...
def run(logits, pre_idx, n: int, out, ws):
    ...
    try:
        from . import gvr_topk_ss_host as ct_dispatch
    except ImportError:
        import gvr_topk_ss_host as ct_dispatch
    b, npad = logits.shape
    k = pre_idx.shape[1]
...
class GvrTopkRegKernel:
    ...
    def kern(
        ...
        if cutlass.const_expr(
            ...
            and self.varlen
            and self.minb < 8
            and not (self.deg and self.blk == 1024)
        ):
        ...
        if short == cutlass.Int32(0):
            ...
            if cutlass.const_expr(self.dtype == cutlass.Float32):
                ...
                if cutlass.const_expr(
                    self.varlen and self.minb < 8 and not (self.deg and self.blk == 1024)
                ):
...
def reg_topk(logits, pre_idx, n, out, rd=None):
    ...
    if rd is None:
        try:
            from .gvr_topk_ss_host import route
        except ImportError:
            from gvr_topk_ss_host import route
    ...
    try:
        from .gvr_topk_ss_host import _dummy_kv
    except ImportError:
        from gvr_topk_ss_host import _dummy_kv
