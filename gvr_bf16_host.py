...
def _device():
    ...
    if _dev_mod is None:
        try:
            from . import gvr_bf16_device as _m  # in-tree
        except ImportError:  # standalone dir
            import gvr_bf16_device as _m
        _dev_mod = _m
    return _dev_mod
...
# ===========================================================================
# ==== bf16 arm (native bf16 logits; fp32 tables above are untouched) =======
# ===========================================================================
"""bf16-arm dispatch + entry.

The bf16 arm dispatches through its OWN tables below (route_bf16 /
route_streaming_bf16); the fp32 route()/route_static()/route_dynamic()/
route_streaming()/route_bands() tables above are not edited or mutated.
Engines are compiled with dtype=BFloat16: the device code reads the bf16
logits natively (8-byte v2.b32 vector loads / 2-byte scalar gathers) and
widens each value to fp32 in registers (exact, bits << 16), so keys,
histograms and the sampling ladder are unchanged.

bf16-specific deviations from the fp32 tables (measured on the bf16 twin
captures; see the campaign notes):
  * tsh_en is set for EVERY SPLIT plan, not only k <= 1024: bf16 rows are
    tie-heavy and the sample rung can overshoot to count(>=T) < k at k=2048
    (v32 128k B=16/32 fell to whole-row narrowing, ~300 us); the staged
    rank-2*TGT floor closes it. Expected staged population ~2*aim <= 8192
    <= GCAP/2, so the slab one-pass consume still holds.
  * the kernel-side per-row TSH gate is n4 <= 36864 in the bf16 constexpr
    branch (fp32 keeps 32768 verbatim): the 128K envelope has n4 = 32771;
    40960 would also enable v32 256k and costs ~10% there.
"""
...
def route_bf16(b: int, n: int, npad: int, k: int) -> dict[str, object]:
    """bf16-arm dispatch table (family admission tiers).

    Adopts the fp32 family/geometry decisions (route() is a pure function
    used here as data) plus one bf16 rung:

    REGCLUS_WIDE — extended clustered-register admission. The fp32 table
    routes k > 1024 and n4 > CS*BLKC*VPT (pow2 VPT <= 4) to the SPLIT slab
    plan, whose publish -> fence -> arrival -> consume chain floors at
    7.5-8.2 us; the campaign's v32 shapes (k = 2048) and the 512k/256k rungs
    miss the register-cluster capacity by 4-16 vectors. The bf16 arm widens
    the VPT lattice to {1,2,3,4,5} and admits k <= 2048 (the bf16 kernel
    folds the first min(k, 2*BLK) elements into the bracket, so the
    count(>=Tv) >= k crossing invariant still holds); the cs=8 b>15 GPC
    packing veto is kept. Geometry only — the reg_clus algorithm family is
    unchanged."""
    plan = route(b, n, npad, k)
    # REGCLUS_REFIT: where the fp32 table already picked the clustered
    # register family, re-run the capacity walk with the fringe so the same
    # shape lands on a SMALLER vpt at the same cluster size (e.g. 256k_bs1
    # (4,8) -> (2,8)): fewer registers per thread, same element coverage.
    if plan["kernel"] == "reg_clus" and k <= 2048:
        n4r = n >> 2
        avr = 148 // (b if b > 0 else 1)
        amaxr = 1
        while (amaxr << 1) <= avr and amaxr < 8:
            amaxr <<= 1
        for v in (1, 2, 3, 4, 5):
            c = 1
            while c * BLKC * v + 64 < n4r:
                c <<= 1
            if c == 8 and b > 15:
                continue
            if c <= amaxr:
                if (v, c) != tuple(plan["tpl"][1:3]):
                    plan = dict(plan)
                    plan["tpl"] = (BLKC, v, c)
                    plan["grid"] = (c, b)
                    plan["cluster"] = c
                break
        return plan
    # REG_FRINGE: register-family ladder with the fringe clamp. Fires for
    # (a) shapes the fp32 table streams (b > 148, or wide misses just past a
    # register rung), and (b) fp32-chosen reg/regimg shapes where the ladder
    # lands a strictly smaller tile or a smaller CTA at equal tile — fewer
    # registers / more resident CTAs at the same element coverage. The bf16
    # kernel clamps n4 to BLK*VPT and the surplus rides the scalar tail.
    _reg_override = False
    if plan["kernel"] in ("reg", "regimg") and k <= 2048 and n > k:
        n4o = n >> 2
        if 256 < n4o <= 4096 + 255:
            ob, ov = plan["tpl"][0], plan["tpl"][1]
            if n4o <= 256 + 63:
                nb_, nv_ = 256, 1
            elif n4o <= 512 + 127:
                nb_, nv_ = 512, 1
                ...
                nb_, nv_ = 0, 0  # (1024,*) tiles lose at 1-2 CTAs/SM past 2 waves
            elif n4o <= 2048 + 255:
                nb_, nv_ = 1024, 2
            else:
                nb_, nv_ = 1024, 4
            if nb_ and ((nb_ * nv_ < ob * ov) or (nb_ * nv_ == ob * ov and nb_ < ob)):
                _reg_override = True
    if _reg_override or (
        plan["kernel"] == "main"
        and not plan["tpl"][5]
        ...
        if n4f <= 256 + 63:
            ...
            tpl_reg = (512, 2, 4, NB)
        elif b > 148 and not (b == 256 and npad == 8256):
            tpl_reg = None  # (1024,*) tiles lose past two waves except the 8K B=256 band
        elif n4f <= 2048 + 255:
            tpl_reg = (1024, 2, 2, 2 * NB)
        else:
            tpl_reg = (1024, 4, 1, 2 * NB)
        if tpl_reg is None:
            blkf, vptf, minbf, nbhf = 0, 0, 0, 0
        else:
            blkf, vptf, minbf, nbhf = tpl_reg
        if tpl_reg is not None and blkf * vptf * 4 >= k:  # hint-free bracket invariant
            cmp_f = n if n < 2560 else 2560
            qcf = 1024 if b > 148 else QUADC
            curef = not (n < 2 * k and b > 148)
            degef = (n <= 3 * k) or (n <= 4 * k + 64)
            if degef and cmp_f < n:
                cmp_f = n
            if degef:
                tplf = (blkf, vptf, minbf, 1, curef, True, False, nbhf)
            else:
                kptf = 1 if k <= blkf else (2 if k <= 2 * blkf else 4)
                tplf = (blkf, vptf, minbf, kptf, curef, False, False, nbhf)
            return {
                "kernel": "reg",
                "tpl": tplf,
                "rt": {
    ...
    if plan["kernel"] in ("main", "clus") and k <= 2048:
        n4 = n >> 2
        if 4096 < n4 <= 5 * BLKC * 8 + 64:
            ...
            cs = 0
            if amax >= 2:
                # 64-vector fringe: the bf16 kernel clamps its vector count to
                # the cluster register capacity and the surplus (<= 259
                # elements < BLK) rides the existing scalar tail, so the pow2
                # capacity ladder admits the campaign's pow2+15 rows at the
                # PRODUCTION register budget (vpt <= 4 everywhere but 256k).
                for v in (1, 2, 3, 4, 5):
                    c = 1
                    while c * BLKC * v + 64 < n4:
                        c <<= 1
                    if c == 8 and b > 15:  # GPC packing veto (fp32 parity;
                        continue  # measured: cs=8 at b=16 runs 21-24 us vs 17-18 on SPLIT)
                    if c <= amax:
                        vsel = v
                        cs = c
                        break
...
def route_streaming_bf16(
    ...
    """bf16-arm streaming table (see route_bf16). Identical decisions to the
    fp32 streaming table: measured bf16 deviations (U halving for 16-byte
    vectors, forced R=1 at small b, TSH staging for k > 1024) all lost time
    on the real captures and were removed."""
    plan = route_streaming(b, n, npad, k, force_main=force_main)
    # The production small-batch split chooses floor(148 / 16) == 9 CTAs
    # per row.  For long v32 rows this creates an extra publish participant
    # while the resulting 144-CTA grid gains little over a 128-CTA wave.
    # Use eight fuller CTAs and re-derive the per-CTA packet count; CMP/SCP
    # capacity only increases because U grows with Q.
    if (
        plan["kernel"] == "main"
        and plan["tpl"][5]
        and b == 16
...
def _varlen_launcher_bf16(
    ...
    """bf16 twin of _varlen_launcher (same admission tiers via the bf16
    tables; engines compiled with dtype=BFloat16; bf16 tsh_en deviation)."""
    key = (num_rows, npad, k, n_env, next_n, cr)
    ...
    amin = 3 * k if r_const == 2 else (7 * k) // 2
    # bf16 k>=1024 candidate-volume retune (R6).  The collect/verify (R==1)
    # and single-CTA merge (SPLIT) cost scales ~0.7 us per 1024 staged
    # candidates (measured flash->pro->v32 at the same R=8/U=4 geometry).
    # (a) R==1 big rows: 4*k over-provisions k>=1024 staging 4x; 2*k keeps a
    #     4-sigma sampling margin at SFAC=64 and the NATT=3 retry ladder
    #     backstops a miss (bounded ~1 extra pass, never the degen sweep).
    # (b) kbig SPLIT big rows (r_const>1): aim rides amin=3.5k into the
    #     SCPB/2=4096 device cap; 1.5k with SFAC 48->192 tightens the count
    #     sigma to ~aim/14 (4.6 sigma to the k-crossing; bf16 tie atoms in
    #     the v32 captures are ~200 << the 1024 margin) and cuts the merge
    #     tail by ~1024 candidates per row.  Host-only: aim_base/sfac/amin
    #     are runtime launch args, no new engine variants.
    if big and r_const == 1 and k >= 1024:
        # R=1 is the COLLECT path (NATT=3 retry ladder), so under-provision is
        # only a latency risk, never correctness: push aim to 1.5k (was 2k;
        # stock 4k) to shrink the CMP-staging/verify candidate volume further.
        aim_base = (3 * k) // 2
    if big and 2 < r_const <= 9 and k > 1024 and tpl[5]:
        aim_base = (3 * k) // 2
        amin = (3 * k) // 2
        sfac = 192
    elif big and r_const == 8 and k == 1024 and tpl[5]:
        # k=1024 SPLIT big twin of (b): amin 3.5k->2k with SFAC 48->128
        # (sigma ~aim/11.3, 5.7 sigma to the k-crossing).
        amin = 2 * k
        sfac = 128
    elif big and r_const == 8 and k == 512 and tpl[5]:
        # k=512 R=8 twin: amin 3.5k->2k, SFAC 48->96 (sigma ~aim/9.8,
        # 4.9 sigma to the k-crossing; SMP ~1.5 gather rounds).
        amin = 2 * k
        sfac = 96
    sd_en = 1 if (k > 1024 and not big) else 0
    # TSH-floor staging: same gate as the fp32 launcher (measured: enabling
    # it for k > 1024 costs 3-5 us on every v32 SPLIT row; the bf16 arm
    # instead removes SPLIT at small b entirely — see NOSPLIT_SMALL_B).
    tsh_en = 1 if (tpl[5] and k <= 1024) else 0
    pre = (0, npad, k, rt["SCAP_"], rt["CMP_"], r_const, 0, 0, 0, 0, 0)
    tail = (aim_base, sfac, amin, sd_en, tsh_en)
    lc = ("main", fn, pre, tail)
...
def run_varlen_bf16(
    ...
    """bf16 twin of run_varlen: native bf16 logits, per-request device
    kv_lens, ONE kernel launch, indices written in place. No values output
    (the bf16 arm has no gather epilogue)."""
    ...
    if max_seq_len is not None:
        ...
        if _is_capturing():
            raise RuntimeError(
                ...
                "host — pass max_seq_len under CUDA graph capture"
            )
        n_env = int(kv_lens.max().item()) >> cshift
        n_env = 1 << max(n_env - 1, 1).bit_length()
    n_env = min(max(n_env, 1), npad)
    ...
    if lc is None:
        if _is_capturing():
            raise RuntimeError(
                "bf16 varlen launcher not compiled for this shape — warm up "
                "before CUDA graph capture"
            )
    ...
    if idx.shape[1] != k:
        idx = idx.reshape(-1)[: num_rows * k].view(num_rows, k)
    pre_arg = idx  # hint-free engines never read the pre_idx ABI slot
    if lc[0] == "reg_clus":
        lc[1](lg, pre_arg, kv_lens, idx, lc[2])
    elif lc[0] == "reg":
        lc[1](lg, pre_arg, kv_lens, idx, *lc[2])
