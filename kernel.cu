...
#define LAUNCH_REGIMG(BLKV, VPTV, MINBV) \
        ...
        if (n4 > 4096 && n4 <= 8 * BLKC * 4 && k <= BLKC) {
            ...
            if (amax >= 2) {
                ...
                // most 15 cs=8 clusters are resident machine-wide.  b=16..18
                // x cs=8 spills to a SECOND cluster wave (measured:
                // flash_512k b15 10.1us -> b16 16.3us, +62%, same kernel).
                // Veto any such fit unconditionally: 128k/256k find a
                // deeper-VPT/smaller-cs single wave, 512k falls through to
                // streaming -- which knife5 made retry-safe (TSH-floor
                // staging on the slab path; gvr_clus has its own ladder), so
                // the L2w pass-1 exception is deleted and the rule is
                // uniform again.
                for (int v = 1; v <= 4; v <<= 1) {
