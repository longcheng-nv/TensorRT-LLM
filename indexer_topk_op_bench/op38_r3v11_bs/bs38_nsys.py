...
"""op38 verdict worker: candidate v2 on the §7b fp32 per-layer envelope.
...
from probe import build, bundle, make_batch, exact_rows  # noqa: E402
...
def main():
    ...
    if args.shard:
        ...
        cells = cells[i::m]
    print(f"[bs38] {len(cells)} cells", flush=True)
    ...
    for model, isl, L in cells:
        ...
        for bs in BS_LIST:
            ...
            if bad:
                print(f"[bs38] INEXACT {cname} BS{bs}: {bad}", flush=True)
            ...
            del lg, pre, out
        print(f"[bs38] {cname} done", flush=True)
    ...
    print(f"[bs38] ALL DONE, inexact {n_bad}/{len(exact_log)}", flush=True)
