...
"""op22rr+radix — add the report.html "Radix single-CTA (CUDA)" and
"Radix multi-CTA (CUDA)" arms (radix_single_cuda / radix_multi_cuda) to
the re-tested REPORT.html sections 1/2, and (re-)derive the mc + op25
backfill arms so the D-blob full replace stays idempotent regardless of
which updater ran before this one.

The radix arms were measured on umbriel-b200-027 (8-GPU shard) TOGETHER
with a co-located gvr_cutedsl anchor re-run (same bundles byte-for-byte,
same nsys cold-L2 protocol, OP22RR_ARMS filter), so radix times are
ANCHOR-TRANSFERRED onto the report's scale per cell:

    us_adj = us_radix(local) * us_base(orig) / us_base(local)  (cold & warm)

This updater is SELF-CONTAINED and must run LAST: it re-derives mc and
op25 rows from their own roots (same transfer), rewrites the full COL/
SHORT consts including every backfill arm, and inserts any missing
checkbox/method-note for mc + op25 + radix (skip-if-present), so the
final REPORT state is identical whether or not update_report_mc.py /
update_report_op25.py were applied first.

Usage: python3 update_report_radix.py [<radix_root>] [<orig_root>]
                                      [<mc_root>] [<op25_root>]
       defaults ../results_b200_op22rr_radix027  ../results_b200_op22rr
                ../results_b200_op22rr_mc074     ../results_b200_op25hls028
"""
import csv
import json
import statistics
...
RADIX_ROOT = Path(sys.argv[1]) if len(sys.argv) > 1 else \
    HERE.parents[0] / "results_b200_op22rr_radix027"
...
OP25_ROOT = Path(sys.argv[4]) if len(sys.argv) > 4 else \
...
BASE = "gvr_cutedsl"
# colors follow report/report.html (and REPORT §3) conventions
COL_MAP = {MC: "#2ec4b6", OP25: "#ff7f0e", RD1: "#ff8c42", RDM: "#e84855"}
SHORT_MAP = {MC: "GVR-mCTA", OP25: "HLS-op25",
             RD1: "Radix-1CTA", RDM: "Radix-mCTA"}
...
def _op25_note():
    return (
        '<div class="card" style="border-color:' + COL_MAP[OP25] + '" '
        'id="' + OP25_NOTE_ID + '">'
...
def _radix_note():
    return (
        '<div class="card" style="border-color:' + COL_MAP[RDM] + '" '
        'id="' + RADIX_NOTE_ID + '">'
        '<div class="i18n-en"><p><b>Radix-CUDA arms added 2026-07-09</b> '
        '(<code>radix_single_cuda</code> / <code>radix_multi_cuda</code> '
        '— the report.html "Radix single-CTA (CUDA)" and "Radix '
        'multi-CTA (CUDA)" reference operators, standalone CUDA '
        'radix-select top-K; multi-CTA = split+merge with '
        'default_blocks_per_row, both cr-aware). Measured on '
        + RADIX_NODE + ' (same B200 SKU, same bundles byte-for-byte, '
        'same nsys cold-L2 protocol, 20 cold / 50 warm reps, 8-GPU '
        'shard) TOGETHER with a co-located GVR-1CTA anchor re-run; '
        'plotted µs are anchor-transferred onto the original baseline '
        'scale (us_radix × us_base(orig)/us_base(local) per cell), so '
        'their speedup curves are same-node ratios. Per-cell exactness '
        'validated at BS=1. Raw local values in '
        '<a href="op22rr_radix_raw027.csv">op22rr_radix_raw027.csv</a>.'
        '</p></div>'
        '<div class="i18n-zh"><p><b>Radix-CUDA 两臂已于 2026-07-09 补充'
        '</b>（<code>radix_single_cuda</code> / '
        '<code>radix_multi_cuda</code> —— 即 report.html 的 "Radix '
        'single-CTA (CUDA)" 与 "Radix multi-CTA (CUDA)" 参考算子，'
        '独立 CUDA radix-select top-K；multi-CTA 为 split+merge，'
        'blocks_per_row 取内核默认，两者均感知压缩率 cr）。采集于 '
        + RADIX_NODE + '（同 B200 SKU、bundle 逐字节相同、nsys '
        'cold-L2 协议不变、20 cold / 50 warm reps、8 卡分片），并与 '
        'GVR-1CTA 锚点臂同机复测；图中 µs 已按 cell 锚点迁移到原基线'
        '刻度（us_radix × us_base(orig)/us_base(local)），因此其加速比'
        '曲线即同机比值。BS=1 处逐格精确性校验。本机原始值见 '
        '<a href="op22rr_radix_raw027.csv">op22rr_radix_raw027.csv</a>。'
        '</p></div></div>\n')
...
def patch_report(all_rows):
    ...
    # ---- 3. checkboxes: chain mc -> op25 -> radix1 -> radixm, each
    #         skip-if-present so any prior updater state is accepted ----
    ...
    if OP25_NOTE_ID not in t:
        t = sub1(t, anchor1, _op25_note() + anchor1, "op25 note before s1")
    if RADIX_NOTE_ID not in t:
        t = sub1(t, anchor1, _radix_note() + anchor1, "radix note before s1")

    REPORT.write_text(t)
    print(f"REPORT.html patched: D={len(all_rows)} rows "
          f"({sum(1 for r in all_rows if r['o'] == RD1)} rd1, "
          f"{sum(1 for r in all_rows if r['o'] == RDM)} rdm, "
          f"{sum(1 for r in all_rows if r['o'] == OP25)} op25, "
          f"{sum(1 for r in all_rows if r['o'] == MC)} mc)")


def write_csvs(all_rows, radix_raw_local, base_local):
    ops_all = OPS[:4] + [MC, OP25, RD1, RDM] + OPS[4:]
    by = {}
    for r in all_rows:
    ...
    # raw local side-file: radix arms + co-located anchor, unadjusted
    ...
    out = [head]
    for r in sorted(radix_raw_local, key=lambda r: (r["o"],) + key(r)):
        b = bl.get(key(r))
        out.append([r["s"], r["w"], r["K"], r["d"], r["N"], r["B"],
                    r["o"], r["c"], r["h"],
                    b["c"] if b else "", b["h"] if b else "",
                    round(b["c"] / r["c"], 4) if b else ""])
    with open(HERE / "op22rr_radix_raw027.csv", "w", newline="") as f:
        csv.writer(f).writerows(out)
    print(f"wrote op22rr_radix_raw027.csv ({len(out) - 1} rows)")


def main():
    gate_exactness(RADIX_ROOT, RD1)
    gate_exactness(RADIX_ROOT, RDM)
    ...
    patch_report(d_rows)
    write_csvs(orig_rows + mc_adj + op25_adj + rd_adj, rd_adj, base_rd)
