...
#
# op29 nsys driver — one .nsys-rep per (scenario, sweep, K) fp32 batch,
# resumable at BATCH granularity via .done_* markers. Clone of
# drive_nsys_op28.sh pointed at sweep_op29.py (gvr29_hbe report arm).
#
# Usage:
#   OUT=results_b200_op29 GPU=1 SCENARIOS="real" KS="512 1024" \
#     bash scripts/drive_nsys_op29.sh
set -u
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BENCH="$(cd "$HERE/../.." && pwd)"
OUT_REL="${OUT:?set OUT=results_b200_op29}"
...
echo "### drive_nsys_op29: host=$(hostname) GPU=$GPU OUT=$OUT reps=$REPS warm=$REPS_WARM rev=$(git rev-parse --short HEAD 2>/dev/null)"
  ...
  for sw in $SWEEPS; do
    ...
    for K in $KS; do
      ...
      if env -u GITHUB_TOKEN -u HF_TOKEN CUDA_VISIBLE_DEVICES="$GPU" \
           ...
           -o "$rep" -f true \
           python3 "$HERE/sweep_op29.py" --sweep "$sw" --scenario "$scen" \
...
echo "ALL OP29 NSYS BATCHES DONE  end=$(date -u +%FT%TZ)"
