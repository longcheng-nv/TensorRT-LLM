...
from cute_vendored.blackwell.utils import TRTLLM_ENABLE_PDL, griddepcontrol_launch_dependents, griddepcontrol_wait
from cute_vendored.blackwell.top_k.block_scan import warp_scan
...
class GvrTopKKernel:
    ...
    def __init__(
        ...
        # ~kNumBins^2 resolution of the K-th value and converges in ~0-1 iters
        # (vs mean 3.5 / max 15). Default False => byte-identical to baseline.
        self.enable_p4_fine_hist = enable_p4_fine_hist
        # P4 in-bin interpolation seed: replace the coarse K-th-bin LOW edge
        # (snap start, systematically below the K-th value → snap walks up many
        # distinct values) with a linear interpolation of the K-th value's
        # position inside the bin. Pure arithmetic, NO extra pass. Snap loop kept
        # as exactness guarantor. Default False => byte-identical baseline.
        self.enable_p4_interp_seed = enable_p4_interp_seed
        # P4 fused rank-and-scatter: dispatch phase4_rank_scatter instead of
        # phase4_histogram_snap (eliminates the iterative snap + 2-pass writeback,
        # ~14 barriers → ~7). See phase4_rank_scatter for the exactness note.
        self.enable_p4_rank_scatter = enable_p4_rank_scatter
        # Exact rank-scatter: resolve the straddling coarse bin by ONE fine-
        # histogram recursion (sub-bins over bin b*'s value range), so the
        # boundary is emitted in value order (vdiff→~0) at the cost of a few
        # barriers back. Requires enable_p4_rank_scatter.
        self.enable_p4_rank_scatter_exact = enable_p4_rank_scatter_exact
        # op14 ≈1-HBM-pass secant (radix-style candidate compaction). When True,
        # the P2 secant + P3 collect maintain a compacted survivor buffer in global
        # scratch so later count_ge evals + the P3 collect read the shrinking
        # survivor set instead of re-streaming all N. compaction_C sets the smem
        # candidate cap as C*K (convergence when survivors ≤ C*K → refine in smem).
        # Default False => byte-identical to the rank-scatter baseline.
        # NOTE: behavior is implemented incrementally by /omni-kernel; the flag is
        # currently a stored no-op so the op wrapper compiles against both paths.
        self.enable_1pass_compaction = enable_1pass_compaction
        self.compaction_C = compaction_C
    ...
    # ==================================================================
    # op14 ≈1-HBM-pass compaction fast-path helpers (enable_1pass_compaction)
    #
    # Global scratch cand_val[cap] (fp32) + cand_idx[cap] (int32) hold the
    # survivors of the pass-1 count+compact (all elements >= t0=pmin). The
    # secant + P3 collect then operate over cand_val[0:c0] (c0 << N) so HBM
    # traffic drops from ~3N to ~N + a few*c0. Exact iff K <= c0 <= cap; else
    ...
    def fused_count_compact(
        ...
        N,
        threshold,       # cutlass.Float32 (= pmin)
        cand_val,        # cute.Tensor [cap] fp32 (global scratch, per-row view)
        cand_idx,        # cute.Tensor [cap] int32 (global scratch, per-row view)
        cap,             # cutlass.Int32 scratch capacity
        smem_wcnt,       # cute.Tensor [NUM_WARPS] int32 (block-reduce scratch)
        s_iscalars,      # cute.Tensor [5] int32 (writes [0] = c0 true count)
        s_slot,          # cute.Tensor [1] int32 (smem slot allocator)
        tidx,
        warp_id,
        lane,
    ):
        """Pass-1: stream all N once; count v>=t0 (uncapped → c0) and compact
        survivors to global scratch via an smem atomic slot allocator (capped
        at cap). Writes s_iscalars[0] = true uncapped count c0.

        Returns nothing; result in s_iscalars[0] and cand_val/cand_idx[0:min(c0,cap)].
        """
        num_threads = cutlass.const_expr(self.num_threads)
        vec_w = cutlass.const_expr(self.vec_bits // self.dtype.width)
        elem_bytes = cutlass.const_expr(self.dtype.width // 8)
    ...
    @cute.jit
    def block_count_ge_scratch(
        self,
        cand_val,        # cute.Tensor [cap] fp32 (global scratch)
        c0,              # cutlass.Int32 number of valid survivors
        threshold,       # cutlass.Float32
        smem_ptcnt,      # cute.Tensor [BLOCK_SIZE] int32 (P3 cache)
        smem_wcnt,       # cute.Tensor [NUM_WARPS] int32
        s_iscalars,      # cute.Tensor [5] int32 (writes [0] = count)
        tidx,
        warp_id,
        lane,
    ):
        """Count cand_val[i] >= threshold over the c0 survivors. fp32-only,
        scalar-strided (c0 ~ a few thousand << N, so vectorization is
        unnecessary; keeps the gmem read minimal)."""
        num_threads = cutlass.const_expr(self.num_threads)
        c = cutlass.Int32(0)
        i = tidx
        while i < c0:
            if cand_val[i] >= threshold:
                c = c + cutlass.Int32(1)
    ...
    @cute.jit
    def phase2_secant_search_scratch(
        self,
        cand_val,
        c0,
        smem_ptcnt,
        ...
        """Secant search identical to phase2_secant_search but evaluating
        count_ge over cand_val[0:c0] instead of full N."""
        kK = cutlass.const_expr(self.top_k)
        kCC = cutlass.const_expr(self.kC)
        kFTarget = cutlass.const_expr(self.kFTarget)

        thr_init = s_thr[0]
        self.block_count_ge_scratch(
            cand_val, c0, thr_init, smem_ptcnt, smem_wcnt, s_iscalars,
            tidx, warp_id, lane,
        )
        cute.arch.barrier()
        ...
        if tidx == 0:
            c_ini = s_iscalars[0]
            t0 = s_thr[0]
            if c_ini >= cutlass.Int32(kK) and c_ini <= cutlass.Int32(kCC):
                s_iscalars[1] = cutlass.Int32(1)
            elif c_ini > cutlass.Int32(kCC):
                s_thr[1] = t0
                s_iscalars[2] = c_ini
            else:
                s_thr[2] = t0
        ...
        while it < cutlass.Int32(self.MAX_REFINE_ITERS) and s_iscalars[1] == cutlass.Int32(0):
            ...
            if s_iscalars[1] == cutlass.Int32(0):
                new_thr = s_thr[0]
                self.block_count_ge_scratch(
                    cand_val, c0, new_thr, smem_ptcnt, smem_wcnt, s_iscalars,
                    tidx, warp_id, lane,
                )
                cute.arch.barrier()
                if tidx == 0:
    ...
    def phase3_collect_scratch(
        ...
        """Collect survivors cand_val>=thr from scratch into smem keys/vals.

        Mirrors phase3_collect_candidates (retry-shrink + prefix-sum write)
        but reads the c0 scratch survivors instead of full N."""
        kK = cutlass.const_expr(self.top_k)
        kCC = cutlass.const_expr(self.kC)
        num_threads = cutlass.const_expr(self.num_threads)

        # ---- Retry-shrink (only if P2 didn't converge cleanly) ----
        if s_iscalars[1] != cutlass.Int32(1):
            cur_thr = s_thr[0]
            self.block_count_ge_scratch(
                cand_val, c0, cur_thr, smem_ptcnt, smem_wcnt, s_iscalars,
                tidx, warp_id, lane,
            )
            cute.arch.barrier()
            if tidx == 0:
        ...
        # ---- Stream-write loop over scratch survivors ----
        thr_final = s_thr[0]
        wc = my_write_pos
        i = tidx
        while i < c0:
    ...
    def phase4_histogram_snap(
        ...
        if cand_count == cutlass.Int32(kK):
            ...
            if warp_id == target_warp and lane == cutlass.Int32(0):
                ...
                for jb2 in cutlass.range_constexpr(bins_per_warp):
                    ...
                    if base_cum >= cutlass.Int32(kK) and set_done == cutlass.Int32(0):
                        if cutlass.const_expr(self.enable_p4_interp_seed):
                            # In-bin interpolation: estimate the K-th value's
                            # position inside the winning bin and seed the snap
                            # there (instead of the bin's LOW edge). cum_before =
                            # count >= bin high edge = base_cum - this bin's count.
                            binw = range1 / cutlass.Float32(kBins)
                            f_lo2 = bmin_local + cutlass.Float32(bidx2) * binw
                            bc = smem_hist[bidx2]
                            cum_before2 = base_cum - bc
                            # rank of K-th from the bin's TOP edge, in (0, bc].
                            rank_in = cutlass.Float32(cutlass.Int32(kK) - cum_before2)
                            frac = rank_in / cutlass.Float32(bc)  # bc >= 1 here
                            thr_local = (f_lo2 + binw) - frac * binw
                            if thr_local < f_lo2:
                                thr_local = f_lo2
                            if thr_local > f_lo2 + binw:
                                thr_local = f_lo2 + binw
                        else:
            ...
            # ---- P4 recursive-digit: fine histogram over the coarse K-th bin ----
            # (bucket p4_recursive_digit) Gated by enable_p4_fine_hist; const_expr
            # so baseline (flag False) emits NOTHING → byte-identical. Narrows
            # s_thr[0] from the coarse K-th-bin low edge to a kNumBins^2-resolution
            # sub-bin edge, so the snap loop below converges in ~0-1 iters. The
            # snap loop is kept as the exactness guarantor → correctness uncond.
            if cutlass.const_expr(self.enable_p4_fine_hist):
                f_lo = s_thr[0]                       # coarse K-th bin low edge
                fstep = range1 / cutlass.Float32(kBins)  # coarse bin width
                f_hi = f_lo + fstep
                # zero histogram for reuse as the fine histogram
                izf = tidx
                while izf < cutlass.Int32(kBins):
                    smem_hist[izf] = cutlass.Int32(0)
                    izf = izf + cutlass.Int32(num_threads)
                cute.arch.barrier()
                finv = (cutlass.Float32(kBins - 1) + cutlass.Float32(0.99)) / fstep
                # build fine hist of candidates in [f_lo, f_hi); also count v >= f_hi
                local_above = cutlass.Int32(0)
                ibf = tidx
                while ibf < cand_count:
                    vf = smem_keys[ibf]
                    if vf >= f_hi:
                        local_above = local_above + cutlass.Int32(1)
                    elif vf >= f_lo:
                        sbf = (vf - f_lo) * finv
                        sbi = cutlass.Int32(sbf)
                        if sbi < cutlass.Int32(0):
                            sbi = cutlass.Int32(0)
                        if sbi > cutlass.Int32(kBins - 1):
                            sbi = cutlass.Int32(kBins - 1)
                        atomicAdd(smem_hist.iterator + sbi, cutlass.Int32(1))
                    ibf = ibf + cutlass.Int32(num_threads)
                # reduce local_above across block → s_iscalars[2] (= cum_above)
                aw = self.warp_reduce_sum_i32(local_above)
                if lane == cutlass.Int32(0):
                    smem_wcnt[warp_id] = aw
                cute.arch.barrier()
                if tidx == cutlass.Int32(0):
                    tot_above = cutlass.Int32(0)
                    for wA in cutlass.range_constexpr(self.num_warps):
                        tot_above = tot_above + smem_wcnt[wA]
                    s_iscalars[2] = tot_above
                cute.arch.barrier()
                # 3-step high→low bin search over the fine histogram
                fwarp_sum = cutlass.Int32(0)
                for jbf in cutlass.range_constexpr(bins_per_warp):
                    bidf = (cutlass.Int32(kBins - 1) - warp_id * cutlass.Int32(bins_per_warp)
                            - cutlass.Int32(jbf))
                    fwarp_sum = fwarp_sum + smem_hist[bidf]
                if lane == cutlass.Int32(0):
                    smem_wcnt[warp_id] = fwarp_sum
                cute.arch.barrier()
                if tidx == cutlass.Int32(0):
                    cum_f = s_iscalars[2]            # cum_above seed
                    twf = cutlass.Int32(num_warps - 1)
                    foundf = cutlass.Int32(0)
                    for w2f in cutlass.range_constexpr(self.num_warps):
                        cum_f = cum_f + smem_wcnt[w2f]
                        if cum_f >= cutlass.Int32(kK) and foundf == cutlass.Int32(0):
                            twf = cutlass.Int32(w2f)
                            foundf = cutlass.Int32(1)
                    cum2f = s_iscalars[2]           # prefix incl cum_above
                    for w3f in cutlass.range_constexpr(self.num_warps):
                        if cutlass.Int32(w3f) < twf:
                            cum2f = cum2f + smem_wcnt[w3f]
                    s_iscalars[2] = cum2f
                    s_iscalars[3] = twf
                cute.arch.barrier()
                twf2 = s_iscalars[3]
                if warp_id == twf2 and lane == cutlass.Int32(0):
                    base_f = s_iscalars[2]
                    thr_f = f_lo
                    set_f = cutlass.Int32(0)
                    for jb3 in cutlass.range_constexpr(bins_per_warp):
                        bidf2 = (cutlass.Int32(kBins - 1) - twf2 * cutlass.Int32(bins_per_warp)
                                 - cutlass.Int32(jb3))
                        base_f = base_f + smem_hist[bidf2]
                        if base_f >= cutlass.Int32(kK) and set_f == cutlass.Int32(0):
                            thr_f = f_lo + cutlass.Float32(bidf2) * fstep / cutlass.Float32(kBins)
                            set_f = cutlass.Int32(1)
                    s_thr[0] = thr_f
                cute.arch.barrier()
    ...
    # ------------------------------------------------------------------
    # Phase 4 ALT (bucket p4_recursive_digit): fused rank-and-scatter.
    # Replaces the iterative snap loop + 2-pass writeback with:
    #   histogram → high→low prefix → each candidate computes its global rank
    #   (rank_above(bin) + within-bin atomic) and scatters to output[rank] if
    #   rank < K, in ONE pass. ~7 barriers vs snap path's ~14. Branch A/C
    #   identical to phase4_histogram_snap.
    # EXACTNESS NOTE: the straddling bin (bin holding rank K) is emitted in
    # arbitrary within-bin order — exact iff its values are tied OR
    # count[b*] ≤ (K − rank_above). For continuous (Beta) data with distinct
    # values in b* this can mis-select at the boundary; vdiff is MEASURED to
    ...
    def gvr_topk_kernel(
        ...
        cand_val: cute.Tensor,  # [numRows, cap] fp32 global scratch (op14) or None
        cand_idx: cute.Tensor,  # [numRows, cap] int32 global scratch (op14) or None
    ):
        ...
        # op14: smem slot allocator for fused pass-1 compaction (1 int32).
        if cutlass.const_expr(self.enable_1pass_compaction):
            s_slot = smem.allocate_tensor(
                element_type=cutlass.Int32,
        ...
        if N <= cutlass.Int32(top_k):
            ...
            if v_hi <= cutlass.Float32(self.NEG_FLT_MAX) or v_lo >= v_hi:
                ...
                if cutlass.const_expr(self.enable_1pass_compaction):
                    # =========================================================
                    # op14 ≈1-HBM-pass fast path:
                    #   Pass-1 fused count+compact survivors (>= pmin) to global
                    #   scratch; gate K <= c0 <= cap; run P2/P3 over scratch
                    #   (<< N). Fall back to baseline full-N P2/P3 otherwise.
                    # =========================================================
                    cand_val_row = cand_val[row_idx, None]
                    cand_idx_row = cand_idx[row_idx, None]
                    cap = cand_val.shape[1]
                    t0 = s_thr[1]  # pmin (conservative low threshold)
                    self.fused_count_compact(
                        input_row,
                        N,
                        t0,
                        cand_val_row,
                    ...
                    c0 = s_iscalars[0]
                    fast_ok = (
                        c0 >= cutlass.Int32(top_k) and c0 <= cutlass.Int32(cap)
                    )
                    if fast_ok:
                        # Reset bracket/done state seeded identically to P1 exit
                        # (fused_count_compact reused s_iscalars[0] as the count).
                        if tidx == 0:
                            s_iscalars[1] = cutlass.Int32(0)  # done
                            s_iscalars[2] = cutlass.Int32(
                                top_k + (top_k >> 2)
                            )  # cnt_lo
                            s_iscalars[3] = cutlass.Int32(1)  # cnt_hi
                            s_iscalars[4] = cutlass.Int32(0)  # out_count
                        cute.arch.barrier()
                        self.phase2_secant_search_scratch(
                            cand_val_row,
                            c0,
                            smem_ptcnt,
                            smem_wcnt,
                        ...
                        # FALLBACK: reset state and run the baseline full-N path.
                        if tidx == 0:
                            s_iscalars[0] = cutlass.Int32(0)
                            s_iscalars[1] = cutlass.Int32(0)
                            s_iscalars[2] = cutlass.Int32(
                        ...
                        self.phase2_secant_search(
                            input_row, N, smem_ptcnt, smem_wcnt,
                            s_thr, s_iscalars, tidx, warp_id, lane,
                        )
                        self.phase3_collect_candidates(
                            input_row, N, smem_keys, smem_vals, smem_ptcnt,
    ...
    def __call__(
        ...
        stream,
        cand_val: cute.Tensor = None,  # op14 global scratch (fp32) or None
        cand_idx: cute.Tensor = None,  # op14 global scratch (int32) or None
    ):
