/// \file topk_impl_exact.cuh  [op36 Track B]
/// \brief DeepSeek-V4 (DSA indexer) top-k implementation classes — EXACTNESS
/// PORT of the vendored ops/sglang_v2 header (sglang@main 2026-07-13).
///
/// Delta vs upstream (marked [op36-TB]): sglang v2 is conditionally exact —
/// the tie collect silently truncates at kMaxNumTie=2048 (real V3.2 L52
/// already crosses the cap, op28 §8.1). This port adds a per-row overflow
/// flag (one st.global.u8) at every truncation site; the host wrapper
/// re-runs flagged rows through the unconditionally-exact in-tree radix
/// path. Flag sites: Register/Streaming/Cluster-primary phase 4
/// (equal_count > kMaxNumTie with tie slots still needed, or above_count >
/// topk), and Cluster non-primary local collect (per-rank cap — catches the
/// edge where one rank overflows but capped contributions sum to exactly
/// kMaxNumTie at rank 0). Everything else is verbatim upstream.
///
...
struct TopKProblem {
  ...
  uint32_t seq_len;
  uint32_t page_bits;
  // [op36-TB] per-row exactness-overflow flag (nullptr = guard disabled).
  uint8_t* __restrict__ overflow = nullptr;

  // [op36-TB] mark this row inexact (tie collect truncated); host re-runs it
  // through the exact radix path. Rare-by-data, so an unconditional st.global
  // from one thread is fine.
  SGL_DEVICE void flag_overflow() const {
...
#pragma unroll
    ...
    // [op36-TB] exactness guard: ties were dropped while tie slots still need
    // filling, or the above set overran topk (threshold invariant breach).
    if (tx == 0 && ((equal_count > kMaxNumTie && remain_topk > 0) || above_count > topk))
      problem.flag_overflow();
 ...
 public:
  ...
  SGL_DEVICE static void forward(TopKProblem problem, void* _smem) {
    ...
    if (!is_primary) {
      ...
      // [op36-TB] per-rank cap: this rank's local ties already truncated.
      // Conservative (rank can't see remain_topk) — over-triggering only
      // costs a rare host re-run, never exactness.
      if (tx == 0 && smem->count_eq > kMaxNumTie) problem.flag_overflow();
...
#pragma unroll
      ...
      const auto remain_topk = above_count < topk ? topk - above_count : 0;
      const auto tie_count = min(equal_count, kMaxNumTie);
      // [op36-TB] exactness guard (cluster rank 0): count_eq here is the
      // uncapped rank-0 collect count plus every peer's capped contribution,
      // so any staging-buffer drop shows up as equal_count > kMaxNumTie.
      if (tx == 0 && ((equal_count > kMaxNumTie && remain_topk > 0) || above_count > topk))
        problem.flag_overflow();
      handle_tie(smem->tie.values, problem, above_count, tie_count, remain_topk, &smem->tie.handle);
    }
