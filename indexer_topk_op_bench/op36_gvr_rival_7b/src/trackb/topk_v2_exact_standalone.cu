/// [op36 Track B] Standalone driver for the EXACTNESS PORT of the SGLang
/// DeepSeek-V4 top-K v2 — identical to ops/sglang_v2/topk_v2_standalone.cu
/// except: device classes come from topk_impl_exact.cuh (per-row tie-overflow
/// flag, see that header), TopKLaunchParams/plan/transform carry an
/// overflow_flags [B] u8 tensor (zeroed inside the UNTIMED plan kernel so the
/// timed transform pays only the rare st.global.u8), and the Python wrapper
/// re-runs flagged rows through the exact in-tree radix path.
///
/// Upstream provenance of the base file:
/// Standalone driver for the LATEST SGLang DeepSeek-V4 top-K (v2 architecture),
...
#include "topk_impl_exact.cuh"  // [op36-TB] patched copy; sgl_kernel/* stays vendored
...
struct TopKLaunchParams {
  ...
  uint8_t* __restrict__ overflow_flags;  // [op36-TB] per-row exactness flag [B]; nullptr disables
  ...
  SGL_DEVICE TopKProblem problem(uint32_t batch_id, uint32_t seq_len) const {
    ...
    return TopKProblem{
        ...
        .seq_len = seq_len,
        .page_bits = page_bits,
        .overflow = overflow_flags != nullptr ? overflow_flags + batch_id : nullptr,  // [op36-TB]
    };
  }
...
__global__ __launch_bounds__(kBlockSize, 1) void topk_plan(
    ...
    uint8_t* __restrict__ overflow_flags) {  // [op36-TB] zeroed here (plan is untimed)
  // [op36-TB] reset the per-row exactness flags inside the untimed plan
  // kernel so the timed transform pays only the rare st.global.u8 set.
  if (overflow_flags != nullptr) {
    for (uint32_t i = threadIdx.x; i < batch_size; i += kBlockSize) overflow_flags[i] = 0;
  }
...
// overflow_flags [B] uint8 ([op36-TB]; zeroed by the plan kernel).
void topk_v2_plan(torch::Tensor seq_lens, torch::Tensor metadata,
                  int64_t static_cluster_threshold, torch::Tensor overflow_flags) {
  ...
  TORCH_CHECK(overflow_flags.is_cuda() && overflow_flags.scalar_type() == torch::kUInt8 &&
                  overflow_flags.numel() >= batch_size,
              "overflow_flags must be CUDA uint8 [>=B]");
  ...
  host::LaunchKernel(1, kBlockSize, stream)(
      ...
      overflow_flags.data_ptr<uint8_t>());
}
...
void topk_v2_transform(torch::Tensor scores, torch::Tensor seq_lens,
                       ...
                       int64_t max_seq_len, torch::Tensor overflow_flags) {
  ...
  TORCH_CHECK(overflow_flags.is_cuda() && overflow_flags.scalar_type() == torch::kUInt8 &&
                  overflow_flags.numel() >= batch_size,
              "overflow_flags must be CUDA uint8 [>=B]");  // [op36-TB]
  ...
  const auto params = TopKLaunchParams{
      ...
      .overflow_flags = overflow_flags.data_ptr<uint8_t>(),  // [op36-TB]
  };
