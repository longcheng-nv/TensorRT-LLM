...
# [op36-A2] distP4 DSMEM reduction primitives. Relaxed cluster-scope atomics
...
class GvrTopKKernel:
    ...
    def __init__(
        ...
        dist_p4: Optional[bool] = None,  # [op36-A2]
    ):
        ...
        # and the hist build pass (~3 barriers + 2 cand passes). Selection
        # exactness does NOT depend on bin quality (rank logic + fine
        # recursion unchanged). cs=1 + enable_r0 + rank-scatter only.
        self._p4_fused_hist_req = p4_fused_hist
        ...
        # op35 iter2a resolve (must follow enable_p4_rank_scatter):
        self.p4_fused_hist = (bool(self._p4_fused_hist_req)
                              and cluster_size == 1
                              and bool(enable_r0)
                              and self.enable_p4_rank_scatter)
        ...
        # [op36-A2] distP4: cluster-distributed Phase 4 (hist build + fine
        ...
        # the exact rank-scatter P4 at cluster_size > 1; incompatible with the
        # oracle/fused-hist diagnostics. Raise loudly instead of silently
        # disabling so a mis-built variant can't masquerade as the dist arm.
        self.dist_p4 = bool(dist_p4) if dist_p4 is not None else False
        if self.dist_p4 and (
            cluster_size == 1
            or not self.enable_p4_rank_scatter_exact
            or self.p4_oracle_skip
            or bool(self._p4_fused_hist_req)
        ):
            raise ValueError(
                "dist_p4=True requires cluster_size > 1 and "
                "enable_p4_rank_scatter_exact=True, and is incompatible with "
                "p4_oracle_skip / p4_fused_hist; got "
                f"cluster_size={cluster_size}, "
                f"enable_p4_rank_scatter_exact={self.enable_p4_rank_scatter_exact}, "
                f"p4_oracle_skip={self.p4_oracle_skip}, "
                f"p4_fused_hist={bool(self._p4_fused_hist_req)}"
            )
    ...
    def phase3_collect_candidates(
        ...
        do_cluster_sync,  # bool: False = cs=1 / short-row degrade (skip cluster sync)
        smem_input=None,  # optional SMEM-cached slice
        smem_hist=None,   # op35 iter2a: fused coarse hist (p4_fused_hist)
        s_p4f=None,       # op35 iter2a: [0]=pmax bin upper bound
    ):
        ...
        # the single-CTA path.
        thr_final = s_thr[0]
        # op35 iter2a: zero the coarse hist, then bin every written candidate.
        # fh_inv/fh_bmin are warp-uniform registers (same math P4 re-derives).
        fh_bmin = cutlass.Float32(0.0)
        fh_inv = cutlass.Float32(0.0)
        if cutlass.const_expr(self.p4_fused_hist):
            izf = tidx
            while izf < cutlass.Int32(cutlass.const_expr(self.kNumBins)):
                smem_hist[izf] = cutlass.Int32(0)
                izf = izf + cutlass.Int32(num_threads)
            fh_bmin = thr_final
            fh_bmax = s_p4f[0]
            if fh_bmax <= fh_bmin:
                fh_bmax = fh_bmin + cutlass.Float32(1e-6)
            fh_inv = (cutlass.Float32(cutlass.const_expr(self.kNumBins) - 1)
                      + cutlass.Float32(0.99)) / (fh_bmax - fh_bmin)
            cute.arch.barrier()
        vec_w = cutlass.const_expr(self.vec_bits // self.dtype.width)
        elem_bytes = cutlass.const_expr(self.dtype.width // 8)
        vec_align = cutlass.const_expr(self.vec_align_bytes)
        ...
        slice_len = slice_end - slice_start
        # op35 diagnostic: shrink the collect-scan bound (WRONG OUTPUT).
        if cutlass.const_expr(self.p3_oracle_frac is not None):
            _frac_i = cutlass.const_expr(int(float(self.p3_oracle_frac) * 1024))
            slice_len = (slice_len * cutlass.Int32(_frac_i)) // cutlass.Int32(1024)
                ...
                for k in cutlass.range(big_iters, unroll=4):
                    ...
                    for j in cutlass.range_constexpr(vec_w):
                        ...
                        if vj >= thr_final and wc < cutlass.Int32(kCC):
                            ...
                            if cutlass.const_expr(self.p4_fused_hist):
                                bfh = cutlass.Int32((vj - fh_bmin) * fh_inv)
                                if bfh < cutlass.Int32(0):
                                    bfh = cutlass.Int32(0)
                                if bfh > cutlass.Int32(cutlass.const_expr(self.kNumBins) - 1):
                                    bfh = cutlass.Int32(cutlass.const_expr(self.kNumBins) - 1)
                                atomicAdd(smem_hist.iterator + bfh, cutlass.Int32(1))
        ...
        while it < N_local:
            ...
            if v >= thr_final and wc < cutlass.Int32(kCC):
                ...
                wc = wc + cutlass.Int32(1)
                if cutlass.const_expr(self.p4_fused_hist):
                    bfh = cutlass.Int32((v - fh_bmin) * fh_inv)
                    if bfh < cutlass.Int32(0):
                        bfh = cutlass.Int32(0)
                    if bfh > cutlass.Int32(cutlass.const_expr(self.kNumBins) - 1):
                        bfh = cutlass.Int32(cutlass.const_expr(self.kNumBins) - 1)
    ...
    def phase4_rank_scatter(
        ...
        if cand_count == cutlass.Int32(kK):
            ...
            bmax_r = cutlass.Float32(0.0)
            if cutlass.const_expr(self.p4_fused_hist):
                # op35 iter2a: hist was built during P3 with bmin=thr_final,
                # bmax=P1 pmax snapshot — re-derive the SAME binning transform
                # and skip the min/max pass + zero + build (~3 barriers).
                bmin_r = s_thr[0]
                bmax_r = s_p4f[0]
                if bmax_r <= bmin_r:
                ...
                while i7 < cand_count:
                    vk = smem_keys[i7]
                    bin_i = cutlass.Int32((vk - bmin_r) * inv1b)
    ...
    # ------------------------------------------------------------------
    # [op36-A2] Phase 4 (dist): cluster-distributed exact rank-and-scatter.
    ...
    # the old gather + tie radix re-rank over the leader's buffer). The
    # scatter/search math is replicated VERBATIM from phase4_rank_scatter.
            ...
            if is_leader:
                ...
                if cutlass.const_expr(self.p4_exact_tail):
                    ...
                    if cnt_strad > need0 and need0 > cutlass.Int32(0):
                        ...
                        # with the OLD exact-tail radix block over the
                        # gathered count.
                        ...
                        # ---- OLD exact-tail block (verbatim, over the
                        # gathered cand count; bmin_r/inv1/b_star/f_lo/finv/
                        # sb_star/ra_fine all live) ----
                        if tidx == cutlass.Int32(0):
                            smem_hist[256] = cutlass.Int32(0)
                            smem_hist[257] = need0
                            smem_hist[258] = cutlass.Int32(0)
    ...
    def run_one_row(
        ...
        # [op36-A2] distP4 per-CTA DSMEM scratch (same offset on every CTA —
        ...
        if cutlass.const_expr(cluster_size > 1 and self.dist_p4):
            ...
            s_dp4 = None
        # op35 iter2a: fused-hist scalar snapshot [0]=pmax (bin upper bound)
        if cutlass.const_expr(self.p4_fused_hist):
            s_p4f = smem.allocate_tensor(
                element_type=cutlass.Float32,
                layout=cute.make_ordered_layout((1,), order=(0,)),
                byte_alignment=16,
            )
    ...
    def _run_phases(
        ...
        lane,
        s_p4f=None,
        s_dp4=None,  # [op36-A2] distP4 DSMEM scratch (cs>1 + dist_p4 only)
    ):
        ...
        v_lo = s_thr[1]
        v_hi = s_thr[2]
        # op35 iter2a: snapshot P1's pmax before any bracket mutation — the
        # fused-hist bin upper bound (values above it clamp into the top bin).
        if cutlass.const_expr(self.p4_fused_hist):
            if tidx == cutlass.Int32(0):
                s_p4f[0] = v_hi
        if cutlass.const_expr(self.floor_oracle):
            ...
            # [op36-A2] Also skipped under dist_p4: phase4_rank_scatter_dist
            # owns its own SYNC1..SYNC6 sequence (no handoff2 gather at all).
            if cutlass.const_expr(
                cluster_size > 1 and not self.p4_oracle_skip and not self.dist_p4
            ):
                if do_cluster_sync:
                    cute.arch.cluster_arrive()
                    cute.arch.cluster_wait()
            ...
            if cutlass.const_expr(self.p4_oracle_skip):
                ...
                # [op36-A2] distP4: ALL cluster CTAs enter the distributed
