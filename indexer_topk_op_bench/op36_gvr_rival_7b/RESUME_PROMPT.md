...
- DELIVERED (07-18, same session): REPORT.html (bilingual, generated by
  analysis/gen_report_op36.py — idempotent last-writer, recomputes every
  table from results/; ship composite 1.017 reproduced from data) +
  PROD_PORT_PLAN.md (artifact->destination map, escape wiring options,
  PR-A sgl_bx+dispatch / PR-B gvr flags split, open decisions O1-O6).
- REMAINING: execute the port per PROD_PORT_PLAN.md when user green-lights
  upstream scope. LEARNINGS captured in REPORT §7: 8-conc eps artifact;
  distP4 sync-tax-vs-work N-boundary ~160k.
