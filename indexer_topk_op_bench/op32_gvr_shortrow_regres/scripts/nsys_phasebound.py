# nsys phase-bound: baseline vs MAX_REFINE_ITERS=0 (bounds the secant-refine cost
# = the barrier chain a barrier-cheapening rewrite could at most remove).
# Run ONE variant per process (REFINE env); wrap in nsys; parse kern time.
import os,sys
...
scen=os.environ.get("SCEN","real"); REFINE=int(os.environ.get("REFINE","15"))
REPS=int(os.environ.get("REPS","60"))
dt=torch.float32
ul,kcc,kft,s2=dispatch_p2_op26(dt,K,N); rs=dispatch_rs_op26(dt,1); use256=(N>=16384)
ko=GvrOp26Kernel(dtype=_DT[dt],top_k=K,next_n=1,num_threads=512,compress_ratio=1,
    use_256bit_load=use256,enable_unroll_4=True,enable_phase3_unroll=True,min_blocks_per_mp=1,
    return_output_values=False,enable_p4_rank_scatter=rs,enable_p4_rank_scatter_exact=rs,
    p2_log=ul,kC_override=kcc,kFTarget_override=kft,p2_secant2=s2,fb_fix=True)
ko.MAX_REFINE_ITERS=REFINE
...
for _ in range(REPS):
    flush(); comp(lg,pre,seq,None,out)
torch.cuda.synchronize()
torch.cuda.cudart().cudaProfilerStop()
print(f"done REFINE={REFINE} N={N} K={K} scen={scen}")
