# time gvr_ms_auto (op27_hls family) under an env-config, nsys window.
import os,sys
...
sel=lg[0].gather(0,o[0].long()).float().sort().values; ref=torch.topk(lg[0][:N].float(),K).values.sort().values
ex=bool(torch.equal(sel,ref)) and len(set(o[0].tolist()))==K
open(os.environ["EXFILE"],"a").write(f"{os.environ['TAG']} exact={ex}\n")
