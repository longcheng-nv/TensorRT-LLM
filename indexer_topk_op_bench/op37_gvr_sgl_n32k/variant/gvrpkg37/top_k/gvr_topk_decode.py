...
# [op37-dp4] distP4 DSMEM reduction primitives. Relaxed cluster-scope atomics
# on a peer CTA's SMEM via a mapa.shared::cluster address; ordering is
# provided by the surrounding release cluster_arrive() + acquire
# cluster_wait() pairs (NEVER cluster_arrive_relaxed on this path).
@dsl_user_op
def _red_shared_cluster_add_i32(mapped_addr, val, *, loc=None, ip=None):
    """Fire-and-forget int32 add into a peer CTA's SMEM (no return value)."""
    llvm.inline_asm(
        None,
...
def _atom_shared_cluster_add_i32(mapped_addr, val, *, loc=None, ip=None):
    """int32 fetch-add into a peer CTA's SMEM; returns the OLD value."""
    return cutlass.Int32(
        llvm.inline_asm(
            T.i32(),
            [mapped_addr.ir_value(loc=loc, ip=ip), val.ir_value(loc=loc, ip=ip)],
...
class GvrTopKKernel:
    ...
    def __init__(
        ...
        # [op37-dp4] distP4: cluster-distributed Phase 4 (hist build + fine
        # build + scatter/writeback run on ALL cluster CTAs; the leader keeps
        # only the scalar bin searches + the rare exact-tail repair). Requires
        # the exact rank-scatter P4 at cluster_size > 1. Raise loudly instead
        # of silently disabling so a mis-built variant can't masquerade as
        # the dist arm.
        self.dist_p4 = bool(dist_p4) if dist_p4 is not None else False
        if self.dist_p4 and (
            cluster_size == 1 or not self.enable_p4_rank_scatter_exact
        ):
            raise ValueError(
                "dist_p4=True requires cluster_size > 1 and "
    ...
    def block_count_ge_multi(
        ...
        mt_unroll = cutlass.const_expr(
            self.mt_unroll if unroll_ov is None else int(unroll_ov)
        )
        num_threads = cutlass.const_expr(self.num_threads)
        ...
        if self.enable_unroll_4:
            ...
            for k in cutlass.range(big_iters, unroll=mt_unroll):
                i_local = i + k * cutlass.Int32(step_elem)
                src_ptr_k = cute.make_ptr(
                    self.dtype,
                    row_addr + cutlass.Int64(i_local) * cutlass.Int64(elem_bytes),
    ...
    # ------------------------------------------------------------------
    # [op37-esc] _p2_fb_falsi — the R0-miss SEEDED bounded log-falsi
    # refine, factored out of _run_phases so the tb_escape build shares
    # ONE traced falsi body with the base build (the v1 escape splice
    # duplicated these ~130 lines, and the code growth cost 7-16% on the
    # latency-bound T1024 small-BS cells via register pressure — measured
    # nsys 132-cell, all regressing cells confirmed base-accept i.e.
    # compile-level). Seeds from the M measured rung points in
    # s_mt_thr/s_mt_cnt: M = M_thr (base first-pass ladder) or M_esc
    # (escape build — the escape count pass overwrote the rung buffers,
    # so its M_esc points seed the falsi strictly tighter). Body text is
    # otherwise IDENTICAL to the pre-factor inline block (PTX identity
    # of the default build re-proven after the factor).
    # ------------------------------------------------------------------
    @cute.jit
    def _p2_fb_falsi(
        self,
        ...
        if tidx == cutlass.Int32(0):
            M = cutlass.const_expr(self.M_esc if self.tb_escape else self.M_thr)
            blo = v_lo
            bhi = v_hi
            clo = cutlass.Int32(-1)
            chi = cutlass.Int32(-1)
    ...
    # ------------------------------------------------------------------
    # [op37-lj] Phase 3 (tight bracket): dual-threshold collect. Only
    # called when the bracket admission FIRED (done == 1, so the base
    # retry-shrink is never needed). Elements >= thr_hi are SURE top-K
    # members and stream DIRECTLY to the output row (region [0, cnt_hi);
    # this CTA's base = sum of peer local hi-counts over cluster ranks
    # < mine, read from the peers' s_cluster_partial_m[hi_m] that
    # block_count_ge_multi released via its cluster_arrive/wait pair).
    # Elements in [thr_lo, thr_hi) go to the smem candidate slots exactly
    # like the base phase3; P4 then ranks the band for the remaining
    # K - cnt_hi slots at output offset cnt_hi. Per-thread lo/hi counts
    # are read IN PLACE from the block_count_ge_multi column cache
    # (smem_ptcnt_multi; the vseed column lives in smem_ptcnt), so the
    # write positions are prefix-derived with zero rescan and zero new
    # cluster syncs. smem_hist (free between P1b and P4) is the sure-set
    # warp-total scratch. Scan uses the 1-way vec loop + scalar tail
    # (no 4-way unroll fast path in v1 — correctness first).
    # ------------------------------------------------------------------
    @cute.jit
    def phase3_collect_band(
        self,
        input_row,
        N,
        slice_start,
        ...
        thr_lo = s_thr[0]
        thr_hi = s_lj_f[0]
        has_hi = cutlass.Int32(0)
        if hi_m >= cutlass.Int32(0):
            has_hi = cutlass.Int32(1)
        ...
        if cutlass.const_expr(self.tb_escape):
            ...
            if lo_m == cutlass.Int32(self.M_qf):
                ...
                pc_lo = smem_ptcnt_multi[lo_m * cutlass.Int32(num_threads) + tidx]
            if has_hi == cutlass.Int32(1):
                if hi_m == cutlass.Int32(self.M_qf):
                    pc_hi = smem_ptcnt[tidx]
                else:
                    pc_hi = smem_ptcnt_multi[hi_m * cutlass.Int32(num_threads) + tidx]
        else:
            pc_lo = smem_ptcnt_multi[lo_m * cutlass.Int32(num_threads) + tidx]
        ...
        # ---- two warp inclusive scans: band (smem slots) + sure (output) ----
        tp = band_pt
        ts = pc_hi
        for off_i in cutlass.range_constexpr(5):
            off_v = cutlass.const_expr(1 << off_i)
            ob = cute.arch.shuffle_sync_up(tp, off_v, mask_and_clamp=0)
            osur = cute.arch.shuffle_sync_up(ts, off_v, mask_and_clamp=0)
            if lane >= cutlass.Int32(off_v):
                tp = tp + ob
                ts = ts + osur
        band_excl = tp - band_pt
        sure_excl = ts - pc_hi
        band_wtot = cute.arch.shuffle_sync(tp, cutlass.Int32(self.WARP_SIZE - 1))
        sure_wtot = cute.arch.shuffle_sync(ts, cutlass.Int32(self.WARP_SIZE - 1))
        if lane == 0:
            smem_wcnt[warp_id] = band_wtot
            smem_hist[warp_id] = sure_wtot
        cute.arch.barrier()

        # tid0 serial exclusive prefix over both warp-total arrays + this
        # CTA's sure-set output base.
        if tidx == 0:
            total_b = cutlass.Int32(0)
            total_s = cutlass.Int32(0)
            for w in cutlass.range_constexpr(self.num_warps):
                cb = smem_wcnt[w]
                csur = smem_hist[w]
                smem_wcnt[w] = total_b
                smem_hist[w] = total_s
                total_b = total_b + cb
                total_s = total_s + csur
            # CTA-local band count: same contract as the base P3 prefix
            # ([0] local total; [5] feeds the P4 cluster gather).
            s_iscalars[0] = total_b
            if cutlass.const_expr(cluster_size > 1):
                s_iscalars[5] = total_b
            sure_base = cutlass.Int32(0)
            if cutlass.const_expr(cluster_size > 1):
                if do_cluster_sync and has_hi == cutlass.Int32(1):
                    local_ptr = s_cluster_partial_m.iterator
                    for peer in cutlass.range_constexpr(cluster_size):
                        if cutlass.Int32(peer) < cta_in_cluster:
                            peer_addr = mapa_shared_cluster(
                                local_ptr + hi_m, cutlass.Int32(peer)
                            )
                            sure_base = sure_base + ld_shared_cluster_i32(peer_addr)
            s_lj_i[4] = sure_base
        cute.arch.barrier()

        my_band_pos = smem_wcnt[warp_id] + band_excl
        my_sure_pos = s_lj_i[4] + smem_hist[warp_id] + sure_excl

        # ---- dual-threshold stream write ----
        vec_w = cutlass.const_expr(self.vec_bits // self.dtype.width)
        elem_bytes = cutlass.const_expr(self.dtype.width // 8)
        vec_align = cutlass.const_expr(self.vec_align_bytes)
        copy_atom = self._make_load_copy_atom()
        ...
        while ic + cutlass.Int32(vec_w - 1) < N_local:
            ...
            for j in cutlass.range_constexpr(vec_w):
                if cutlass.const_expr(self.dtype == cutlass.Float32):
                    ...
                    vj = cutlass.Float32(tail_frag[j])
                # sure/band classify must mirror block_count_ge_multi's
                # `v >= thr` compares so the prefix-derived positions are
                # exact. has_hi predicate (Int32 idiom) guards the +FLT_MAX
                # sentinel against genuine FLT_MAX/inf row values.
                sure_p = cutlass.Int32(0)
                if vj >= thr_hi:
                    sure_p = has_hi
                if sure_p == cutlass.Int32(1):
                    if sc < cutlass.Int32(kK):
                        if cutlass.const_expr(self.return_output_values):
                            output_values_row[sc] = self.dtype(vj)
                        output_indices_row[sc] = global_base_t + cutlass.Int32(j)
                        sc = sc + cutlass.Int32(1)
                elif vj >= thr_lo and wc < cutlass.Int32(kCC):
                    smem_keys[wc] = vj
                    smem_vals[wc] = global_base_t + cutlass.Int32(j)
                    wc = wc + cutlass.Int32(1)
            ic = ic + step
        ...
        while it < N_local:
            ...
            if sure_p == cutlass.Int32(1):
                if sc < cutlass.Int32(kK):
                    if cutlass.const_expr(self.return_output_values):
                        output_values_row[sc] = self.dtype(v)
                    output_indices_row[sc] = pos_global
                    sc = sc + cutlass.Int32(1)
            elif v >= thr_lo and wc < cutlass.Int32(kCC):
                smem_keys[wc] = v
                smem_vals[wc] = pos_global
                wc = wc + cutlass.Int32(1)
            it = it + cutlass.Int32(num_threads)
    ...
    # ------------------------------------------------------------------
    # [op37-lj] Phase 4 (tight bracket): band rank-and-scatter. Verbatim
    # copy of phase4_rank_scatter with (a) the compile-time kK selection
    # rank replaced by the RUNTIME `need` = kK - cnt_hi (the sure set
    # already occupies output [0, cnt_hi)), and (b) every output write
    # offset by `out_base` = cnt_hi so winners land in [cnt_hi, kK).
    # p4_exact_tail / [p4tt] operate within the band unchanged. With
    # cnt_hi == 0 (no hi rung) this is behaviorally identical to the
    # base phase4_rank_scatter. Only traced when tight_bracket=True.
    # ------------------------------------------------------------------
    @cute.jit
    def phase4_rank_scatter_band(
        self,
        smem_keys,
        smem_vals,
        smem_hist,
        ...
        output_indices_row,
        cand_count,
        need,  # [op37-lj] runtime Int32: winners to select (= kK - cnt_hi)
        out_base,  # [op37-lj] runtime Int32: output row offset (= cnt_hi)
        tidx,
        warp_id,
        lane,
    ):
        # [op37-lj] `need` replaces the compile-time kK rank everywhere below;
        # every output write is offset by `out_base`. Otherwise a verbatim
        # copy of phase4_rank_scatter (traced only when tight_bracket=True).
        kBins = cutlass.const_expr(self.kNumBins)
        num_threads = cutlass.const_expr(self.num_threads)
        num_warps = cutlass.const_expr(self.num_warps)
        bins_per_warp = cutlass.const_expr(kBins // self.num_warps)
        ...
        if cand_count == need:
            ...
            while i4 < need:
                if cutlass.const_expr(self.return_output_values):
                    output_values_row[out_base + i4] = self.dtype(smem_keys[i4])
                output_indices_row[out_base + i4] = smem_vals[i4]
                i4 = i4 + cutlass.Int32(num_threads)
        elif cand_count > need:
            # ---- block min/max over candidates ----
            local_cmin = cutlass.Float32(self.FLT_MAX)
            local_cmax = cutlass.Float32(self.NEG_FLT_MAX)
            i5 = tidx
            ...
            if tidx == cutlass.Int32(0):
                ...
                for w2 in cutlass.range_constexpr(self.num_warps):
                    cum = cum + smem_wcnt[w2]
                    if cum >= need and found == cutlass.Int32(0):
                        tw = cutlass.Int32(w2)
                        found = cutlass.Int32(1)
                cum2 = cutlass.Int32(0)
                for w3 in cutlass.range_constexpr(self.num_warps):
            ...
            if warp_id == target_warp and lane == cutlass.Int32(0):
                ...
                for jb2 in cutlass.range_constexpr(bins_per_warp):
                    ...
                    ra_before = base_cum
                    base_cum = base_cum + smem_hist[bidx2]
                    if base_cum >= need and set_d == cutlass.Int32(0):
                        b_star = bidx2
                        rank_above = ra_before  # count in bins strictly above b*
                        set_d = cutlass.Int32(1)
                s_iscalars[2] = rank_above
            ...
            if cutlass.const_expr(self.enable_p4_rank_scatter_exact):
                ...
                if tidx == cutlass.Int32(0):
                    ...
                    for w2 in cutlass.range_constexpr(self.num_warps):
                        cumf = cumf + smem_wcnt[w2]
                        if cumf >= need and fnd == cutlass.Int32(0):
                            twf = cutlass.Int32(w2)
                            fnd = cutlass.Int32(1)
                    pre = rank_above
                    for w3 in cutlass.range_constexpr(self.num_warps):
                ...
                if warp_id == twf2 and lane == cutlass.Int32(0):
                    ...
                    for jb3 in cutlass.range_constexpr(fbpw):
                        ...
                        ra_b = base_f
                        base_f = base_f + smem_hist[sbi]
                        if base_f >= need and sd == cutlass.Int32(0):
                            sb_star = sbi
                            ra_fine = ra_b
                            sd = cutlass.Int32(1)
                    smem_hist[2] = sb_star
                ...
                while isc < cand_count:
                    ...
                    if bin_i > b_star:
                        ...
                        if pos < need:
                            if cutlass.const_expr(self.return_output_values):
                                output_values_row[out_base + pos] = self.dtype(v)
                            output_indices_row[out_base + pos] = smem_vals[isc]
                    elif bin_i == b_star:
                        sb = cutlass.Int32((v - f_lo) * finv)
                        if sb < cutlass.Int32(0):
                            sb = cutlass.Int32(0)
                ...
                while ipad < need:
                    if cutlass.const_expr(self.return_output_values):
                        output_values_row[out_base + ipad] = self.dtype(self.NEG_FLT_MAX)
                    output_indices_row[out_base + ipad] = cutlass.Int32(-1)
                    ipad = ipad + cutlass.Int32(num_threads)
                        ...
                        if cnt_strad <= cutlass.Int32(128):
                            ...
                            if tidx == cutlass.Int32(0):
                                ...
                                while tj < need0:
                                    ...
                                    pos = rank_above_fine + tj
                                    if cutlass.const_expr(self.return_output_values):
                                        output_values_row[out_base + pos] = self.dtype(tbv)
                                    output_indices_row[out_base + pos] = smem_hist[tbi + tbi + cutlass.Int32(1)]
                                    smem_hist[tbi + tbi + cutlass.Int32(1)] = cutlass.Int32(-1)
                                    tj = tj + cutlass.Int32(1)
                            cute.arch.barrier()
                        else:
                                ...
                                if br == b_star:
                                    ...
                                    if sr == sb_star:
                                        ...
                                        if ks2 > ks_thr:
                                            ...
                                            if pos < need:
                                                if cutlass.const_expr(self.return_output_values):
                                                    output_values_row[out_base + pos] = self.dtype(vr)
                                                output_indices_row[out_base + pos] = smem_vals[ir2]
                                        elif ks2 == ks_thr:
                                            q2 = atomicAdd(
                                                s_iscalars.iterator + cutlass.Int32(0),
                                                cutlass.Int32(1),
            ...
            while i10 < cand_count:
                if cutlass.const_expr(self.return_output_values):
                    output_values_row[out_base + i10] = self.dtype(smem_keys[i10])
                output_indices_row[out_base + i10] = smem_vals[i10]
                i10 = i10 + cutlass.Int32(num_threads)
            i11 = cand_count + tidx
            while i11 < need:
                if cutlass.const_expr(self.return_output_values):
                    output_values_row[out_base + i11] = self.dtype(self.NEG_FLT_MAX)
                output_indices_row[out_base + i11] = cutlass.Int32(-1)
                i11 = i11 + cutlass.Int32(num_threads)


    # ------------------------------------------------------------------
    # [op37-dp4] Phase 4 (dist): cluster-distributed exact rank-and-scatter.
    # ALL cluster CTAs enter (do_cluster_sync=True guaranteed by caller).
    # Replaces the handoff2 value-gather + leader-only P4: each CTA bins and
    # scatters its OWN candidates (histogram atomics land in the leader's
    # smem_hist / s_iscalars via mapa red/atom; gmem output writes go direct
    # from every CTA); the leader keeps only the scalar coarse/fine bin
    # searches and the RARE ambiguous exact-tail repair (which falls back to
    # the old gather + tie re-rank over the leader's buffer, composing with
    # the [p4tt] tiny-tie fast path exactly as the leader-local P4 does).
    # The scatter/search math is replicated VERBATIM from phase4_rank_scatter.
    # Memory model: relaxed cluster atomics ordered by the release
    # cute.arch.cluster_arrive() + acquire cluster_wait() pairs SYNC1..SYNC6
    # (NEVER cluster_arrive_relaxed anywhere in this method).
    # ------------------------------------------------------------------
    @cute.jit
    def phase4_rank_scatter_dist(
        self,
        smem_keys,
        smem_vals,
        smem_hist,
        ...
        # Per-CTA local candidate count (stable post-P3), capped at kC.
        own_cnt = min(s_iscalars[5], cutlass.Int32(kCC))

        # ---- stage A: own cand min/max -> s_dp4[0]/[1] as fp32 BITS ----
        # (original warp-staging pattern, staged into smem_wmin/smem_wmax
        # which are dead post-P1).
        local_cmin = cutlass.Float32(self.FLT_MAX)
        local_cmax = cutlass.Float32(self.NEG_FLT_MAX)
        i5 = tidx
        while i5 < own_cnt:
        ...
        cmax = self.warp_reduce_max_f32(local_cmax)
        if lane == cutlass.Int32(0):
            smem_wmin[warp_id] = cmin
            smem_wmax[warp_id] = cmax
        cute.arch.barrier()
        if tidx == cutlass.Int32(0):
            cmin_all = cutlass.Float32(self.FLT_MAX)
            cmax_all = cutlass.Float32(self.NEG_FLT_MAX)
            for w in cutlass.range_constexpr(self.num_warps):
                cmin_all = _fmin_f32_inline(cmin_all, smem_wmin[w])
                cmax_all = cute.arch.fmax(cmax_all, smem_wmax[w])
            s_dp4[0] = float_as_int32(cmin_all)
            s_dp4[1] = float_as_int32(cmax_all)
        # Leader zeroes its FULL coarse hist (peers' red-adds land there).
        izh = cutlass.Int32(tidx)
        if is_leader:
            while izh < cutlass.Int32(kBins):
                smem_hist[izh] = cutlass.Int32(0)
                izh = izh + cutlass.Int32(num_threads)
        cute.arch.barrier()

        # SYNC1 — releases own s_dp4 staging (and, transitively, this CTA's
        # P3 smem_keys/smem_vals + s_iscalars[5] for the rare gather).
        cute.arch.cluster_arrive()
        cute.arch.cluster_wait()

        # Every CTA's thread0: mapa-read all peers' (min,max) bits and capped
        # counts -> cluster bmin/bmax into own s_thr[1]/s_thr[2] (fp32 slots,
        # dead post-P3) + my_prefix/total_capped into s_dp4[2]/[3].
        if tidx == cutlass.Int32(0):
            bmin_t = cutlass.Float32(self.FLT_MAX)
            bmax_t = cutlass.Float32(self.NEG_FLT_MAX)
            my_prefix_t = cutlass.Int32(0)
            total_capped = cutlass.Int32(0)
            dp4_ptr0 = s_dp4.iterator + cutlass.Int32(0)
            dp4_ptr1 = s_dp4.iterator + cutlass.Int32(1)
            cnt5_ptr = s_iscalars.iterator + cutlass.Int32(5)
            for peer in cutlass.range_constexpr(cluster_size):
                pa0 = mapa_shared_cluster(dp4_ptr0, cutlass.Int32(peer))
                pa1 = mapa_shared_cluster(dp4_ptr1, cutlass.Int32(peer))
                pmn = cutlass.Float32(
                    llvm.bitcast(
                        cutlass.Float32.mlir_type,
                        ld_shared_cluster_i32(pa0).ir_value(),
                    )
                )
                pmx = cutlass.Float32(
                    llvm.bitcast(
                        cutlass.Float32.mlir_type,
                        ld_shared_cluster_i32(pa1).ir_value(),
                    )
                )
                bmin_t = _fmin_f32_inline(bmin_t, pmn)
                bmax_t = cute.arch.fmax(bmax_t, pmx)
                pca = mapa_shared_cluster(cnt5_ptr, cutlass.Int32(peer))
                pcnt = ld_shared_cluster_i32(pca)
                pcnt = min(pcnt, cutlass.Int32(kCC))
                total_capped = total_capped + pcnt
                if cutlass.Int32(peer) < cta_in_cluster:
                    my_prefix_t = my_prefix_t + pcnt
            s_thr[1] = bmin_t
            s_thr[2] = bmax_t
            s_dp4[2] = my_prefix_t
            s_dp4[3] = total_capped
        cute.arch.barrier()
        bmin_r = s_thr[1]
        bmax_r = s_thr[2]
        if bmax_r <= bmin_r:
        ...
        if cand_total == cutlass.Int32(kK):
            # Distributed copy: each CTA strided-copies its own cands to
            # output[my_prefix + i]; done (all CTAs fall through to the
            # final cluster barrier OUTSIDE this method).
            i4 = tidx
            while i4 < own_cnt:
                if cutlass.const_expr(self.return_output_values):
                    output_values_row[my_prefix + i4] = self.dtype(smem_keys[i4])
                output_indices_row[my_prefix + i4] = smem_vals[i4]
                i4 = i4 + cutlass.Int32(num_threads)
        elif cand_total > cutlass.Int32(kK):
            range1 = bmax_r - bmin_r
            inv1 = (cutlass.Float32(kBins - 1) + cutlass.Float32(0.99)) / range1
            hist_base = smem_hist.iterator
            # ---- distributed coarse hist build (into the leader's hist) ----
            i7 = tidx
            while i7 < own_cnt:
                vk = smem_keys[i7]
                bin_i = cutlass.Int32((vk - bmin_r) * inv1)
                ...
                if is_leader:
                    ...
                    ha = mapa_shared_cluster(hist_base + bin_i, cutlass.Int32(0))
                    red_shared_cluster_add_i32(ha, cutlass.Int32(1))
                i7 = i7 + cutlass.Int32(num_threads)
            # SYNC2
            cute.arch.cluster_arrive()
            cute.arch.cluster_wait()

            # ---- leader-only 3-step coarse search (verbatim) + publish ----
            if is_leader:
                warp_bin_sum = cutlass.Int32(0)
                for jb in cutlass.range_constexpr(bins_per_warp):
                    bidx_s = (
                ...
                if warp_id == target_warp and lane == cutlass.Int32(0):
                    ...
                    s_iscalars[1] = cutlass.Int32(0)  # cnt_straddle
                cute.arch.barrier()
                # Publish b_star/rank_above to smem_hist[256]/[257]
                # (kNumBins >= 512 always; slots survive the fine re-zero of
                # [0:256) below), then zero the fine bins for the incoming
                # cluster-wide fine build.
                if tidx == cutlass.Int32(0):
                    smem_hist[256] = s_iscalars[3]  # b_star
                    smem_hist[257] = s_iscalars[2]  # rank_above
                izf = tidx
            ...
            # ---- broadcast coarse result; all CTAs fine-bin own cands ----
            if tidx == cutlass.Int32(0):
                a256 = mapa_shared_cluster(hist_base + cutlass.Int32(256), cutlass.Int32(0))
                a257 = mapa_shared_cluster(hist_base + cutlass.Int32(257), cutlass.Int32(0))
                s_dp4[2] = ld_shared_cluster_i32(a256)
                s_dp4[3] = ld_shared_cluster_i32(a257)
            cute.arch.barrier()
            b_star = s_dp4[2]
            rank_above = s_dp4[3]
            ...
            while ifb < own_cnt:
                ...
                if cb == b_star:
                    ...
                    if is_leader:
                        ...
                        fa = mapa_shared_cluster(hist_base + sb, cutlass.Int32(0))
                        red_shared_cluster_add_i32(fa, cutlass.Int32(1))
                ifb = ifb + cutlass.Int32(num_threads)
            # SYNC4
            cute.arch.cluster_arrive()
            cute.arch.cluster_wait()

            # ---- leader-only fine 3-step search (verbatim), publish to
            # smem_hist[2]/[3] (existing convention) + counter resets ----
            if is_leader:
                fws = cutlass.Int32(0)
                for jbf in cutlass.range_constexpr(fbpw):
                    bif = (
            ...
            # ---- broadcast fine result; all CTAs scatter own cands ----
            if tidx == cutlass.Int32(0):
                a2 = mapa_shared_cluster(hist_base + cutlass.Int32(2), cutlass.Int32(0))
                a3 = mapa_shared_cluster(hist_base + cutlass.Int32(3), cutlass.Int32(0))
                s_dp4[2] = ld_shared_cluster_i32(a2)
                s_dp4[3] = ld_shared_cluster_i32(a3)
            cute.arch.barrier()
            sb_star = s_dp4[2]
            rank_above_fine = s_dp4[3]

            # Scatter (verbatim math) EXCEPT the three rank counters live in
            # the LEADER's s_iscalars[4]/[0]/[1]: leader uses local atomicAdd,
            # peers use atom.relaxed.cluster via mapa. Output gmem writes go
            # direct from every CTA.
            isc4_ptr = s_iscalars.iterator + cutlass.Int32(4)
            isc0_ptr = s_iscalars.iterator + cutlass.Int32(0)
            isc1_ptr = s_iscalars.iterator + cutlass.Int32(1)
            isc = tidx
            while isc < own_cnt:
                v = smem_keys[isc]
                bin_i = cutlass.Int32((v - bmin_r) * inv1)
                ...
                if bin_i > b_star:
                    pos = cutlass.Int32(0)
                    if is_leader:
                        pos = atomicAdd(isc4_ptr, cutlass.Int32(1))
                    else:
                        p4a = mapa_shared_cluster(isc4_ptr, cutlass.Int32(0))
                        pos = atom_shared_cluster_add_i32(p4a, cutlass.Int32(1))
                    if pos < cutlass.Int32(kK):
                        if cutlass.const_expr(self.return_output_values):
                            output_values_row[pos] = self.dtype(v)
                        output_indices_row[pos] = smem_vals[isc]
                    ...
                    if sb > sb_star:
                        o = cutlass.Int32(0)
                        if is_leader:
                            o = atomicAdd(isc0_ptr, cutlass.Int32(1))
                        else:
                            p0a = mapa_shared_cluster(isc0_ptr, cutlass.Int32(0))
                            o = atom_shared_cluster_add_i32(p0a, cutlass.Int32(1))
                        pos = rank_above + o
                        if pos < cutlass.Int32(kK):
                            if cutlass.const_expr(self.return_output_values):
                                output_values_row[pos] = self.dtype(v)
                        ...
                        o = cutlass.Int32(0)
                        if is_leader:
                            o = atomicAdd(isc1_ptr, cutlass.Int32(1))
                        else:
                            p1a = mapa_shared_cluster(isc1_ptr, cutlass.Int32(0))
                            o = atom_shared_cluster_add_i32(p1a, cutlass.Int32(1))
                        pos = rank_above_fine + o
                        if pos < cutlass.Int32(kK):
                            if cutlass.const_expr(self.return_output_values):
                                output_values_row[pos] = self.dtype(v)
            ...
            # ---- leader-only pad + RARE ambiguous exact-tail repair ----
            # (peers exit the method here and fall to the existing final
            # cluster barrier in _run_phases, keeping SMEM alive for the
            # rare gather — the old-path liveness contract.)
            if is_leader:
                cnt_strad = s_iscalars[1]
                filled = rank_above_fine + cnt_strad
                if filled > cutlass.Int32(kK):
                ...
                if cutlass.const_expr(self.p4_exact_tail):
                    need0 = cutlass.Int32(kK) - rank_above_fine
                    if cnt_strad > need0 and need0 > cutlass.Int32(0):
                        # RARE ambiguity: the (b*, sb*) tie set overfills the
                        # remaining slots. Gather the peers' candidates into
                        # the leader's smem_keys/smem_vals (OLD handoff2
                        # gather, verbatim) and re-rank the tie set exactly
                        # over the gathered count — via the [p4tt] tiny-tie
                        # COLLECT+SELECT fast path when the class is small,
                        # else the OLD exact-tail radix block (both verbatim
                        # from phase4_rank_scatter with cand_count ->
                        # cand_count_g).
                        local_cnt_self = own_cnt
                        ...
                        cute.arch.barrier()
                        cand_count_g = min(base_offset, cutlass.Int32(kCC))

                        # ---- exact-tail re-rank over the gathered count;
                        # bmin_r/inv1/b_star/f_lo/finv/sb_star/ra_fine all
                        # live. [p4tt] structure mirrors phase4_rank_scatter:
                        # tiny classes take the COLLECT+SELECT fast path,
                        # larger classes the radix select (verbatim). ----
                        if cutlass.const_expr(self.p4_tail_fast):  # [p4tt]
                            if cnt_strad <= cutlass.Int32(128):
                                # [p4tt] SMEM: (value_bits, cand_idx) pairs at
                                # smem_hist[2*o]/[2*o+1], o < 128 (slots
                                # 0..255; the leader's fine bins are dead
                                # here, sb_star/ra_fine already broadcast).
                                # Collect counter = s_iscalars[0] (dead after
                                # the scatter).
                                if tidx == cutlass.Int32(0):
                                    s_iscalars[0] = cutlass.Int32(0)
                                cute.arch.barrier()
                                itc = tidx
                                while itc < cand_count_g:
                                    tv = smem_keys[itc]
                                    tb = cutlass.Int32((tv - bmin_r) * inv1)
                                    if tb < cutlass.Int32(0):
                                        tb = cutlass.Int32(0)
                            ...
                            # ---- OLD exact-tail radix block (verbatim, over
                            # the gathered cand count) ----
                            if tidx == cutlass.Int32(0):
                                smem_hist[256] = cutlass.Int32(0)
                                smem_hist[257] = need0
                                smem_hist[258] = cutlass.Int32(0)
            ...
            # Undershoot (cand_total < kK): distributed copy at my_prefix;
            # leader pads the [cand_total, kK) slot range.
            i10 = tidx
            while i10 < own_cnt:
                if cutlass.const_expr(self.return_output_values):
                    output_values_row[my_prefix + i10] = self.dtype(smem_keys[i10])
                output_indices_row[my_prefix + i10] = smem_vals[i10]
                i10 = i10 + cutlass.Int32(num_threads)
            i11 = cand_total + tidx
            if is_leader:
                while i11 < cutlass.Int32(kK):
    ...
    def run_one_row(
        ...
        # [op37-dp4] distP4 per-CTA DSMEM scratch (same offset on every CTA —
        # mapa requirement). Sequentially reused slots:
        #   [0]/[1] own cand min/max as fp32 BITS (float_as_int32),
        #   [2]/[3] my_prefix/total_capped, then b_star/rank_above bcast,
        #           then sb_star/ra_fine bcast (each stage separated by a
        #           block barrier after thread0 writes).
        if cutlass.const_expr(cluster_size > 1 and self.dist_p4):
            s_dp4 = smem.allocate_tensor(
                element_type=cutlass.Int32,
        ...
        if v_hi <= cutlass.Float32(self.NEG_FLT_MAX) or v_lo >= v_hi:
            ...
            if cutlass.const_expr(self.enable_r0):
                ...
                if cutlass.const_expr(self.tight_bracket):  # [op37-lj]
                    # Bracket admission: lo = rung with the SMALLEST count
                    # >= K (tightest admissible threshold), hi = rung with
                    # the LARGEST count < K (tightest sure-set). Fire when
                    # the band cnt_lo - cnt_hi fits the kC candidate buffer
                    # (NOTE: this fires MORE often than the base rule —
                    # cnt_lo may exceed kC as long as the band fits, because
                    # P3 only stores the band). No hi rung => cnt_hi = 0 and
                    # thr_hi = +FLT_MAX sentinel (degenerates to the base
                    # single-rung behavior). Every CTA's thread0 computes
                    # the same values from the cluster-wide s_mt_cnt.
                    if tidx == 0:
                        lo_m = cutlass.Int32(-1)
                        lo_c = cutlass.Int32(2147483647)
                        hi_m = cutlass.Int32(-1)
                        hi_c = cutlass.Int32(-1)
                        for m in cutlass.range_constexpr(cutlass.const_expr(self.M_thr)):
                            cm = s_mt_cnt[m]
                            if cm >= cutlass.Int32(self.top_k) and cm < lo_c:
                                lo_m = cutlass.Int32(m)
                                lo_c = cm
                            if cm < cutlass.Int32(self.top_k) and cm > hi_c:
                                hi_m = cutlass.Int32(m)
                                hi_c = cm
                        hi_c0 = cutlass.Int32(0)
                        if hi_m >= cutlass.Int32(0):
                            hi_c0 = hi_c
                        fired = cutlass.Int32(0)
                        if lo_m >= cutlass.Int32(0) and (lo_c - hi_c0) <= cutlass.Int32(
                            self.kC
                        ):
                            fired = cutlass.Int32(1)
                        s_lj_i[0] = fired
                        s_lj_i[1] = hi_m
                        s_lj_i[2] = hi_c0
                        s_lj_i[3] = lo_m
                        if fired == cutlass.Int32(1):
                            s_r0col[0] = lo_m
                            thr_hi_v = cutlass.Float32(self.FLT_MAX)
                            if hi_m >= cutlass.Int32(0):
                                thr_hi_v = s_mt_thr[hi_m]
                            s_lj_f[0] = thr_hi_v
                            s_thr[0] = s_mt_thr[lo_m]
                            # cluster-wide band count; the band collect
                            # overwrites it with the CTA-local total (same
                            # contract as the base P3 prefix sum).
                            s_iscalars[0] = lo_c - hi_c0
                    ...
                    cute.arch.barrier()
                    bc = s_r0col[0]
                    # No ptcnt column copy here: the band collect reads the
                    # lo/hi columns from smem_ptcnt_multi / smem_ptcnt in
                    # place; the miss path re-counts via the refine below
                    # (block_count_ge refreshes smem_ptcnt) as in the base.
                    if cutlass.const_expr(self.tb_debug):
                        if tidx == cutlass.Int32(0) and cta_in_cluster == cutlass.Int32(0):
                            cute.printf(
                                "[tb] fired=%d lo_m=%d hi_m=%d cnt_hi=%d band=%d\n",
                                s_lj_i[0],
                                s_lj_i[3],
                                s_lj_i[1],
                                s_lj_i[2],
            ...
            if cutlass.const_expr(self.tb_band):  # [op37-lj]/[op37-esc]
                ...
                # s_mt_cnt), so the runtime branch is convergent and the
                # barriers inside both callees are safe.
                if s_lj_i[0] == cutlass.Int32(1):
                    self.phase3_collect_band(
                        input_row,
                        N,
                        slice_start,
                        slice_end,
            ...
            # [op37-dp4] Also skipped under dist_p4: phase4_rank_scatter_dist
            # owns its own SYNC1..SYNC6 sequence (no handoff2 gather at all).
            if cutlass.const_expr(cluster_size > 1 and not self.dist_p4):
                if do_cluster_sync:
                    cute.arch.cluster_arrive()
                    cute.arch.cluster_wait()
                ...
                if cutlass.const_expr(self.tb_band):  # [op37-lj]/[op37-esc]
                    if s_lj_i[0] == cutlass.Int32(1):
                        ...
                        self.phase4_rank_scatter_band(
                            ...
                            output_indices_row,
                            cand_count_p4,
                            cutlass.Int32(self.top_k) - tb_base,
                            tb_base,
                            tidx,
                            warp_id,
                            lane,
            ...
            elif cutlass.const_expr(self.dist_p4):
                # [op37-dp4] distP4: ALL cluster CTAs enter the distributed
                # Phase 4 (it owns its own SYNC1..SYNC6). Requires cluster
                # sync; the short-row degrade (do_cluster_sync=False, only
                # CTA 0 alive and it already holds ALL candidates) falls
                # back to the EXISTING leader-local exact rank-scatter.
                if do_cluster_sync:
                    self.phase4_rank_scatter_dist(
                        smem_keys,
                        smem_vals,
                        smem_hist,
                        smem_wcnt,
