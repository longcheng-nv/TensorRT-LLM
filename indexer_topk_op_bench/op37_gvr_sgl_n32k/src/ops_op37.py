...
PROBE_ARMS = ["gvr_cs2", "gvr_cs4", "gvr_cs8", "gvr_a2", "gvr_dp4",
              "gvr_base", "op26_r0auto", "gvr_lj", "gvr_t512", "gvr_esc"]
...
def _build_esc(K, dtype, N, BS, cr, logits_row, preidx_row):
    """gvrpkg37 (current head + [op37-esc] splice) with tb_escape=True
    (escape-only bracket: base narrow first pass, dense thin-ladder escape
    count + band P3/P4 ONLY on a base-admission miss). Any cs; launch shape
    stays whatever pick_config gives (no forcing)."""
    import torch
    _VAR37 = _HERE.parents[0] / "variant"
    if str(_VAR37) not in sys.path:
        sys.path.insert(0, str(_VAR37))
    ...
    call = (lambda lg=lg, pre=pre, sl=sl, out=out:
            Gvr37.launch(lg, pre, sl, out, K, compress_ratio=cr,
                         tb_escape=True))
    call()
    extra = {"cluster_size": cfg["cluster_size"], "flags": "tb_escape"}
    return call, [lg, pre, sl, out], extra, (lambda: out)
...
def build_call_op37(op, K, dtype, N, BS, cr, logits_row, preidx_row):
    ...
    if op == "gvr_esc":
        return _build_esc(K, dtype, N, BS, cr, logits_row, preidx_row)
