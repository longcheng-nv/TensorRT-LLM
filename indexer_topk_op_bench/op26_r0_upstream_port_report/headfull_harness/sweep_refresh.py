...
from ops_refresh import build_call_rival, ops_for_rival, GVR_ANCHOR  # noqa: E402
...
N_BS_REP = N_SEQ                               # FULL ISL coverage for the BS sweep
...
def cells_real(sweep, model):
    if sweep == "seqlen":
        return [(isl, 1) for isl in REAL_ISLS[model]]
    return [(isl, BS) for isl in REAL_ISLS[model] for BS in BS_GRID]
...
def run_real(sweep, model, dt_name, out_path, rc, rw):
    dtype = DTYPES[dt_name]
    done = _load_done(out_path)
    cells = cells_real(sweep, model)
...
def main():
    ...
    if a.family == "synth":
        ...
        out = root / f"real_{a.model}_{a.sweep}_{a.dtype}.jsonl"
