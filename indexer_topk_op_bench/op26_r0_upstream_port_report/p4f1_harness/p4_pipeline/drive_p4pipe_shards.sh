#!/bin/bash
# 8-GPU sharded 865-cell sub-P4 pipeline breakdown run under nsys.
#   bash drive_p4pipe_shards.sh <tag> [ngpu]
set -e
TAG=${1:-full}; NG=${2:-8}
...
for g in $(seq 0 $((NG-1))); do
  setsid env -u GITHUB_TOKEN -u HF_TOKEN CUDA_VISIBLE_DEVICES=$g \
    ...
    --capture-range-end=stop -f true -o "$HERE/nsys_reps/p4pipe_${TAG}_g$g" \
    python3 "$HERE/measure_p4pipe_full.py" --shard $g/$NG \
