...
class GvrTopKKernel:
    ...
    def __init__(
        ...
        p4_finebin_loop: Optional[bool] = None,  # [p4f1]
    ):
        ...
        # --- [p4f1] iterative fine recursion (default OFF) ---
        # p4_finebin_loop: replace the ONE-shot 256-sub-bin fine recursion in
        # phase4_rank_scatter's exact path with an ITERATIVE fine recursion
        # (max 4 levels, ULP-floor termination) so two distinct fp32 values
        # falling in the same straddling fine bin are still resolved exactly.
        # Default OFF keeps the kernel byte-identical (const_expr-gated).
        self.p4_finebin_loop = (  # [p4f1]
            bool(p4_finebin_loop) if p4_finebin_loop is not None else False
        )
        if self.p4_finebin_loop:  # [p4f1]
            assert self.enable_p4_rank_scatter_exact, (
                "p4_finebin_loop requires enable_p4_rank_scatter_exact"
            )
            # metadata slots live at smem_hist[256..277] above the fine bins
            assert self.kNumBins >= 512, self.kNumBins
    ...
    def phase4_rank_scatter(
        ...
        if cand_count == cutlass.Int32(kK):
            ...
            # [p4f1] ---- EXACT (iterative): fine recursion loop, max 4 levels,
            # ULP-floor termination. Fixes the one-shot defect where two
            # distinct fp32 values in the same straddling fine bin are
            # emitted in arbitrary order. const_expr-gated: flag OFF compiles
            # the original one-shot branch below, byte-identical.
            if cutlass.const_expr(
                self.enable_p4_rank_scatter_exact and self.p4_finebin_loop
            ):  # [p4f1]
                ...
                CAP = 128  # [p4f1] v4 scratch-ring capacity (pairs in smem_hist[4..259])
                ...
                if warp_id == twf2 and lane == cutlass.Int32(0):
                    base_f = pre_f
                    sb_star0 = cutlass.Int32(fbins - 1)
                    ra_fine = base_f
                    sd = cutlass.Int32(0)
                    ...
                    smem_hist[HB + 2] = smem_hist[sb_star0]
                    smem_hist[2] = sb_star0
                    smem_hist[3] = ra_fine
                cute.arch.barrier()
                if tidx == cutlass.Int32(0):
                    s_iscalars[4] = cutlass.Int32(0)  # cnt_above
            ...
            elif cutlass.const_expr(self.enable_p4_rank_scatter_exact):  # [p4f1] if->elif only
