...
Cells: the 9 FIRING per-layer fixture cells (fire census; all K>=1024).
...
def main():
    ...
    f = open(out / "ab_fixtures.jsonl", "w")
    prof.start()
    try:
        for model, isl, L in FIXTURES:
            ...
            for rnd in range(3):
                for arm, tf in (("slow", False), ("fast", True)):
                    base = f"{arm}|{model}|{isl}|L{L}|{N}|r{rnd}"
                    ...
                    f.write(json.dumps(dict(model=model, isl=isl, L=L, N=N,
                                            arm=arm, rnd=rnd, exact=ex,
                                            range_cold=f"c|{base}",
                                            range_warm=f"w|{base}")) + "\n")
                    f.flush()
            print(f"[{model}] {isl} L{L} done", flush=True)
    finally:
        prof.stop()
    f.close()
