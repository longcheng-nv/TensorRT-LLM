...
Arms (same process, paired, 3 interleaved rounds, NVTX c|/w| protocol):
  old = gvrpkg_snapshot (pre-vseed @018251950f) == the REPORT's gvr_pr arm
  new = gvrpkgprod2 (PR#16457 latest head @0d6fc4f1f2)
...
from gvrpkg.top_k.gvr_topk_decode import GvrTopKKernel as OldK       # noqa: E402
from gvrpkgprod2.top_k.gvr_topk_decode import GvrTopKKernel as NewK  # noqa: E402
...
def main():
    ...
    try:
        for isl in REAL_ISLS[a.model]:
            ...
            for rnd in range(3):
                for arm, KK in (("old", OldK), ("new", NewK)):
                    ...
                    # exactness folded (new arm must be exact; old arm known
                    # inexact only on non-bench layers)
                    idx = outb[0].long()
                    v = lg[0, :N].float()
