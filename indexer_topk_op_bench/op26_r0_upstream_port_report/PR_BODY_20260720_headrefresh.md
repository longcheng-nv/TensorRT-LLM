...
- **K2048 tuning on real-content distributions.** The K2048 low rung is recalibrated (0.85 → 0.6) against real capture value distributions, and the K2048 R0 Phase-4 histogram is trimmed 2048 → 512 bins — together lifting the K2048/V3.2 axis from ~1.07× to ~1.25× (synthetic best) and 1.44× (real captures) without touching other K.
...
| --: | --: | --: | --: |
| 4,096 | 12.74 | 9.68 | 1.32× |
| 8,192 | 12.95 | 9.77 | 1.32× |
| 16,384 | 14.02 | 10.88 | 1.29× |
| 32,768 | 14.01 | 12.45 | 1.13× |
| 65,536 | 20.20 | 13.61 | 1.48× |
| 131,072 | 18.31 | 14.94 | 1.23× |
| 262,144 | 21.12 | 16.28 | 1.30× |

*K512 (V4 Flash) geomean speedup (N=4K–256K): best 1.291×*

**K1024 (V4 Pro)**

| N | base best (µs) | R0 best (µs) | best ↑ |
| --: | --: | --: | --: |
| 4,096 | 10.38 | 9.38 | 1.11× |
| 8,192 | 18.79 | 9.87 | 1.90× |
| 16,384 | 16.64 | 10.91 | 1.53× |
| 32,768 | 22.82 | 12.26 | 1.86× |
| 65,536 | 19.17 | 14.42 | 1.33× |
| 131,072 | 19.72 | 15.10 | 1.31× |
| 262,144 | 24.60 | 17.47 | 1.41× |

*K1024 (V4 Pro) geomean speedup (N=4K–256K): best 1.467×*

**K2048 (V3.2)**

| N | base best (µs) | R0 best (µs) | best ↑ |
| --: | --: | --: | --: |
| 8,192 | 14.78 | 10.69 | 1.38× |
| 16,384 | 14.66 | 11.92 | 1.23× |
| 32,768 | 17.79 | 14.09 | 1.26× |
| 65,536 | 22.25 | 16.01 | 1.39× |
| 131,072 | 21.62 | 17.31 | 1.25× |
| 262,144 | 26.33 | 19.80 | 1.33× |
| 524,288 | 22.84 | 23.77 | 0.961× |
| 1,048,576 | 39.31 | 30.63 | 1.28× |

*K2048 (V3.2) geomean speedup: best 1.254×*

**Synthetic overall (22 cells shown; best axis, V4 tables show the deployment-envelope range N=4K–256K): geomean best 1.330×; exactness 52/52 over the full measured envelope (incl. the worst-axis and V4 N≥512K cells not tabulated).**
...
- **R0 itself is BS-invariant.** The R0/base gain stays flat from BS 1 → 1024 (synthetic geomean 1.157× → 1.214× over the FULL seq-len grid, 9 rungs 4K–1M × 3 dtypes; real 1.256× → 1.262× over ALL captured ISL rungs × 3 dtypes) at production launch shapes. R0 changes Phase-2/Phase-4 arithmetic only — grid shape, smem budget, and cluster semantics are identical to base — so it introduces no batch-size coupling.
...
- **fp32 (the dtype the production indexer runs)**: worst-axis synth geomean 1.050× (8/26 worst cells < 1.0, min **0.935×**; one best cell 0.961×). Over the full fp32 dtype × BS × N audit grid there is **no cell below 0.915×** of base.
