...
- **R0 itself is BS-invariant.** The R0/base gain stays flat from BS 1 → 1024 (real-capture geomean 1.256× → 1.262× over ALL captured ISL rungs × 3 dtypes) at production launch shapes. R0 changes Phase-2/Phase-4 arithmetic only — grid shape, smem budget, and cluster semantics are identical to base — so it introduces no batch-size coupling.
