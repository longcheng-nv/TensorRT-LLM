...
C = [  # (anchor, id, name, status, rounds, internal, local, start, end, cost, owner_session, goal, constraint, code, data, note)
 ...
 dict(a="r6a", id="gspn6jvr1h44n8akdredmr8nt8", name="gvr-topk-bs40(R6 v1)", st="Cancelled(投机解止损)", rd=1,
      internal="0.8218(DQ)", local="—", t0="07-24 03:47", t1="07-24 ~05:0x", cost="$41.40", own="本会话(R6)",
      goal="BS1-1024 双 K 组 gm≥1.40 & min≥0.95 & 全 exact(1615 格),原教旨 GVR 骨架",
      cons="骨架硬锁;radix 仅可吸收进子阶段;禁 per-case dispatch 到其他算子;起点隔离 c74f/compA",
      code="—(榜首 bcast_v1 = row0 radix+结果广播, DQ)", data=f"{KFP}/R6_CLOSEOUT.md",
      note="broadcast trick 经 insights 扩散 → cancel;prompt v2 增设跨行复用精确合规线(P1-only 摊销)"),
 dict(a="r6b", id="pq3hwx7eh94k1arcf0hwmn7wem", name="gvr-topk-bs40-v2(R6 v2)", st="**Failed(fable-5 配额归零 @round6)**", rd=5,
      internal="0.8249(违例系)/合规 0.6345", local="合规 champion fb1e6848: **1615/1615 exact**;865 bs1 gm 0.8331 / 750 bs>1 gm 0.6706;**双 bar 未达**(K512+1024 0.7269, K2048 0.7979 vs 1.40)",
      t0="07-24 05:10", t1="07-25 00:51", cost="$997.03", own="本会话(R6)",
      goal="同 R6 v1 + 跨行合规线;跑满 8 轮再裁决(实际 5 轮被配额截断)",
      cons="同上;收割门四层进化: 计数→语义→launcher 级 dispatch 审查",
      code=f"{KFP}/harvest/r6v2_fb1e6848 · fork:kf/r6-compliant-gvr-bs @a674638b",
      data=f"{KFP}/grid_r6bs1.csv · r5_bs/grid_r6g_final.csv · R6_CLOSEOUT.md",
      note="对抗战争日志: broadcast→装饰性 hint→死代码伪装,全被收割门拦截;平台 fitness 无法承载算法约束的结论坐实;合规链 0.4456→0.6345(bs1 首过平价)"),
 ...
 dict(a="ra", id="rd06zd9zf55jdfxfr5z077t6wg", name="gvr-topk-bs1-recal(战役 A)", st="Completed(种子未被超越)", rd=5,
      internal="1.1502", local="champion 6d9fe9e4 nsys 双探针(8 格): **1.3951** vs 种子 28dc11f6 **1.6867** — 种子 8/8 全胜,champion 不采纳;**A 验收 bar 由种子本身满足**(R4 全网格 1.6531)", t0="07-24 06:07", t1="07-25 01:47", cost="$1083.68", own="本会话(recal 线)",
