...
- R6 v2 round-2 收口(0.6718): 榜首前 6 全为 prior-free `_radix` 家族(DQ)—
  违例谱系垄断内部选择(fitness 与合规结构性错位)。全量 grep 门扫描:
  合规集合 = {1425fb14 0.4701 persistent_long, 981f066f 0.465 small512_fast,
  b54159e7 0.4456, …}。**合规前沿 1425fb14 探针 gm 0.6797**(45/45 exact,
  bs1024 过 1.0, persistent CTA 方向 = 峡谷正解)。按用户指令跑满 8 轮,
  终局只在合规集合上裁决;每轮收割 = grep 门 + 探针。
- R6 v2 round-3 收口(0.7582): 榜首 1abcf0e5 grep 门误放行 → 语义核查
  **DQ**(pre_idx 仅在 exclude3 消费且门控到单形状 n1027/k1024 = 装饰性
  hint 路径;主路径 prior-free radix)。**门升级: pre_idx 须在通用路径
  kernel 中实际消费**。真合规前沿 = 15e80901(`gvr_per_row_secant_wide_v3`,
  内部 0.5405, 3 kernel 实消费先验): 探针 **gm 0.7378**(45/45 exact;
  链 0.6465→0.6797→0.7378, bs1024 过 1.0, bs1 仍 0.647)。round 4 进行中
  (内部 0.7943, 归属待甄别)。
- R6 v2 round-4 收口(0.8002): 榜首家族 `indexer_topk_gvr_r4_v1` 全员
  **DQ** — launcher 全部通用路径走无 pre_idx 的 topk_fused/topk_coop,
  源码内 GVR kernel 为**不被 dispatch 的死代码**(命名伪装,骗过计数门/
  平台判官)。门再升级: 必须审 launcher 实际 dispatch 路径是否消费先验。
  真合规前沿仍 = 15e80901(local 0.7378)。round 5/8 进行中。
  对抗态势记录: 违例谱系 0.80 vs 合规 0.54(内部),命名伪装出现 —
  平台 fitness 无法承载算法约束的结论进一步坐实,终局裁决只认
  launcher 级语义审查通过的合规集合。
