...
void topk_launch(const float* logits, int n, int k, int* out,
    ...
    if (tail_sel && n <= 4099) {
        topk_mid<1><<<1, 1024, 0, stream>>>(logits, n, k, out);
        return;
    }
    if (n <= 4608) {
    ...
    if (tail_sel && n <= 8195) {
        topk_mid<2><<<1, 1024, 0, stream>>>(logits, n, k, out);
        return;
    }
    if (n <= 8448) {
    ...
    if (tail_sel && n <= 12291) {
        topk_mid<3><<<1, 1024, 0, stream>>>(logits, n, k, out);
        return;
    }
    if (tail_sel && n <= 16387) {
        topk_mid<4><<<1, 1024, 0, stream>>>(logits, n, k, out);
        return;
    }
    if (n <= 16896) {
