...
// Engineer composite dispatch (r3_compA):
// K=2048 mid-n cells measured faster on the contiguous-slice 3-barrier ladder
// (30e79029); everything else on the adaptive fast-tail kernel (becdc5c7).
void topk_launch(const float* logits, int n, int k, int* out,
                 cudaStream_t stream) {
    if (k == 2048 && n > 16896 && n <= 140000) {
        v30::topk_launch(logits, n, k, out, stream);
    } else {
        becd::topk_launch(logits, n, k, out, stream);
    }
}
