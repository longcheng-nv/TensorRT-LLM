...
// Sense-reversing grid barrier, formally ordered at near-relaxed cost:
// each block RELEASES its pre-barrier writes via atom.add.release on arrive;
// the leader pairs with one ld.acquire of arrive, publishes the sense token
// with st.release; waiters spin RELAXED and issue a single trailing
// ld.acquire once the token matches (double-checked acquire).
__device__ __forceinline__ void global_barrier(unsigned int* arrive,
                                                unsigned int* release,
                                                int gridsz, unsigned int sense) {
    __syncthreads();
    if (threadIdx.x == 0) {
        unsigned int a = atom_add_release_gpu(arrive, 1u) + 1u;
        if (a == (unsigned int)gridsz) {
            (void)ld_acquire_gpu(arrive);
            atomicExch(arrive, 0u);
            st_release_gpu(release, sense);
        } else {
            while (ld_relaxed_gpu(release) != sense) { }
            (void)ld_acquire_gpu(release);
        }
    }
    __syncthreads();
}
