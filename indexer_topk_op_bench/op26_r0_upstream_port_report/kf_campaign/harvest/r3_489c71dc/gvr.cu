...
#pragma unroll
      ...
      if (lane == 0) {
        if constexpr (CS == 1)
          ...
          s->ipartial[par][wid] = sum;
      }
    }
  }
  if constexpr (CS == 1) {
    // The following CTA barrier publishes the warp-leader reductions.
    if (tid < R) s->rpre[tid] = 0;
    __syncthreads();
  } else {
    cg::cluster_group cl = cg::this_cluster();
    // cluster sync publishes the warp-leader writes to every DSMEM peer.
    cl.sync();
    if (tid < R) {
      int tot = 0, pre = 0;
#pragma unroll
...
#pragma unroll
  ...
  // P4 reuses hist.  Zero CTA0's copy while peer CTAs finish their candidate
  // pushes; the existing cluster/CTA handoff below publishes both operations.
  if (rank == 0 && tid < 256) s->hist[tid] = 0;
  if constexpr (CS > 1) {
...
#pragma unroll
  ...
  // Overlap the first P4 histogram clear with peer candidate pushes.
  if (rank == 0 && tid < 256) s->hist[tid] = 0;
  if constexpr (CS > 1) {
    cg::this_cluster().sync();
    if (rank != 0) return;
...
void gvr_topk_launch(const float* logits, const int* pre_idx, int* out, int npad, int K,
  ...
  if (npad <= DKCMAX)
    ...
    launch_reg<16, 512, 5>(logits, pre_idx, out, npad, K, kC, stream);  // vpc <= 2560
  else if (npad <= 262144)
    if (K == 1024)
      launch_reg<16, 512, 8, 6>(logits, pre_idx, out, npad, K, kC, stream);  // 6-rung: fewer barriers on latency-bound worst cell
    else
