...
// Sense-reversing grid barrier with gpu-scoped acquire/release atomics:
// arriving blocks release their pre-barrier writes via the acq_rel arrive
// increment (the leader simultaneously acquires all of them);
// the last arriver publishes the sense token with a release store; waiters
// ACQUIRE it, ordering their post-barrier reads. No membar.gl needed.
__device__ __forceinline__ void global_barrier(unsigned int* arrive,
                                                unsigned int* release,
                                                int gridsz, unsigned int sense) {
    __syncthreads();
    if (threadIdx.x == 0) {
        unsigned int a = atom_add_acqrel_gpu(arrive, 1u) + 1u;
        if (a == (unsigned int)gridsz) {
            atomicExch(arrive, 0u);
            st_release_gpu(release, sense);
        } else {
            while (ld_acquire_gpu(release) != sense) { }
        }
    }
    __syncthreads();
}
