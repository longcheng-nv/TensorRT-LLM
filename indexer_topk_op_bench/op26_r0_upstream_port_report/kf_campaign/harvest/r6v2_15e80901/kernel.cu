...
// Exact 256-bin selection with one warp and no block-wide suffix scan.  Lanes
// own descending eight-bin chunks.  The log-secant pivot only selects the
// first half to inspect; exact counts determine both the digit and rank above.
__device__ __forceinline__ void choose_byte_warp(
    const int* hist, int wanted, int pivot_guess,
    int* chosen_s, int* above_s, int* population_s) {
...
#pragma unroll
  for (int offset = 1; offset < 32; offset <<= 1) {
    int v = __shfl_up_sync(0xffffffffu, inclusive, offset);
    if (lane >= offset) inclusive += v;
  }
  unsigned owners = __ballot_sync(0xffffffffu, inclusive >= wanted);
  int owner = __ffs(owners) - 1;
  int higher = owner
      ? __shfl_sync(0xffffffffu, inclusive, owner - 1) : 0;
  if (lane == owner) {
    int pivot = max(lo, min(hi, pivot_guess));
    int above_pivot = 0;
#pragma unroll
    for (int d = pivot + 1; d <= hi; ++d) above_pivot += hist[d];
    int above = higher;
    int chosen = lo;
    if (higher + above_pivot >= wanted) {
      for (int d = hi; d > pivot; --d) {
        int c = hist[d];
        if (above + c >= wanted) { chosen = d; break; }
        above += c;
      }
    } else {
      above += above_pivot;
      for (int d = pivot; d >= lo; --d) {
        int c = hist[d];
        if (above + c >= wanted) { chosen = d; break; }
        above += c;
      }
...
constexpr int PERSIST_STRIDE = 264;
constexpr int PERSIST_PRIOR = 256;
constexpr int PERSIST_PREFIX = 257;
constexpr int PERSIST_RANK = 258;
constexpr int PERSIST_PIVOT = 259;
constexpr int PERSIST_STRICT = 260;
constexpr int PERSIST_EQUAL = 261;

// Occupancy-valley tier: one cooperative resident grid stripes (row,slice)
// tasks across the full B200, with exact grid barriers between GVR rungs.
// Scratch is per invocation and every row builds/verifies its own temporal
// prior, threshold, candidate set, and tie tickets.
__global__ __launch_bounds__(256, 2) void gvr_persistent_radix_kernel(
    const float* __restrict__ logits,
    const int* __restrict__ pre_idx,
    int n,
    int npad,
  ...
  for (int p = gtid; p < batch * PERSIST_STRIDE; p += gstride)
    workspace[p] = 0;
  for (int row = gtid; row < batch; row += gstride)
    workspace[row * PERSIST_STRIDE + PERSIST_PRIOR] = int(0xffffffffu);
  grid.sync();

  // P1: exact gathered lower-bound prior, independently for every row.
  for (int p = gtid; p < batch * k; p += gstride) {
    int row_id = p / k;
    int j = p - row_id * k;
    int idx = pre_idx[size_t(row_id) * k + j];
    uint32_t key = (unsigned(idx) < unsigned(n))
        ? ordered_key(logits[size_t(row_id) * npad + idx]) : 0u;
    atomicMin(reinterpret_cast<unsigned*>(
        workspace + row_id * PERSIST_STRIDE + PERSIST_PRIOR), key);
  }
  grid.sync();
  for (int row_id = gtid; row_id < batch; row_id += gstride) {
    int* state = workspace + row_id * PERSIST_STRIDE;
    state[PERSIST_PREFIX] = 0;
    state[PERSIST_RANK] = k;
    state[PERSIST_PIVOT] = int(uint32_t(state[PERSIST_PRIOR]) >> 24);
  }
  grid.sync();

  constexpr int ITEMS_PER_TASK = 1024;
  int chunks = (n + ITEMS_PER_TASK - 1) / ITEMS_PER_TASK;
  int tasks = batch * chunks;

#pragma unroll
  for (int pass = 0; pass < 4; ++pass) {
    for (int p = gtid; p < batch * 256; p += gstride)
      workspace[(p >> 8) * PERSIST_STRIDE + (p & 255)] = 0;
    grid.sync();

    int shift = 24 - pass * 8;
    uint32_t mask = (shift + 8 >= 32) ? 0u : (0xffffffffu << (shift + 8));
    for (int task = int(blockIdx.x); task < tasks; task += int(gridDim.x)) {
      int row_id = task / chunks;
      int chunk = task - row_id * chunks;
      int start = chunk * ITEMS_PER_TASK;
      int end = min(n, start + ITEMS_PER_TASK);
      int* state = workspace + row_id * PERSIST_STRIDE;
      uint32_t prior = uint32_t(state[PERSIST_PRIOR]);
      uint32_t prefix = uint32_t(state[PERSIST_PREFIX]);
      for (int d = tid; d < 256; d += int(blockDim.x)) local_hist[d] = 0;
      __syncthreads();
#pragma unroll
      for (int q = 0; q < 4; ++q) {
        int i = start + tid + q * int(blockDim.x);
        if (i < end) {
          uint32_t key = ordered_key(logits[size_t(row_id) * npad + i]);
          bool member = pass == 0 ? key >= prior
                                  : (key & mask) == (prefix & mask);
          if (member) atomicAdd(local_hist + ((key >> shift) & 255u), 1);
        }
      }
      __syncthreads();
      for (int d = tid; d < 256; d += int(blockDim.x)) {
        int count = local_hist[d];
        if (count) atomicAdd(state + d, count);
      }
      __syncthreads();
    }
    grid.sync();

    // One exact log-secant-guided digit selection per row.
    for (int row_id = gtid; row_id < batch; row_id += gstride) {
      int* state = workspace + row_id * PERSIST_STRIDE;
      int current_rank = state[PERSIST_RANK];
      int current_pivot = max(0, min(255, state[PERSIST_PIVOT]));
      int above_pivot = 0;
      for (int d = current_pivot + 1; d < 256; ++d)
        above_pivot += state[d];
      int higher = 0;
      int chosen = 0;
      if (above_pivot >= current_rank) {
        for (int d = 255; d > current_pivot; --d) {
          int count = state[d];
          if (higher + count >= current_rank) { chosen = d; break; }
          higher += count;
        }
      } else {
        higher = above_pivot;
        for (int d = current_pivot; d >= 0; --d) {
          int count = state[d];
          if (higher + count >= current_rank) { chosen = d; break; }
          higher += count;
        }
      }
      int next_rank = current_rank - higher;
      state[PERSIST_PREFIX] =
          int(uint32_t(state[PERSIST_PREFIX]) | (uint32_t(chosen) << shift));
      state[PERSIST_RANK] = next_rank;
      state[PERSIST_PIVOT] =
          log_secant_pivot(next_rank, max(1, state[chosen]), 256);
    }
    grid.sync();
  }
  ...
  for (int row_id = gtid; row_id < batch; row_id += gstride) {
    int* state = workspace + row_id * PERSIST_STRIDE;
    state[PERSIST_STRICT] = 0;
    state[PERSIST_EQUAL] = 0;
  }
  grid.sync();

  // P3/P4 exact row-local collect and tie-ticket writeback.
  for (int task = int(blockIdx.x); task < tasks; task += int(gridDim.x)) {
    int row_id = task / chunks;
    int chunk = task - row_id * chunks;
    int start = chunk * ITEMS_PER_TASK;
    ...
    int* state = workspace + row_id * PERSIST_STRIDE;
    int* out = indices + size_t(row_id) * k;
    uint32_t threshold = uint32_t(state[PERSIST_PREFIX]);
    int need_equal = state[PERSIST_RANK];
    int greater = k - need_equal;
#pragma unroll
    for (int q = 0; q < 4; ++q) {
      int i = start + tid + q * int(blockDim.x);
      bool valid = i < end;
      uint32_t key = valid
          ? ordered_key(logits[size_t(row_id) * npad + i]) : 0u;
      bool is_greater = valid && key > threshold;
      unsigned gm = __ballot_sync(0xffffffffu, is_greater);
      int gb = 0;
      int gc = __popc(gm);
      int gr = __popc(gm & ((1u << lane) - 1u));
      if (gc) {
        int leader = __ffs(gm) - 1;
        if (lane == leader) gb = atomicAdd(state + PERSIST_STRICT, gc);
        gb = __shfl_sync(0xffffffffu, gb, leader);
      }
      if (is_greater && gb + gr < greater) out[gb + gr] = i;

      bool is_equal = valid && key == threshold;
      unsigned em = __ballot_sync(0xffffffffu, is_equal);
      int eb = 0;
      int ec = __popc(em);
      int er = __popc(em & ((1u << lane) - 1u));
      if (ec) {
        int leader = __ffs(em) - 1;
        if (lane == leader) eb = atomicAdd(state + PERSIST_EQUAL, ec);
        eb = __shfl_sync(0xffffffffu, eb, leader);
      }
      if (is_equal && eb + er < need_equal)
        out[greater + eb + er] = i;
...
void gvr_topk_launcher(
  ...
  if (npad >= 131136 && batch == 1) {
    int* workspace = nullptr;
    size_t bytes = size_t(batch) * PERSIST_STRIDE * sizeof(int);
    cudaMallocAsync(reinterpret_cast<void**>(&workspace), bytes, stream);
    void* args[] = {
        reinterpret_cast<void*>(&logits), reinterpret_cast<void*>(&pre_idx),
        &n_valid, &npad, &k, &batch, &indices, &workspace};
    cudaLaunchCooperativeKernel(
        reinterpret_cast<void*>(gvr_persistent_radix_kernel),
        dim3(148), dim3(256), args, 0, stream);
  ...
  if (npad <= 32832) {
    ...
    if (npad >= 8192 && batch < 256) {
      constexpr int wide_threads = 1024;
      static bool wide_configured = false;
      if (!wide_configured) {
        cudaFuncSetAttribute(
            gvr_topk_kernel<true, wide_threads>,
            cudaFuncAttributeMaxDynamicSharedMemorySize,
            164 * 1024);
        wide_configured = true;
      }
      gvr_topk_kernel<true, wide_threads><<<batch, wide_threads, smem, stream>>>(
          logits, pre_idx, n_valid, npad, k, indices);
    } else {
      static bool staged_configured = false;
      if (!staged_configured) {
        cudaFuncSetAttribute(
            gvr_topk_kernel<true, threads>,
            cudaFuncAttributeMaxDynamicSharedMemorySize,
            164 * 1024);
        staged_configured = true;
      }
    ...
    if (npad <= 131136) {
      constexpr int wide_threads = 1024;
      static bool wide_candidate_configured = false;
      if (!wide_candidate_configured) {
        cudaFuncSetAttribute(
            gvr_candidate_kernel<wide_threads>,
            cudaFuncAttributeMaxDynamicSharedMemorySize,
            64 * 1024);
        wide_candidate_configured = true;
      }
      gvr_candidate_kernel<wide_threads><<<batch, wide_threads, smem, stream>>>(
          logits, pre_idx, n_valid, npad, k, capacity, indices);
    } else {
      static bool candidate_configured = false;
