...
#pragma unroll
  ...
  for (int i = tid; i < n; i += TB)
    warp_append(&emit, i != e0 && i != e1 && i != e2, i, dst, K);
}
...
extern "C" void topk_launch(const float* logits, const int* pre_idx, int* out, int b, int npad,
  ...
  if (n - K == 3) {
    if (b <= 4)
      exclude_bottom3_kernel<512><<<b, 512, 0, stream>>>(logits, out, n, npad, K);
    else
...
#define STRM(CSV, RV, KCV) \
  ...
  if (b >= 16 && b < 64 && K >= 1024 && K < 2048 && npad < 32768) {
    launch_gvr<1, 512, 9, 8, 8192, 1>(logits, pre_idx, out, b, npad, n, K, kC, stream);
    return;
  }
  if (b >= 32 && b < 64 && K >= 2048 && npad > 32768 && npad < 100000) {
    launch_gvr<2, 1024, 0, 4, 4096, 1>(logits, pre_idx, out, b, npad, n, K, kC, stream);
    return;
  }
  if (b >= 32 && b < 64 && K >= 1024 && K < 2048 && npad >= 131072) {
    launch_gvr<4, 1024, 0, 4, 6144, 1>(logits, pre_idx, out, b, npad, n, K, kC, stream);
    return;
  }
  if (b >= 32 && b < 64 && K >= 2048 && npad >= 100000) {
    launch_gvr<4, 1024, 0, 4, 8192, 1>(logits, pre_idx, out, b, npad, n, K, kC, stream);
    return;
  }
  if (b >= 64 && b < 256 && K >= 2048 && npad > 32768 && npad < 100000) {
    launch_gvr<1, 1024, 0, 4, 4096, 1>(logits, pre_idx, out, b, npad, n, K, kC, stream);
    return;
  }
