...
constexpr int RUNGS = 8;        // thresholds measured in the first count pass
constexpr int KCMAX = 6144;     // candidate buffer capacity
...
struct Smem {
  ...
  int ptcnt[RUNGS * T];            // per-thread >= counts per rung (P3 offsets)
...
template <int R>
__device__ __forceinline__ void count_pass(Smem* s, const float4* base, int v0, int v1, int tid) {
...
template <int CS>
__device__ __forceinline__ void exchange_counts(Smem* s, int par, int tid, int rank) {
  __syncthreads();  // rcnt_local final
  if constexpr (CS == 1) {
...
#pragma unroll
      for (int rr = 0; rr < CS; ++rr) {
        Smem* ps = (Smem*)cl.map_shared_rank(s, rr);
...
template <int CS>
__device__ __forceinline__ float max_below_pass(Smem* s, const float4* base, int v0, int v1,
...
__device__ __forceinline__ void phase1(Smem* s, const float* __restrict__ logits,
                                       const int* __restrict__ pre_idx, int K, int tid) {
...
#pragma unroll
  for (int jj = 0; jj < 4; ++jj) {
    ...
    hok[jj] = (j < K);
    if (hok[jj]) {
      float v = __ldg(logits + __ldg(pre_idx + j));
      hv[jj] = v;
      mn = fminf(mn, v);
      mx = fmaxf(mx, v);
    }
...
#pragma unroll
    for (int r = 0; r < RUNGS - 1; ++r) {
      if (A < qt[r] && x >= qt[r]) {
        ...
        s->rungs[r] = tlow + binw2 * (float)b;  // lower bin edge: CCDF >= target
      }
      if (tid == 31 && tot < qt[r]) s->rungs[r] = tlow;  // target below trim range
    }
  }
  __syncthreads();
}
...
template <int CS>
__global__ void __launch_bounds__(T, 1) gvr_topk_kernel(
  ...
  // async L2 prefetch of the slice (covers the cold-L2 first pass while P1 runs)
  ...
  if (npad <= kC) {
    ...
    __syncthreads();
    count_pass<1>(s, base, v0, v1, tid);
    exchange_counts<CS>(s, xch & 1, tid, rank);
    xch++;
    thr = -FLT_MAX;
    chosen = 0;
    C = s->rcnt[0];
    ...
    phase1(s, logits, pre_idx, K, tid);
    count_pass<RUNGS>(s, base, v0, v1, tid);
    exchange_counts<CS>(s, xch & 1, tid, rank);
    xch++;
...
#pragma unroll
    ...
    if (chosen < 0) {
      ...
      while (true) {
        float vstar = max_below_pass<CS>(s, base, v0, v1, t_hi, xch & 1, tid);
        xch++;
        __syncthreads();
        if (tid < RUNGS) s->rcnt_local[tid] = 0;
  ...
  if (m_gt >= 0) {
    ...
    if constexpr (CS > 1) {
      ...
      cl.sync();
      s0 = (Smem*)cl.map_shared_rank(s, 0);
    } else {
...
static void launch_gvr(const float* logits, const int* pre_idx, int* out, int npad, int K, int kC,
  ...
  if (!inited) {
    cudaFuncSetAttribute(gvr_topk_kernel<CS>, cudaFuncAttributeMaxDynamicSharedMemorySize,
                         (int)sizeof(Smem));
    if (CS > 8)
      cudaFuncSetAttribute(gvr_topk_kernel<CS>, cudaFuncAttributeNonPortableClusterSizeAllowed, 1);
    inited = true;
  }
  if constexpr (CS == 1) {
    gvr_topk_kernel<1><<<1, T, sizeof(Smem), stream>>>(logits, pre_idx, out, npad, K, kC);
  } else {
    ...
    cfg.attrs = attrs;
    cfg.numAttrs = 1;
    cudaLaunchKernelEx(&cfg, gvr_topk_kernel<CS>, logits, pre_idx, out, npad, K, kC);
  }
}
...
void gvr_topk_launch(const float* logits, const int* pre_idx, int* out, int npad, int K,
                     cudaStream_t stream) {
  int kC = (K >= 2048) ? 6144 : 5120;
  if (npad < 16384)
    launch_gvr<1>(logits, pre_idx, out, npad, K, kC, stream);
  else if (npad < 32768)
    launch_gvr<4>(logits, pre_idx, out, npad, K, kC, stream);
  else if (npad < 131072)
    launch_gvr<8>(logits, pre_idx, out, npad, K, kC, stream);
  else
    launch_gvr<16>(logits, pre_idx, out, npad, K, kC, stream);
}
