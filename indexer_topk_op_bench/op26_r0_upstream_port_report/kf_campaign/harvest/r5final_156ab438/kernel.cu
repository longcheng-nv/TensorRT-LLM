...
template <int CS, int TB, int MAXV, int R, int KC, int MB, bool FAST_P1 = true, bool RFAST = false>
...
#pragma unroll
  ...
  if constexpr (MAXV == 0 && R == 2 && FAST_P1) {
    // Throughput path: preserve the same 256-entry pre_idx sample, but let one
    // warp gather/reduce it entirely in registers.  The former CTA-wide 64-bin
    // histogram needed several barriers and shared atomics even though the
    // two-rung ladder only consumes hmin and a coarse scale estimate.  P2 still
    // verifies both thresholds against the full row, so this changes placement,
    // never exactness.
...
#pragma unroll
      ...
      if (lane == 0) {
        ...
        float hmin = u2f(kmn);
        float hmax = u2f(kmx);
        // Exponential-tail order statistics give range ~= lambda*ln(HS).
        // Only the second rung uses lambda; hmin itself remains unchanged.
        float lam = (hmax - hmin) * 0.18033688f;  // 1/ln(256)
        ...
        if (K >= 1024) {
          // Real decode hints are undershoot-biased: bracket above and at the
          // sampled minimum.  If the upper rung is too sparse, hmin is still
          // tested in the same vectorized count pass; unlike the old lower rung,
          // this can admit an overfull hmin without forcing a secant re-scan.
          s->rungs[0] = fmaxf(hmin + 0.69314718f * lam, TMIN);
          s->rungs[1] = fmaxf(hmin, TMIN);
        } else {
          // K=512's wider survivor window already admits the sampled minimum;
          // keep its proven downward backup rung while retaining warp-only P1.
          s->rungs[0] = fmaxf(hmin, TMIN);
          s->rungs[1] = fmaxf(hmin - 2.20f * lam, TMIN);
        }
        s->rcnt_local[0] = 0;
        s->rcnt_local[1] = 0;
        s->cnt_m = 0;
...
#pragma unroll
  ...
  if constexpr (MAXV > 0 && RFAST) {
    // Register-path warp-only prior: the exact-quartile kth_key_smem costs 16
    // serial barriers, but register count-passes are free, so a coarse order-stat
    // lambda (from the warp-reduced hint min/max, ~lam*ln(HS) range) plus the
    // undershoot ladder lands within a cheap secant pass or two.  No histogram,
    // no radix passes -> removes the whole P1 barrier chain.  P2 still verifies
    // every threshold against the full row, so exactness is unchanged.
    // (k25/hist left unused; ladder below reads RFAST branch.)
  } else if constexpr (MAXV > 0) {
    if (!cheap_prior)
  ...
  if (tid == 0) {
    ...
    float hmax_r = u2f(hkmax);
    float t25;
    if constexpr (MAXV > 0 && RFAST) {
      // Order-statistic lambda: for an exponential tail the sampled range
      // ~= lam*ln(HS); recover t25 (the 0.25*HS quantile) as hmin + lam*ln(4).
      float lam0 = (hmax_r - hmin) * (1.f / logf((float)HS));
      t25 = hmin + lam0 * 1.3862944f;  // ln(4)
    } else if constexpr (MAXV > 0) {
      if (cheap_prior)
    ...
    if (R >= 8) {
      if constexpr (MAXV > 0 && RFAST) {
        // hmin/lam-only ladder (no tq): bracket the undershoot-biased window.
        rg[0] = t25;
      ...
      if constexpr (MAXV > 0 && RFAST) {
        // Four-rung latency ladder: retain the two upper brackets most useful
        // for undershoot-biased decode hints, plus hmin and one lower backup.
        rg[0] = t25;
        rg[1] = hmin + 0.6931f * lam;
        rg[2] = hmin;
        rg[3] = hmin - 2.20f * lam;
      ...
      // R==1 single-rung at the modeled window center t0 (count~sqrt(K*kC), inside
      // [K,kC]).  Halves count-pass compares/element on the huge-row hot loop;
      // secant recovers if it misses.  Measured best rung for idx19 (t0=40us vs
      // hmin=44us).  rg[1..] not written (rg has R=1 slot).
      rg[0] = s->t0;
    }
    float prev = FLT_MAX;
#pragma unroll
...
static void launch_gvr(const float* logits, const int* pre_idx, int* out, int b, int npad, int n,
  ...
  auto kfn = gvr_kernel<CS, TB, MAXV, R, KC, MB, FAST_P1, RFAST>;
  static bool inited = false;
  if (!inited) {
...
#define STRM(CSV, RV, KCV) \
  ...
  // The 96..147-row band underfills B200 with one CTA per row.  Two CTAs per
  // row keep the short row register-resident with exactly five float4 slots
  // per thread (2 * 512 * 5 * 4 = 20480 elements of coverage).
  if (b >= 96 && b < 148 && npad <= 18432) {
    if (K < 1024)
      launch_gvr<2, 512, 5, 4, 6144, 1>(logits, pre_idx, out, b, npad, n, K, kC, stream);
    else
      launch_gvr<2, 512, 5, 4, 4096, 1>(logits, pre_idx, out, b, npad, n, K, kC, stream);
    return;
  }
  ...
  if (b >= 128 && K >= 1024 && K < 2048 && npad < 32768) {
    launch_gvr<1, 512, 0, 2, 4096, 2>(logits, pre_idx, out, b, npad, n, K, kC, stream);
    return;
  }
  if (b >= 128 && K < 1024 && npad < 32768) {
    launch_gvr<1, 512, 0, 2, 4096, 2, false>(logits, pre_idx, out, b, npad, n, K, 4096, stream);
    return;
  }
  ...
  if (b >= 16 && b < 64 && K >= 1024 && K < 2048 && npad < 32768 && npad <= 18432) {
    // RFAST: warp-only order-stat prior (skip 16-barrier exact quartile) for this
    // register tier; count passes are free so a coarser ladder only costs cheap
    // secant.  Serves idx16 (pro64 bs32 K1024).
    launch_gvr<1, 512, 9, 8, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC, stream);
    return;
  }
  if (b >= 32 && b < 64 && K >= 2048 && npad > 32768 && npad < 100000) {
    // idx26 (v32_64k bs32) is under-occupied at cs2 (64 CTAs=43%); cs4 fills the idle
    // SMs (128 CTAs=86%).  Measured cs2=26, cs4=24, cs8=47us -> cs4 optimal.  Orthogonal
    // to FAST_P1 (this tier keeps the ladder path).  Verified 30/30 PASS on platform.
    launch_gvr<4, 1024, 0, 2, 4096, 1>(logits, pre_idx, out, b, npad, n, K, kC, stream);
    return;
  }
  if (b >= 32 && b < 64 && K >= 1024 && K < 2048 && npad >= 131072) {
    // idx19 (pro_1024k bs32): R=1 single-rung streaming halves count-pass
    // compares/element on the huge-row hot loop (peer-validated -12% on this tier).
    launch_gvr<4, 1024, 0, 1, 6144, 1>(logits, pre_idx, out, b, npad, n, K, kC, stream);
    return;
  }
  ...
  if (b >= 32 && b < 64 && K < 1024 && npad >= 100000) {
    launch_gvr<8, 512, 0, 2, 6144, 2, false>(logits, pre_idx, out, b, npad, n, K, kC, stream);
    return;
  }
  // Small-batch K=512 rows are fully register resident.  Use the warp-only
  // order-statistic prior here as well: P2 still verifies every rung exactly,
  // while avoiding the serialized sampled-quartile radix chain in P1.
  if (b >= 4 && b < 16 && K < 1024 && npad < 32768 && mv <= 4) {
    if (cs == 4 && mv <= 3)
      launch_gvr<4, 512, 3, 8, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC,
                                                    stream);
    else if (cs == 4)
      launch_gvr<4, 512, 4, 8, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC,
                                                    stream);
    else
      launch_gvr<1, 512, 8, 8, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC,
                                                    stream);
    return;
  }
  ...
  // Avoid the npad=32768 -> 32832 register/streaming cliff at BS=64.  CS4
  // needs five float4 slots per thread through npad 40960; the old generic
  // switch only instantiated four and therefore fell back to full re-reads.
  if (b >= 64 && b < 96 && npad > 32768 && npad <= 40960) {
    if (K >= 2048)
      launch_gvr<4, 512, 5, 4, 4096, 1>(logits, pre_idx, out, b, npad, n, K, kC, stream);
    else
      launch_gvr<4, 512, 5, 4, 6144, 1>(logits, pre_idx, out, b, npad, n, K, kC, stream);
    return;
  }
  ...
  // RFAST warp-only prior for the remaining register K>=1024 mid-batch tiers
  // (b in [1,32)).  Removes the 16-barrier
  // exact-quartile P1 chain like idx16; count passes are free so a coarser ladder
  // only costs cheap secant.  Mirrors the generic REG cs/MAXV selection below.
  if (b >= 1 && b < 32 && K >= 1024 && mv <= 8) {
    if (b == 1 && cs == 16) {
      if (mv <= 5) launch_gvr<16, 512, 5, 4, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC, stream);
      else if (mv <= 6) launch_gvr<16, 512, 6, 4, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC, stream);
      else launch_gvr<16, 512, 8, 4, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC, stream);
      return;
    }
    switch (cs) {
      case 1:
        if (mv <= 7) launch_gvr<1, 512, 7, 8, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC, stream);
        else launch_gvr<1, 512, 8, 8, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC, stream);
        return;
      case 4:
        if (mv <= 3) { launch_gvr<4, 512, 3, 8, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC, stream); return; }
        if (mv <= 4) { launch_gvr<4, 512, 4, 8, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC, stream); return; }
        break;
      case 8:
        if (mv <= 3) launch_gvr<8, 512, 3, 8, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC, stream);
        else if (mv <= 4) launch_gvr<8, 512, 4, 8, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC, stream);
        else if (mv <= 6) launch_gvr<8, 512, 6, 8, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC, stream);
        else launch_gvr<8, 512, 8, 8, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC, stream);
        return;
      case 16:
        if (mv <= 5) launch_gvr<16, 512, 5, 8, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC, stream);
        else if (mv <= 6) launch_gvr<16, 512, 6, 8, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC, stream);
        else launch_gvr<16, 512, 8, 8, 8192, 1, true, true>(logits, pre_idx, out, b, npad, n, K, kC, stream);
        return;
      default: break;
    }
  }
