...
template <int CS, int TB, int MAXV>
...
#pragma unroll
  ...
  phase1<TB>(s, logits, pre_idx, K, npad, tid);
  count_reg<TB, RUNGS, MAXV>(s, a, tid);
  exchange_counts<TB, CS>(s, xch & 1, tid, rank);
  xch++;
...
#pragma unroll
          for (int r = 0; r < RUNGS; ++r) nr[r] = t_hi - dt * (float)(r + 1);
          if (!(nr[RUNGS - 1] > t_lo && nr[0] < t_hi)) descend = true;
        }
      }
      if (descend) break;
...
static void launch_reg(const float* logits, const int* pre_idx, int* out, int npad, int K, int kC,
  ...
  static bool inited = false;
  if (!inited) {
    cudaFuncSetAttribute(gvr_topk_reg<CS, TB, MAXV>, cudaFuncAttributeMaxDynamicSharedMemorySize,
                         (int)sizeof(Smem<TB>));
    if (CS > 8)
      cudaFuncSetAttribute(gvr_topk_reg<CS, TB, MAXV>,
                           cudaFuncAttributeNonPortableClusterSizeAllowed, 1);
    inited = true;
  }
  if constexpr (CS == 1) {
    gvr_topk_reg<1, TB, MAXV><<<1, TB, sizeof(Smem<TB>), stream>>>(logits, pre_idx, out, npad, K, kC);
  } else {
    cudaLaunchConfig_t cfg = {};
    cfg.gridDim = dim3(CS, 1, 1);
    cfg.blockDim = dim3(TB, 1, 1);
    ...
    cfg.attrs = attrs;
    cfg.numAttrs = 1;
    cudaLaunchKernelEx(&cfg, gvr_topk_reg<CS, TB, MAXV>, logits, pre_idx, out, npad, K, kC);
  }
}
