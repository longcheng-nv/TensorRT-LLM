...
template <bool USE_WARM>
__global__ __launch_bounds__(512, 4)
void topk_coop(const float* __restrict__ logits,
               const int* __restrict__ pre_idx, int n, int k,
    ...
    __shared__ unsigned int warm_scalar;
    __shared__ int boundary;
    __shared__ int above;

    // Measured admission rule: warm filtering only amortizes its random
    // gathers for rows at least 512K long.
    constexpr bool use_warm = USE_WARM;
    unsigned int warm_key = 0u;
    if (use_warm) {
        unsigned int warm = 0xffffffffu;
        for (int i = tid; i < k; i += blockDim.x)
            warm = min(warm, fkey(logits[pre_idx[i]]));
#pragma unroll
        for (int off = 16; off; off >>= 1)
            warm = min(warm, __shfl_down_sync(0xffffffffu, warm, off));
        if ((tid & 31) == 0) warp_totals[tid >> 5] = warm;
        __syncthreads();
        if (tid == 0) {
            unsigned int result = 0xffffffffu;
            for (int w = 0; w < 16; ++w) result = min(result, warp_totals[w]);
            warm_scalar = result;
        }
        __syncthreads();
        warm_key = warm_scalar;
    }
...
#pragma unroll
    for (int pass = 0; pass < 3; ++pass) {
        ...
        for (int i4 = gtid; i4 < (n4 >> 2); i4 += gstride) {
            ...
            if ((!use_warm || pass || u0 >= warm_key) && (u0 & high_mask) == (prefix & high_mask)) atomicAdd(&hist[(u0 >> shift) & digit_mask], 1u);
            if ((!use_warm || pass || u1 >= warm_key) && (u1 & high_mask) == (prefix & high_mask)) atomicAdd(&hist[(u1 >> shift) & digit_mask], 1u);
            if ((!use_warm || pass || u2 >= warm_key) && (u2 & high_mask) == (prefix & high_mask)) atomicAdd(&hist[(u2 >> shift) & digit_mask], 1u);
            if ((!use_warm || pass || u3 >= warm_key) && (u3 & high_mask) == (prefix & high_mask)) atomicAdd(&hist[(u3 >> shift) & digit_mask], 1u);
        }
        for (int i = n4 + gtid; i < n; i += gstride) {
            unsigned int key = fkey(logits[i]);
            if ((!use_warm || pass || key >= warm_key) && (key & high_mask) == (prefix & high_mask))
    ...
    for (int it = 0; it < iters; ++it) {
        ...
        unsigned int raw_key = i < n ? fkey(logits[i]) : 0u;
        unsigned int key = final_shift ? (raw_key >> final_shift) : raw_key;
        bool eligible = i < n && (!use_warm || raw_key >= warm_key);
        bool gt = eligible && key > threshold;
        bool eq = eligible && key == threshold;
        unsigned int gm = __ballot_sync(0xffffffffu, gt);
        unsigned int em = __ballot_sync(0xffffffffu, eq);
        if (gt) {
            int rank = __popc(gm & ((1u << lane) - 1));
...
void topk_launch(const float* logits, const int* pre_idx, int n, int k, int* out,
    ...
    if (!g_scratch) {
        cudaMalloc(&g_scratch, (3 * 4096 + 3) * sizeof(unsigned int));
        cudaMemset(g_scratch, 0, (3 * 4096 + 3) * sizeof(unsigned int));
    }
    if (!g_max_coop_blocks) {
        int active = 0;
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            &active, topk_coop<false>, 512, 0);
    ...
    void* args[] = {(void*)&logits, (void*)&pre_idx, (void*)&n, (void*)&k,
                    (void*)&g_scratch, (void*)&out};
    void* kernel = n >= 524288 ? (void*)topk_coop<true> : (void*)topk_coop<false>;
    cudaLaunchCooperativeKernel(kernel, dim3(blocks), dim3(512),
                                args, 0, stream);
}
