...
__global__ void __launch_bounds__(BLK) topk_coop(
    ...
    // ---- Phase B: per-row collect against shared T (uint4-vectorized) ----
    ...
    int* __restrict__ ce = &cnt[2*row+1];
    // vector chunk boundaries aligned to 4 elements
    ...
    for(long iv = startv + tid; iv < limv; iv += BLK){
        ...
        for(int j=0;j<4;++j){
            ...
            unsigned int gm=__ballot_sync(0xffffffffu,isgt);
            int rank=__popc(gm&((1u<<lane)-1)); int gc=__popc(gm); int gb=0;
            if(gc){ int ld=__ffs(gm)-1; if(lane==ld) gb=atomicAdd(cg_,gc); gb=__shfl_sync(0xffffffffu,gb,ld); }
            if(isgt) outp[gb+rank]=idx;
            bool iseq = vv && (ks[j]==T);
            unsigned int em=__ballot_sync(0xffffffffu,iseq);
            int er=__popc(em&((1u<<lane)-1)); int ec=__popc(em); int eb=0;
    ...
    // scalar tail for the (n_valid - 4*nvec) leftover elements: only CTA c==0
    // handles it (small, <4 elems) to avoid duplicate collection.
    if(c==0){
        int baseTail = nvec<<2;
        for(int i=baseTail+tid;i<n_valid;i+=BLK){
            uint32_t key=f2key(rowp[i]);
            if(key>T){ int s=atomicAdd(cg_,1); outp[s]=i; }
            else if(key==T){ int s=atomicAdd(ce,1); if(s<need_eq) outp[gt+s]=i; }
        }
    }
}
