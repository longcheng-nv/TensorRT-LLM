...
constexpr int KCMAX = 8448;     // candidate buffer capacity; covers the 8K padded rung
...
__global__ void __launch_bounds__(TB, 1) gvr_topk_kernel(
  ...
  if (npad <= kC) {
    // Whole row fits: C and every destination position are already known.
    // Skip threshold counting and let the direct P3 path pack each item once.
    C = npad;
  } else {
...
#pragma unroll
  ...
  if (npad <= kC) {
    for (int i = tid; i < (npad >> 2); i += T) {
      float4 a = __ldg(base + i);
      int gi = i << 2;
      s->cand[gi]     = ((unsigned long long)f2u(a.x) << 32) | (unsigned)gi;
      s->cand[gi + 1] = ((unsigned long long)f2u(a.y) << 32) | (unsigned)(gi + 1);
      s->cand[gi + 2] = ((unsigned long long)f2u(a.z) << 32) | (unsigned)(gi + 2);
      s->cand[gi + 3] = ((unsigned long long)f2u(a.w) << 32) | (unsigned)(gi + 3);
    }
  } else {
  ...
  for (int r = 0; r < 4; ++r) {
    ...
    for (int i = tid; i < C; i += T) {
      ...
      if (r == 0) {
        unsigned u;
        if (npad <= kC) {
          u = (unsigned)(kv >> 32);  // direct whole-row path already ordered
        } else {
          u = f2u(__uint_as_float((unsigned)(kv >> 32)));
...
void gvr_topk_launch(const float* logits, const int* pre_idx, int* out, int npad, int K,
                     cudaStream_t stream) {
  int kC = (npad <= KCMAX) ? KCMAX : ((K >= 2048) ? 8192 : 6144);
