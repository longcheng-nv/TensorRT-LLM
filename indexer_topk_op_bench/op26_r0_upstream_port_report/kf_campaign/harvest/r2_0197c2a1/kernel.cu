...
template<int KPT>
__global__ void __launch_bounds__(768, 1) topk_small(
...
#pragma unroll
        ...
        __syncthreads();
        consumed += 8;
        const int selected = (prefix >> shift) & 255u;
        if ((int)hist[pass][selected] == remaining) break;
    }
...
#pragma unroll
    for (int j = 0; j < KPT; ++j) {
        ...
        const bool gt = idx < n && hi > threshold;
        const bool eq = idx < n && hi == threshold;
...
#pragma unroll
    ...
    // Each warp independently extracts its three minima.  There is no
    // block-wide synchronization between picks: the selected lane-unique
    // index is broadcast inside the warp and removed from its owner's triple.
#pragma unroll
...
#pragma unroll
        ...
        const int chosen = __shfl_sync(0xffffffffu, best_idx, 0);
#pragma unroll
        for (int j = 0; j < 3; ++j) {
            if (ids[j] == chosen) {
                keys[j] = 0xffffffffu;
    ...
    // Warp 0 merges the 16 warp triples using the same three-min reduction.
    if (warp == 0) {
#pragma unroll
        for (int j = 0; j < 3; ++j) {
            keys[j] = lane < 16 ? warp_keys[lane][j] : 0xffffffffu;
            ids[j] = lane < 16 ? warp_ids[lane][j] : 0x7fffffff;
        }
        for (int pick = 0; pick < 3; ++pick) {
            unsigned int best_key = keys[0];
            int best_idx = ids[0];
...
__global__ void __launch_bounds__(512, 4) topk_coop(
    ...
    int* gt_cursor = (int*)(scratch + 3 * 4096);
    int* tie_cursor = (int*)(scratch + 3 * 4096 + 1);
    __shared__ unsigned int local_hist[4096];
    ...
    int total_above = 0;
    int consumed = 0;
    // Medium rows with larger k are the cases most likely to collide after
    // 22 high bits.  Keep the cheap 11-bit first digit, then use 12 bits so
    // the boundary is often unique after 23; 9 bits remain as exact fallback.
...
#pragma unroll
    for (int pass = 0; pass < 3; ++pass) {
        ...
        const int width = wide ? (pass == 0 ? 11 : (pass == 1 ? 12 : 9))
                               : (pass < 2 ? 11 : 10);
        ...
        __syncthreads();
        unsigned int* global_pass = ghist + pass * histogram_stride;
    ...
    for (int i = gtid; i < 3 * histogram_stride; i += stride) ghist[i] = 0;
}
...
void topk_launch(const float* logits, int n, int k, int* out,
    ...
    if (n <= 1024) topk_small<2><<<1, 768, 0, stream>>>(logits, n, k, out);
        ...
        if (!scratch_ptr) {
            cudaMalloc(&scratch_ptr, (3 * 4096 + 2) * sizeof(unsigned int));
            cudaMemset(scratch_ptr, 0, (3 * 4096 + 2) * sizeof(unsigned int));
        }
        if (!max_coop_blocks) {
            int per_sm = 0;
