...
- **Required:** beat the SEED solution (see the SEED section below — it
  already sits at ~1.65× external geomean). No workload may fall below
  0.95× of baseline (max 5% per-cell regression is the hard acceptance
  bar; a kernel that wins big on average but loses >5% on any cell will
...
## SEED SOLUTION — START FROM THIS, DO NOT START FROM SCRATCH

A previous campaign already produced a strong CUDA C++ solution from this
exact brief. Its complete source ships with the workload data in the
evaluation staging directory (same place as the `cell_*.safetensors`):
**`seed_r4_champion.cu`** (the whole kernel, compiles as-is with
`-std=c++17 -O3 --use_fast_math`, sm_100a) and **`seed_r4_main.cpp`**
(torch-extension binding). Read them FIRST. Submit the seed essentially
unchanged as your first candidate to establish your floor, then improve
it. Any submission that does not beat the seed's internal score is wasted
work.

Seed's EXTERNAL verdict on the full 865-cell nsys cold-L2 grid vs the same
baseline: **geomean 1.6531x, 865/865 exact, zero real regressions**
(p5 1.221 / median 1.593 / max 3.516). Per-(model, n-rung) speedups — your
map of where the remaining money is:
...
Seed algorithm sketch (details in the source): P1 hint-CCDF two-level
histogram 8-quantile threshold ladder -> P2 single-pass 8-threshold count +
log-secant bracket (~9x/pass shrink) with exact plateau fallback -> P3
DSMEM collect (odd/even dual bank) -> P4 CTA0 4x8-bit radix + tie ticket.
Dispatch on (npad, K) only: <=12288 direct single-block path.

**Weak bands to attack (in priority order):**
1. n = 32K-64K, ALL models (1.28-1.36) — the seed's worst zone and the
   deployment sweet spot. The seed's single-CTA/cluster crossover sits here.
2. V3.2 K=2048 at n = 8K-16K (1.29-1.47) — heavy-tail K=2048 rungs.
3. Mid-N 128K-256K (1.44-1.59) — solid but below the 512K-1M level.

**Do NOT give back the strongholds** (4k-8k >=1.9, 512K-1M >=1.7): check
every candidate on the full workload set, not just the band you tune.

Verified facts from a LATER independent campaign on the same problem (its
best never beat this seed, but these per-technique findings were
nsys-verified and are safe to reuse):
- Multi-threshold single-pass probing beats 2-point secant on barrier-heavy
  iterations — the seed already does this in P2; widening the bracket
  further REGRESSED (saturated).
- SMEM-cached whole-row radix works for n <= ~8192 only; it failed at 8195.
- 1024 threads on single-block latency-bound cells (grid <= #SMs) shortens
  the serial scan chain; 256 threads doubles occupancy at high grid counts.
- ILP double-buffering in collect is NEUTRAL (scans are L2-served).
- 16-CTA clusters do NOT beat tuned 8-CTA DSMEM; distributed cluster radix
  LOSES to rank-0 P4; fusing radix pass-0 into collect REGRESSES.
- Identical-row output broadcast and cooperative-groups grid.sync are both
  COMPLIANCE-REJECTED — do not attempt.
