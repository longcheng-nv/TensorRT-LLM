...
#pragma unroll
    ...
    int qt[RUNGS - 2];
    // op41: per-K quantile fractions. Hetero-row study (sim_dispatch_fracs2):
    // per-row first-pass convergence is a LAYER property and the optimal
    // rung placement tracks K (== model family h-distribution): K2048 rows
    // sit high (h med ~0.67 -> fracs 55/88), K1024 mid (35/70), K512 keeps
    // the original 25/65 (higher fracs regress flash). AR6 likewise shifts
    // only at K2048. Zero P2 cost: same rung count, different constants.
    if constexpr (AR == 4) {
      if (K >= 2048 && (npad < 49152 || npad > 98304)) {
        // measured exception: npad~65600 K2048 hetero batches LOSE with the
        // shifted fracs at BS>=256 (frac_scan_64k: 0.87-0.99 all candidates,
        // stock-frac control 1.00) — that tier keeps 25/65.
#ifndef QT2048_LO
#define QT2048_LO 55
#define QT2048_HI 88
#endif
        qt[0] = (Ks * QT2048_LO) / 100;
        qt[1] = (Ks * QT2048_HI) / 100;
      } else if (K == 1024) {
        qt[0] = (Ks * 35) / 100;
        qt[1] = (Ks * 70) / 100;
      } else {
