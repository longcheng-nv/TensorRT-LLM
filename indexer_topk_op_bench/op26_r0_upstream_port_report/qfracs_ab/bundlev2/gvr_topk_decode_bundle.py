...
class GvrTopKKernel:
    ...
    def __init__(
        ...
        # op35 bundle-v2 (bench A/B flags, OFF = byte-identical):
        skip_h1: bool = False,
        k_num_bins: Optional[int] = None,
    ):
        ...
        # op35 bundle-v2: P4 histogram bin-count override (kb512@K2048) +
        # drop the redundant end-of-P2 cluster handoff (admission state is
        # cluster-deterministic after block_count_ge*'s count-merge sync;
        # P3 is CTA-local). P1b reuses the P4 hist buffer and needs >=256.
        if k_num_bins is not None:
            assert k_num_bins >= 256, k_num_bins
            self.kNumBins = int(k_num_bins)
    ...
    def _run_phases(
        ...
        if v_hi <= cutlass.Float32(self.NEG_FLT_MAX) or v_lo >= v_hi:
            ...
            # do_cluster_sync is False (cs=1 or short-row degrade), or
            # entirely when op35 bundle-v2 skip_h1 is on (redundant sync:
            # admission is cluster-deterministic post count-merge, P3 is
            # CTA-local).
