...
"""Parse the refresh nsys-reps into one merged results.jsonl.
...
_BENCH = _HERE.parents[1]            # indexer_topk_op_bench/ (layers_harness parser)
...
KEEP = ("family", "sweep", "scenario", "model", "op", "K", "dtype", "N", "BS",
        "cr", "hit", "isl", "data_src", "exact", "r0_arm", "ms_path", "cluster_size", "launch_cfg", "L")
...
def main():
    ...
    for batch in sorted(root.glob("*.jsonl")):
        ...
        if first and first.get("sweep") == "seqlen":
            reptag = f"real_seqlen_{first['model']}_{first['isl']}"
        elif first and first.get("sweep") == "bs":
            reptag = f"real_bs_{first['model']}_L{first['L']}"
        else:
            reptag = tag
