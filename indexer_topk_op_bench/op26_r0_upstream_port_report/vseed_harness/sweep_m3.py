...
ARMS = [("base", {"enable_r0": False}), ("pr", {}), ("m3", {"r0_qfracs": (0.85, 0.50, 0.35)}), ("vseed", {"r0_vseed": True})]

# (model, isl, layer, dtype, [BS...])
CELLS = [
    ("flash", "1024k", 22, "fp32", [1, 128, 1024]),
    ("flash", "1024k", 22, "bf16", [1, 1024]),
    ("flash", "128k", 22, "fp32", [1]),
    ("pro", "128k", 30, "fp32", [1]),
    ("v32", "256k", 34, "fp32", [1, 1024]),
    ("v32", "64k", 34, "fp32", [1]),
]
...
def main():
    ...
    meta_f = open("/tmp/gvrval1/vseed_m3/cells.jsonl", "a")
    prof.start()
