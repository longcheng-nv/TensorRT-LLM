...
print(f"{'cell':>26} {'N':>7} {'hit':>5} | {'base':>7} {'pr':>7} {'vseed':>7} {'vs2':>7} | "
      f"{'pr/b':>5} {'vs/b':>5} {'v2/b':>5} | rel | exact(b/p/v/v2)")
gm_reg, gm_guard = [], []
for key in sorted(rows, key=lambda k: (k[0], k[3], k[1], k[2])):
    r = rows[key]
    b, p, v, v2 = (r.get(a, (None,) * 3) for a in ("base", "pr", "vseed", "vs2"))
    if not (b[0] and p[0] and v[0] and v2[0]):
        continue
    tag = f"{key[0]}/{key[1]}/{key[2]}/BS{key[3]}"
    ex = "/".join("T" if x[2] else "F" for x in (b, p, v, v2))
    print(f"{tag:>26} {r['N']:>7} {r['hit']:.2f} | {b[0]:7.1f} {p[0]:7.1f} {v[0]:7.1f} {v2[0]:7.1f} | "
          f"{b[0]/p[0]:5.2f} {b[0]/v[0]:5.2f} {b[0]/v2[0]:5.2f} | vs/pr={p[0]/v[0]:.2f} vs2/pr={p[0]/v2[0]:.2f} | {ex}")
