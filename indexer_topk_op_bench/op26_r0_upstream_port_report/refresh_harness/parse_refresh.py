...
_BENCH = _HERE.parents[1]            # indexer_topk_op_bench/ (refresh_harness is 2 levels down)
...
KEEP = ("family", "sweep", "scenario", "model", "op", "K", "dtype", "N", "BS",
        "cr", "hit", "isl", "data_src", "exact", "r0_arm", "ms_path", "cluster_size", "launch_cfg")
...
def main():
    ...
    for batch in sorted(root.glob("*.jsonl")):
        ...
        if first and first.get("family") == "synth":
