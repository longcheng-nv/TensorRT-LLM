...
# Reads batches from batches_refresh.py; runs batch index i where i%NW==W on GPU.
...
W=$1; GPU=$2; NW=$3
WD=/tmp/gvrval1
RH=/home/scratch.loncheng_gpu/workspace/perf/workloads/DSV4/TensorRT-LLM/indexer_topk_op_bench/op26_r0_upstream_port_report/refresh_harness
...
mapfile -t BATCHES < <(python3 "$RH/batches_refresh.py")
i=0
