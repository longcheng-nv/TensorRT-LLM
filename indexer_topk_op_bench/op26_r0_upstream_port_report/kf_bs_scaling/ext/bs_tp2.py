...
"""D2 sampled-estimate single-pass sweep: tp2 vs tp (D1) vs ext_v4 vs
gvr_pr. Protocol == bs_tp.py; tp2 exactness = all rows + fallback count.
  python3 bs_tp2.py --batch "flash 512k 22"   |   --list
"""
import argparse
import json
import sys
...
from bs_tp import tp_call  # noqa: E402

DEV = "cuda"
REPS_COLD, REPS_WARM = 20, 50
BS_GRID = [8, 16, 32, 64, 256, 1024]
CELLS = [("flash", "512k", 22), ("pro", "512k", 30)]
ARMS = ["gvr_pr", "ext_v4", "tp", "tp2"]


def tp2_call(b, BS, mod):
    K, N = b["K"], b["N"]
    lg = X.padded_batch(b, BS)
    out = torch.empty(BS, K, dtype=torch.int32, device=DEV)
    mod.tp2_fallbacks()
    call = lambda: mod.run_batch_tp2(lg, N, out)   # noqa: E731
    call()
    torch.cuda.synchronize()
    fb = int(mod.tp2_fallbacks())
    return call, [lg, out], {"fb": fb}, out


def run_batch_cells(model, isl, L):
    tag = f"{model}_{isl}_L{L}"
    out_path = HERE / "results" / f"tp2_{tag}.jsonl"
    done = set()
    if out_path.exists():
        ...
        for op in ARMS:
            ...
            try:
                if op == "gvr_pr":
                    ...
                    call, keep, extra, out = tp_call(b, BS, ext)
                else:
                    call, keep, extra, out = tp2_call(b, BS, ext)
                rec.update(extra)
                rec["exact"] = (exact_all_rows(out, b, BS)
                                if op in ("tp", "tp2")
