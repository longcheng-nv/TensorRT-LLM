...
"""D1 throughput-arm sweep: tp (barrier-free 3-kernel hist/collect/finish)
vs ext_v4 (chunked latency arm) vs gvr_pr, BS 8..1024. Protocol == bs_pq.py.
  python3 bs_tp.py --batch "flash 512k 22"   |   --list
"""
import argparse
import json
import sys
...
ARMS = ["gvr_pr", "ext_v4", "tp"]


def tp_call(b, BS, mod):
    K, N = b["K"], b["N"]
    lg = X.padded_batch(b, BS)
    out = torch.empty(BS, K, dtype=torch.int32, device=DEV)
    call = lambda: mod.run_batch_tp(lg, N, out)   # noqa: E731
    call()
    torch.cuda.synchronize()
    C = max(1, min(8, (2 * 148 + BS - 1) // BS))
    return call, [lg, out], {"C": C}, out


def run_batch_cells(model, isl, L):
    tag = f"{model}_{isl}_L{L}"
    out_path = HERE / "results" / f"tp_{tag}.jsonl"
    done = set()
    if out_path.exists():
    ...
    for BS in BS_GRID:
        for op in ARMS:
            ...
            try:
                ...
                rec.update(extra)
                rec["exact"] = (exact_all_rows(out, b, BS) if op == "tp"
