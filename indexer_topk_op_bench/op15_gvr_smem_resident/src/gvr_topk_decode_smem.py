...
class GvrTopKKernel:
    ...
    def __init__(
        ...
        enable_smem_resident: bool = False,
        smem_resident_n: int = 0,
    ):
        ...
        # SMEM-resident small-N fast path (op15 bucket): when True, the kernel
        # body loads the entire row of `smem_resident_n` logits into shared
        # memory ONCE (one coalesced gmem pass) and runs P1/P2/P3 reading from
        # SMEM instead of re-streaming gmem each pass. Only valid when the row
        # fits in SMEM (caller gates n <= cap and passes smem_resident_n == n).
        # The vectorized count/collect loops switch their make_ptr address
        # space gmem->smem via this flag. Default False => byte-identical to
        # the rank-scatter baseline.
        self.enable_smem_resident = enable_smem_resident
        self.smem_resident_n = smem_resident_n
    ...
    def _logits_addr_space(self):
        # SMEM-resident (op15): the vectorized count/collect loops read from
        # the SMEM row copy when enabled, else from gmem. The base pointer in
        # those loops comes from the passed tensor's iterator, which is the
        # smem_logits view in resident mode, so only the address space changes.
    ...
    def gvr_topk_kernel(
        ...
        # SMEM-resident row buffer (op15): native-dtype copy of the full row so
        # P1/P2/P3 read from SMEM. Sized to the compile-time cap; 128B aligned so
        # the vectorized (128/256-bit) ld.shared in block_count_ge/phase3 stays
        # aligned. Only allocated when the fast path is enabled.
        if cutlass.const_expr(self.enable_smem_resident):
            smem_logits = smem.allocate_tensor(
                element_type=self.dtype,
                layout=cute.make_ordered_layout((self.smem_resident_n,), order=(0,)),
                byte_alignment=128,
            )
        ...
        if N <= cutlass.Int32(top_k):
            ...
            # =================================================================
            # SMEM-resident staging (op15): copy the full row gmem->smem ONCE
            # (coalesced: consecutive threads write consecutive elements), then
