...
"""smem-cache probe (adapted from csopt) (rung 2): on large-N real cells, time arm v5best at
    ...
    for model, isl, L in CELLS:
        ...
        for sc in CSS:
            ...
            for _ in range(REPS):
                _EVICT.random_()
                torch.cuda.synchronize()
                torch.cuda.nvtx.range_push(f"c|sc{sc}|{uuid}|fp32")
                fn(logits, pre, sl, None, oi, None)
                torch.cuda.synchronize()
                torch.cuda.nvtx.range_pop()
            print(f"{uuid} smem_cache={sc} exact={ok}", flush=True)
        del logits, pre
