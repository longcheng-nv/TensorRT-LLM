...
Writes results/phase_base_40.json + prints the table.
"""
import csv
import json
import statistics
...
from gvrpkg40t.top_k.gvr_topk_decode import GvrTopKKernel as TimedK  # noqa: E402
...
def bl0_us():
    us = {}
    for r in csv.DictReader(open(OP40 / "results" / "bl0" / "cells.csv")):
        us[r["uuid"]] = float(r["base_us"])
    return us
...
def main():
    ...
    (OP40 / "results" / "phase_base_40.json").write_text(
        json.dumps(results, indent=1))
    print("\nwritten results/phase_base_40.json")
