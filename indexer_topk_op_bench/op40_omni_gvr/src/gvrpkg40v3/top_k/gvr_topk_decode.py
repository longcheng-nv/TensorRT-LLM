...
class GvrTopKKernel:
    ...
    def __init__(
        ...
        p3_hist_fuse: bool = False,  # [v6] build P4's coarse hist during P3 (cs=1)
    ):
        ...
        # across CTAs: at cs>1 each CTA histograms its OWN candidates per
        # level and only the 256-int histograms are exchanged via DSMEM;
        # every CTA picks the same digit redundantly (integer counts) and
        # scatters its own candidates straight to the GLOBAL output at
        # deterministic offsets (no cross-CTA atomics). Replaces cluster
        # handoff #2 + peer-push + leader-only P4 when p4_radix_dist; the
        # same method with the exchange compiled out serves cs=1
        # (p4_radix_cs1) and the cs>1 short-row degrade. Bit-exact ties
        # (level-3 classes) make the select exact on plateau/near-tie data
        ...
        # [v6] p3_hist_fuse: the P3 stream-write also bins each accepted
        # candidate into smem_hist over the FIXED range [thr, pmax] (pmax =
        # P1's preIdx max, s_thr[2]; values above clamp to the top bin, the
        # exact tail absorbs the skew). phase4_rank_scatter then skips its
        # candidate min/max scan AND the zero+build pass (2 fewer candidate
        # sweeps + 2 barriers). Single-CTA rows only: at cs>1 the leader's
        # P4 sees gathered candidates whose per-CTA hists are not merged.
        self.p3_hist_fuse = bool(p3_hist_fuse)
    ...
    def phase3_collect_candidates(
        ...
        do_cluster_sync,  # bool: False = cs=1 / short-row degrade (skip cluster sync)
        smem_input=None,  # optional SMEM-cached slice
        smem_hist=None,  # [v6] p3_hist_fuse: P4 coarse hist target
    ):
        ...
        # the single-CTA path.
        thr_final = s_thr[0]
        # [v6] fused-hist constants + zero. Formula MUST byte-match the
        # p3_hist_fuse branch of phase4_rank_scatter (deterministic fp32).
        fuse_hist = cutlass.const_expr(self.p3_hist_fuse and self.cluster_size == 1)
        fh_inv = cutlass.Float32(0.0)
        if cutlass.const_expr(fuse_hist):
            kB6 = cutlass.const_expr(self.kNumBins)
            fh_hi = s_thr[2]
            if fh_hi <= thr_final:
                fh_hi = thr_final + cutlass.Float32(1e-6)
            fh_inv = (cutlass.Float32(kB6 - 1) + cutlass.Float32(0.99)) / (fh_hi - thr_final)
            jz6 = tidx
            while jz6 < cutlass.Int32(kB6):
                smem_hist[jz6] = cutlass.Int32(0)
                jz6 = jz6 + cutlass.Int32(num_threads)
            cute.arch.barrier()
        vec_w = cutlass.const_expr(self.vec_bits // self.dtype.width)
        elem_bytes = cutlass.const_expr(self.dtype.width // 8)
        vec_align = cutlass.const_expr(self.vec_align_bytes)
                ...
                for k in cutlass.range(big_iters, unroll=4):
                    ...
                    for j in cutlass.range_constexpr(vec_w):
                        ...
                        if vj >= thr_final and wc < cutlass.Int32(kCC):
                            ...
                            if cutlass.const_expr(fuse_hist):
                                b6 = cutlass.Int32((vj - thr_final) * fh_inv)
                                if b6 < cutlass.Int32(0):
                                    b6 = cutlass.Int32(0)
                                if b6 > cutlass.Int32(self.kNumBins - 1):
                                    b6 = cutlass.Int32(self.kNumBins - 1)
                                atomicAdd(smem_hist.iterator + b6, cutlass.Int32(1))
        ...
        while it < N_local:
            ...
            if v >= thr_final and wc < cutlass.Int32(kCC):
                ...
                wc = wc + cutlass.Int32(1)
                if cutlass.const_expr(fuse_hist):
                    b6 = cutlass.Int32((v - thr_final) * fh_inv)
                    if b6 < cutlass.Int32(0):
                        b6 = cutlass.Int32(0)
                    if b6 > cutlass.Int32(self.kNumBins - 1):
                        b6 = cutlass.Int32(self.kNumBins - 1)
    ...
    def phase4_rank_scatter(
        ...
        if cand_count == cutlass.Int32(kK):
            ...
            # [v6] p3_hist_fuse: smem_hist already holds the coarse hist
            # (built during P3 over [thr, pmax]); re-derive the IDENTICAL
            # binning constants and skip the min/max + zero + build sweeps.
            if cutlass.const_expr(self.p3_hist_fuse and self.cluster_size == 1):
                bmin_r = s_thr[0]
                fh_hi6 = s_thr[2]
                if fh_hi6 <= bmin_r:
                    fh_hi6 = bmin_r + cutlass.Float32(1e-6)
                inv1 = (cutlass.Float32(kBins - 1) + cutlass.Float32(0.99)) / (fh_hi6 - bmin_r)
            else:
    ...
    #
    # Selects the top-K of the collected candidates by 8-bit digits of the
    # u32 order key (f32_order_key), MSB first, <=4 levels. Candidates never
    # leave their CTA: per level each CTA histograms its OWN in-class
    # candidates (256 bins), the histograms are cluster-summed via one DSMEM
    # exchange (peers push into the leader's smem_hxc, everyone reads back
    # totals + own-rank-exclusive prefixes), every CTA picks the same
    # crossing digit redundantly (integer counts -> deterministic), and
    # scatters its own candidates straight to the row's GLOBAL output at
    # deterministic offsets (no cross-CTA atomics). Exact on bit-tie
    # classes: at the last level all class members share the full 32-bit
    # key, hence the same value, so any need_rem of them is a correct
    # multiset. Solo mode (cs=1 or cs>1 short-row degrade) compiles the
    # exchange out / skips it at runtime.
    #
    # SMEM contract: smem_hist >=256 ints (totals -> desc-cum-excl),
    # smem_ptcnt >=512 ints ([0..256) per-digit cross-CTA prefix,
    # [256..512) per-digit local arrival counters), smem_hxc cs*256 ints
    # (exchange region, cs>1 only). Uses s_iscalars[0]/[2]/[3]/[4] as
    # publish slots (free after P2/P3); [5] is NOT touched.
    # ------------------------------------------------------------------
    @cute.jit
    def phase4_radix_select(
        self,
        smem_keys,
        smem_vals,
        smem_hist,
        ...
        num_threads = cutlass.const_expr(self.num_threads)
        cs = cutlass.const_expr(self.cluster_size)
        SEG = cutlass.const_expr(8)  # 256 bins / 32 lanes
        ...
        for level in cutlass.range_constexpr(4):
            ...
            pmask = cutlass.const_expr((1 << (8 * level)) - 1)
            if done == cutlass.Int32(0):
                # ---- zero hist + prefix + arrival counters ----
                jz = tidx
                while jz < cutlass.Int32(256):
                    smem_hist[jz] = cutlass.Int32(0)
                    smem_ptcnt[jz] = cutlass.Int32(0)
                    ...
                    jz = jz + cutlass.Int32(num_threads)
                cute.arch.barrier()
                # ---- local digit histogram over in-class candidates ----
                ic = tidx
                while ic < cand_loc:
                    k32 = f32_order_key(smem_keys[ic])
                    inclass = cutlass.Int32(1)
                    if cutlass.const_expr(level > 0):
                        hi = (k32 >> cutlass.Int32(shift + 8)) & cutlass.Int32(pmask)
                        if hi != sel_hi:
                    ...
                    ic = ic + cutlass.Int32(num_threads)
                cute.arch.barrier()
                # ---- cluster histogram exchange (cs>1 long-row only) ----
                if cutlass.const_expr(cs > 1):
                    if do_cluster_sync:
                        jp = tidx
                        while jp < cutlass.Int32(256):
                            ...
                            st_shared_cluster_i32(hxaddr, smem_hist[jp])
                            jp = jp + cutlass.Int32(num_threads)
                        # release own pushes / acquire peers' pushes
                        cute.arch.cluster_arrive()
                        cute.arch.cluster_wait()
                        jr = tidx
                        while jr < cutlass.Int32(256):
                            ...
                            smem_ptcnt[jr] = pfx
                            jr = jr + cutlass.Int32(num_threads)
                        # all CTAs done READING leader's hxc before the next
                        # level's pushes overwrite it
                        cute.arch.cluster_arrive()
                        cute.arch.cluster_wait()
                cute.arch.barrier()
                # ---- warp0: desc scan -> crossing digit + publish cumsum ----
                if warp_id == cutlass.Int32(0):
                    top = cutlass.Int32(255) - lane * cutlass.Int32(SEG)
                    seg_frag = cute.make_fragment((SEG,), cutlass.Int32)
                    part = cutlass.Int32(0)
                    ...
                    for off_i in cutlass.range_constexpr(5):
                        ...
                        if lane >= cutlass.Int32(off_v):
                            tp = tp + other
                    excl = tp - part
                    total_all = cute.arch.shuffle_sync(
                        tp, cutlass.Int32(self.WARP_SIZE - 1)
                    )
                    run = cutlass.Int32(0)
                    for j in cutlass.range_constexpr(SEG):
                        db = top - cutlass.Int32(j)
                        cb = excl + run  # count strictly above digit db
                        run = run + seg_frag[j]
                        ca = excl + run  # count >= digit db (class incl)
                        smem_hist[db] = cb  # desc-cum-excl for the scatter
                        if cb < need_rem and ca >= need_rem:
                            s_iscalars[2] = db
                            s_iscalars[3] = cb
                            s_iscalars[4] = seg_frag[j]
                    if lane == cutlass.Int32(0):
                        s_iscalars[0] = total_all
                        if total_all < need_rem:
                            s_iscalars[2] = cutlass.Int32(-1)
                            s_iscalars[3] = total_all
                            s_iscalars[4] = cutlass.Int32(0)
                cute.arch.barrier()
                d_star = s_iscalars[2]
                cum_above = s_iscalars[3]
                class_sz = s_iscalars[4]
                total_all2 = s_iscalars[0]
                exact_fit = cutlass.Int32(0)
                if class_sz == (need_rem - cum_above):
                    exact_fit = cutlass.Int32(1)
                # ---- scatter: digits > d*, plus the class itself when it
                # exactly fits, is the last level (bit-ties), or undershoot ----
                isc2 = tidx
                while isc2 < cand_loc:
                    k32b = f32_order_key(smem_keys[isc2])
                    inc2 = cutlass.Int32(1)
                    if cutlass.const_expr(level > 0):
                        hi2 = (k32b >> cutlass.Int32(shift + 8)) & cutlass.Int32(pmask)
                        if hi2 != sel_hi:
                    ...
                    if inc2 == cutlass.Int32(1):
                        ...
                        if emit == cutlass.Int32(1):
                            ...
                            if slot < cutlass.Int32(kK):
                                if cutlass.const_expr(self.return_output_values):
                                    output_values_row[slot] = self.dtype(smem_keys[isc2])
                                output_indices_row[slot] = smem_vals[isc2]
                    isc2 = isc2 + cutlass.Int32(num_threads)
                cute.arch.barrier()
                # ---- undershoot pad (leader/solo thread0) + level advance ----
                if d_star == cutlass.Int32(-1):
                    pad_here = cutlass.Int32(1)
                    if cutlass.const_expr(cs > 1):
                        if do_cluster_sync:
                            if cta_in_cluster != cutlass.Int32(0):
                                pad_here = cutlass.Int32(0)
                    if (pad_here == cutlass.Int32(1)) and (tidx == cutlass.Int32(0)):
                        jp2 = base_rank + total_all2
                        while jp2 < cutlass.Int32(kK):
                            if cutlass.const_expr(self.return_output_values):
                                output_values_row[jp2] = self.dtype(
                                    cutlass.Float32(self.NEG_FLT_MAX)
                                )
                            output_indices_row[jp2] = cutlass.Int32(-1)
                            jp2 = jp2 + cutlass.Int32(1)
                    done = cutlass.Int32(1)
                if exact_fit == cutlass.Int32(1):
                    done = cutlass.Int32(1)
                if cutlass.const_expr(shift == 0):
    ...
    def run_one_row(
        ...
        # [v3] DSMEM histogram-exchange region for phase4_radix_select:
