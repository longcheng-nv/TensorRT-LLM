...
"""op26_r0mcp (iter7 D2 cluster-cooperative P4) A/B verdict vs op26_r0mc(p4_rs default).

Input = results_b200_op26_r0mcp_ab (batch-paired 2-arm nsys sweep,
worst+real x seqlen/bs/bs_hugeN x 3K x 3dtype). Per-cell metric =
us_cold(r0mc) / us_cold(r0mcp)  (>1 = coop faster). Views:
...
Usage: python3 analyze_r0mcp_ab.py [<root>]
"""
import json
import math
import statistics
...
ROOT = Path(sys.argv[1]) if len(sys.argv) > 1 else \
    HERE.parents[0] / "results_b200_op26_r0mcp_ab"
...
def main():
    cells = load()
    pairs = {k: v["op26_r0mc"] / v["op26_r0mcp"] for k, v in cells.items()
             if "op26_r0mc" in v and "op26_r0mcp" in v}
