...
"""op26_r0mcc (mc-port p1b_cache) A/B verdict vs op26_r0mc.

Input = results_b200_op26_r0mcc_ab (batch-paired 2-arm nsys sweep,
worst+real x seqlen/bs/bs_hugeN x 3K x 3dtype). Per-cell metric =
us_cold(r0mc) / us_cold(r0mcc)  (>1 = cache faster). Views:
...
Decision rule (mirror of the 1cta dispatch_p1bc_op26 verdict): gate ON
per dtype iff mc-region gm >= 1.005 with no systematic loss band.

Usage: python3 analyze_r0mcc_ab.py [<root>]
"""
import json
import math
import statistics
...
ROOT = Path(sys.argv[1]) if len(sys.argv) > 1 else \
    HERE.parents[0] / "results_b200_op26_r0mcc_ab"
...
def main():
    cells = load()
    pairs = {k: v["op26_r0mc"] / v["op26_r0mcc"] for k, v in cells.items()
             if "op26_r0mc" in v and "op26_r0mcc" in v}
