#!/usr/bin/env python3
"""op21 iter6 A/B: small-bin P4 fast paths (OP21_P4_FAST=1) vs always-fine
(=0) on gvr_ms_auto. Cold-L2 CUDA-graph event medians — SCREENING ONLY.
Same-process paired A/B via the env-keyed compile cache (throttle-immune)."""
import math
import os
import sys
...
# P0 grid + P1 representatives (single-CTA band path also benefits)
CELLS = [
...
print(f"{'K':>5} {'N':>7} {'BS':>5} | {'fine_us':>8} {'fast_us':>8} "
      f"{'fine/fast':>9} exact")
rats = []
for K, N, BS in CELLS:
    cr = CRMAP[K]
    ...
    print(f"{K:>5} {N:>7} {BS:>5} | {res['0']:8.2f} {res['1']:8.2f} "
          f"{r:9.3f} {ex}")
print(f"gm fine/fast = {math.exp(sum(math.log(x) for x in rats)/len(rats)):.3f} "
      f"(>1 = fast paths faster), win {sum(1 for x in rats if x > 1.0)}/{len(rats)}")
