#!/usr/bin/env python3
"""op21 iter7 A/B: P3 band remote-store push (OP21_P3_PUSH=1) vs leader
DSMEM gather (=0) on gvr_ms_auto. Cold-L2 CUDA-graph event medians —
SCREENING ONLY. Same-process paired A/B via the env-keyed compile cache
(throttle-immune). Only cluster cells are affected (single-CTA gvr_ms has
no gather); P1 reps included as no-regression canaries."""
import math
import os
import sys
...
print(f"{'K':>5} {'N':>7} {'BS':>5} | {'gath_us':>8} {'push_us':>8} "
      f"{'gath/push':>9} exact")
rats = []
for K, N, BS in CELLS:
    cr = CRMAP[K]
    ...
    print(f"{K:>5} {N:>7} {BS:>5} | {res['0']:8.2f} {res['1']:8.2f} "
          f"{r:9.3f} {ex}")
print(f"gm gather/push = {math.exp(sum(math.log(x) for x in rats)/len(rats)):.3f} "
      f"(>1 = push faster), win {sum(1 for x in rats if x > 1.0)}/{len(rats)}")
