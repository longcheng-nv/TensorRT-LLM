...
# op21 nsys pure-kernel runner: gvr_ms (production entry, auto fuse gate),
# cold-L2 flush before each launch, inside a cudaProfilerApi window.
# Run under: nsys profile -c cudaProfilerApi
# Usage: python nsys_run.py <K> <dtype> <N> <BS> [iters]
import sys
...
def main():
    ...
    call = lambda: gvr_ms(lo, pr, sl, K, compress_ratio=crv, out=out)
    for _ in range(20):
    ...
    torch.cuda.synchronize()
    prof.stop()
    print(f"done gvr_ms K={K} {dtn} N={N} BS={BS} iters={iters}")
