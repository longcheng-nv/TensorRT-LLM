...
Cloned from op38/bs41_nsys.py; ONLY the kernel build differs.
...
from ab_v3mt import build_mt  # noqa: E402
...
def main():
    ...
    if args.shard:
        ...
        cells = cells[i::m]
    print(f"[bs41] {len(cells)} cells", flush=True)
    ...
    for model, isl, L in cells:
        ...
        for bs in BS_LIST:
            ...
            if bad:
                print(f"[bs41] INEXACT {cname} BS{bs}: {bad}", flush=True)
            ...
            del lg, pre, out
        print(f"[bs41] {cname} done", flush=True)
    ...
    print(f"[bs41] ALL DONE, inexact {n_bad}/{len(exact_log)}", flush=True)
