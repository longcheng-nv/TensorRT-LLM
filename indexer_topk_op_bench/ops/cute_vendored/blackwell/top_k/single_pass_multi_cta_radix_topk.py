...
class SinglePassMultiCTARadixTopKKernel:
    ...
    def __init__(
        ...
        # When True (single-CTA only), the row is NOT staged in shared memory;
        # each radix round re-reads it from global memory and converts on the
        # fly. This removes the chunk_size<=smem cap, so one CTA can process a
        # row of ANY length (at the cost of num_rounds global re-reads). The
        # default resident path is unchanged, so production / cluster callers
        # that never set this flag are bit-for-bit identical.
        self.single_streaming = single_streaming
