...
void tp2_sample(const float* __restrict__ logits, long rstride, int n, int C,
    ...
    const int nv4 = n >> 2;
    (void)C;  // whole-row uniform sampling: slice-independent, unbiased
    const float4* in4 = reinterpret_cast<const float4*>(lgr);
    unsigned int cnt = 0;
    for (int i = tid; i < nv4; i += 512 * D2_SSTRIDE) {
        float4 f = __ldcs(in4 + i);
        atomicAdd(&sh[fkey(f.x) >> 21], 1u);
        atomicAdd(&sh[fkey(f.y) >> 21], 1u);
        atomicAdd(&sh[fkey(f.z) >> 21], 1u);
...
void tp2_collect(const float* __restrict__ logits, long rstride, int n, int k,
    ...
    __shared__ unsigned int warp_totals[16];
    __shared__ int s_bin, s_above, s_stage;
    __shared__ __align__(16) uint2 stagebuf[ARENA2];
    __shared__ unsigned int s_base;
    if (threadIdx.x == 0) s_stage = 0;
    const int total = (int)d2_r[2];
    // budget-driven safe boundary: deepest bucket whose expected candidate
    // count stays <= CAP2/2 (adapts to boundary-region density; k <= 2048
    // < CAP2/2 keeps cand >= k on non-adversarial rows). Pathologies land
    // in the finish-kernel fallback via the count checks.
    ...
    for (int i = beg + tid; i < end; i += 512) {
        float4 f = __ldcs(in4 + i);
        unsigned int kk[4] = {fkey(f.x), fkey(f.y), fkey(f.z), fkey(f.w)};
        // stage hits in smem; ONE bulk global reservation per CTA (same-
        // address L2 atomic serialization was the collect bottleneck).
#pragma unroll
        for (int j = 0; j < 4; ++j) {
            if ((int)(kk[j] >> 21) > b_safe) {
                const int sp = atomicAdd(&s_stage, 1);
                const uint2 e = make_uint2(kk[j], (unsigned int)(4 * i + j));
                if (sp < ARENA2) stagebuf[sp] = e;
                else {
                    const unsigned int s = atomicAdd(cand_write, 1u);
                    if (s < CAP2) candbuf[s] = e;
                }
            }
        }
    }
    if (slice == C - 1) {
        for (int i = (nv4 << 2) + tid; i < n; i += 512) {
            ...
            if ((int)(key >> 21) > b_safe) {
                const int sp = atomicAdd(&s_stage, 1);
                const uint2 e = make_uint2(key, (unsigned int)i);
                if (sp < ARENA2) stagebuf[sp] = e;
                else {
                    const unsigned int s = atomicAdd(cand_write, 1u);
                    if (s < CAP2) candbuf[s] = e;
    ...
    __syncthreads();
    {
        const int staged = min(s_stage, ARENA2);
        if (tid == 0) s_base = atomicAdd(cand_write, (unsigned int)staged);
        __syncthreads();
        for (int i = tid; i < staged; i += 512) {
            const unsigned int s = s_base + (unsigned int)i;
            if (s < CAP2) candbuf[s] = stagebuf[i];
        }
    }
}
...
void tp2_launch(const float* logits, long W, int n, int k, int* out, int BS,
    ...
    if (C < 1) C = 1;
    const dim3 grid((unsigned int)C, (unsigned int)BS);
    tp2_sample<<<dim3(1, (unsigned int)BS), 512, 0, stream>>>(
        logits, W, n, 1, g_d2_shist, g_d2);
...
void tp3_kernel(const float* __restrict__ logits, long rstride, int n, int k,
    ...
    __shared__ __align__(16) uint2 arena[ARENA2];
    __shared__ unsigned int warp_totals[16];
    __shared__ int s_bin, s_above, s_cnt, s_tie, s_stage;
...
#pragma unroll
                for (int j = 0; j < 4; ++j) {
                    if ((int)(kk[j] >> 21) > b_safe) {
                        const int sp = atomicAdd(&s_stage, 1);
                        const uint2 e = make_uint2(
                            kk[j], (unsigned int)(4 * (i + u * 512) + j));
                        if (sp < ARENA2) arena[sp] = e;
                        else {  // staging overflow: direct (rare, dense row)
                            const unsigned int s = atomicAdd(cand_write, 1u);
                            if (s < CAP2)
                                atomicExch(
...
#pragma unroll
        ...
        if (c == C - 1) {
            for (int i2 = (nv4 << 2) + tid; i2 < n; i2 += 512) {
                ...
                if ((int)(key >> 21) > b_safe) {
                    const int sp = atomicAdd(&s_stage, 1);
                    const uint2 e = make_uint2(key, (unsigned int)i2);
                    if (sp < ARENA2) arena[sp] = e;
                    else {
                        const unsigned int s = atomicAdd(cand_write, 1u);
                        if (s < CAP2)
    ...
    // bulk flush: ONE global reservation per CTA instead of one per hit
    __syncthreads();
    {
        const int staged = min(s_stage, ARENA2);
        __shared__ unsigned int s_base;
        ...
        for (int i = tid; i < staged; i += 512) {
            ...
            if (s < CAP2) {
                const uint2 e = arena[i];
                atomicExch(reinterpret_cast<unsigned long long*>(&candbuf[s]),
                           ((unsigned long long)e.y << 32) | e.x);
            }
        }
    }
    ...
    asm volatile("fence.acq_rel.gpu;" ::: "memory");   // compB tail contract
...
void tp3_launch(const float* logits, long W, int n, int k, int* out, int BS,
    ...
    int C = g_tp3_cap / BS;
    if (C > 32) C = 32;
    if (g_tp3_forceC > 0 && g_tp3_forceC * BS <= g_tp3_cap) C = g_tp3_forceC;
...
// unified large-n dispatcher: 0 = ext_v4 single-wave, 1 = tp4 (exact-hist
// fused 2-pass, mid-BS), 2 = tp3 (sampled single-pass, high-BS), 3 = tp2
int g_tp4_max_bs = 512;  // slice-coverage rule above is the gate
int auto_pick(int n, int BS) {
    int team, cap4;
    diet_caps<4>(&team, &cap4, n);
    if ((long long)BS * team <= cap4) return 0;
    if (g_tp3_cap == 0) {
        cudaFuncSetAttribute(tp3_kernel,
                             cudaFuncAttributePreferredSharedMemoryCarveout,
                             cudaSharedmemCarveoutMaxShared);
    ...
    // tp4 zone from the measured 4-cell crossover table: it needs its smem
    // row cache to cover most of the slice AND an underfed-GPU regime
    const int nv4 = n >> 2;
    if (((BS <= 32 && nv4 <= 32768) || (BS <= 64 && nv4 <= 16384))
        && BS <= g_tp3_cap && BS <= g_tp4_max_bs) return 1;
    if (BS <= g_tp3_cap) return 2;
    return 3;
}
...
#define TP4_CACHE 1504   // float4s of slice cached in smem during phase A
    __shared__ __align__(16) unsigned int sh_hist[2048];
    __shared__ __align__(16) uint2 arena[TP4_TIE];
    __shared__ __align__(16) float4 s_cache[TP4_CACHE];
    __shared__ unsigned int warp_totals[16];
    ...
    // ---- phase A: sliced full read -> exact per-row hist; the head of
    //      the slice is cached in smem so phase B mostly avoids a second
    //      global read ----
    for (int i = tid; i < 2048; i += 512) sh_hist[i] = 0u;
    __syncthreads();
    for (int i = beg + tid; i < end; i += 512) {
    ...
    // ---- phase B: mostly-smem re-scan; gt staged in smem arena, ONE bulk
    //      cursor reservation per CTA ----
    if (tid == 0) { s_gt_n = 0; s_tie_n = 0; }
    const int cached_end = min(end, beg + TP4_CACHE);
    __syncthreads();
    for (int i = beg + tid; i < end; i += 512) {
        const float4 f = i < cached_end ? s_cache[i - beg] : __ldcs(in4 + i);
        const unsigned int kk[4] = {fkey(f.x), fkey(f.y), fkey(f.z),
                                    fkey(f.w)};
#pragma unroll
        for (int j = 0; j < 4; ++j) {
            ...
            if (d > b0) {
                ...
                if (whole) {
                    ...
                    atomicExch(reinterpret_cast<unsigned long long*>(
                                   &tiestage[s]),
                               ((unsigned long long)(unsigned int)(4 * i + j)
                                << 32) | kk[j]);
                }
            }
        }
    ...
    if (c == C - 1) {
        for (int i = (nv4 << 2) + tid; i < n; i += 512) {
            ...
            const int d = (int)(key >> 21);
            if (d > b0) {
                arena[atomicAdd(&s_gt_n, 1)] = make_uint2(key, (unsigned int)i);
            } else if (d == b0 && tie_ok) {
                const unsigned int s = atomicAdd(tie_cur, 1u);
                if (whole) {
                    ...
                    atomicExch(reinterpret_cast<unsigned long long*>(
                                   &tiestage[s]),
                               ((unsigned long long)(unsigned int)i << 32)
                                   | key);
                }
            }
        }
    ...
    if (whole) return;
    if (tie_ok) {
        // refine straight from the (L2-resident) global tie stage: the
        // refine passes re-read it 3x but T*8B is ~1-16KB — cheaper than a
        // staged copy + extra __syncthreads
        d2_refine_arena(tiestage, T, R, above0, outr, sh_hist, warp_totals);
        return;
    }
...
void tp4_launch(const float* logits, long W, int n, int k, int* out, int BS,
    ...
    int C = g_tp4_cap / BS;
    if (C > 32) C = 32;
    if (g_tp4_forceC > 0 && g_tp4_forceC * BS <= g_tp4_cap) C = g_tp4_forceC;
    if (C < 1) {
...
void topk_set_forceC(int c3, int c4) {
    aefm::g_tp3_forceC = c3;
    aefm::g_tp4_forceC = c4;
}
