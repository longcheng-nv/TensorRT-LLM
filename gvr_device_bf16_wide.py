...
def _ld_g_nc_v2_b32(gaddr, *, loc=None, ip=None):
    """Pinned 8-byte native-bf16 load: four packed bf16 elements."""
    ...
    return (
        cutlass.Uint32(mlir_llvm.extractvalue(T.i32(), r, [0], loc=loc, ip=ip)),
        cutlass.Uint32(mlir_llvm.extractvalue(T.i32(), r, [1], loc=loc, ip=ip)),
    )


@dsl_user_op
def _ld_g_nc_v4_b32(gaddr, *, loc=None, ip=None):
    """Pinned 16-byte native-bf16 load: eight packed bf16 elements."""
    st = llvm.StructType.get_literal([T.i32(), T.i32(), T.i32(), T.i32()])
    r = mlir_llvm.inline_asm(
        st,
        [gaddr.ir_value(loc=loc, ip=ip)],
    ...
    return (
        cutlass.Uint32(mlir_llvm.extractvalue(T.i32(), r, [0], loc=loc, ip=ip)),
        cutlass.Uint32(mlir_llvm.extractvalue(T.i32(), r, [1], loc=loc, ip=ip)),
        cutlass.Uint32(mlir_llvm.extractvalue(T.i32(), r, [2], loc=loc, ip=ip)),
        cutlass.Uint32(mlir_llvm.extractvalue(T.i32(), r, [3], loc=loc, ip=ip)),
    )


@dsl_user_op
def _ld_g_nc_u16(gaddr, *, loc=None, ip=None):
    """Pinned scalar native-bf16 load returned in a 32-bit carrier."""
...
def ld_g_f32x4(copy_atom, base_addr, v_idx, frag):
    ...
    """
    if cutlass.const_expr(copy_atom == cutlass.BFloat16):
...
def ld_g_bf16x8(copy_atom, base_addr, v_idx, frag):
    """Load eight bf16 values at eight-element vector index ``v_idx``."""
    w0, w1, w2, w3 = _ld_g_nc_v4_b32(
        base_addr + cutlass.Int64(v_idx) * cutlass.Int64(16)
    )
    frag[0] = f32_of_u32(w0 << cutlass.Uint32(16))
    frag[1] = f32_of_u32(w0 & cutlass.Uint32(0xFFFF0000))
    frag[2] = f32_of_u32(w1 << cutlass.Uint32(16))
    frag[3] = f32_of_u32(w1 & cutlass.Uint32(0xFFFF0000))
    frag[4] = f32_of_u32(w2 << cutlass.Uint32(16))
    frag[5] = f32_of_u32(w2 & cutlass.Uint32(0xFFFF0000))
    frag[6] = f32_of_u32(w3 << cutlass.Uint32(16))
    frag[7] = f32_of_u32(w3 & cutlass.Uint32(0xFFFF0000))


def ld_g_bf16x8_raw(base_addr, v_idx, frag):
    """Load eight bf16 values while retaining four packed u32 carriers."""
    w0, w1, w2, w3 = _ld_g_nc_v4_b32(
        base_addr + cutlass.Int64(v_idx) * cutlass.Int64(16)
    )
    frag[0] = w0
...
def bf16x8_raw_lane(frag, q: cutlass.Constexpr):
    """Widen constexpr lane ``q`` from a packed eight-bf16 packet."""
    word = frag[q // 2]
...
def ld_g_bf16x8_pair(copy_atom, base_addr, p4, fa, fb):
    """Load the sample arm's adjacent pair of four-element bf16 fragments."""
    w0, w1, w2, w3 = _ld_g_nc_v4_b32(
        base_addr + cutlass.Int64(p4) * cutlass.Int64(8)
    )
    fa[0] = f32_of_u32(w0 << cutlass.Uint32(16))
    fa[1] = f32_of_u32(w0 & cutlass.Uint32(0xFFFF0000))
    fa[2] = f32_of_u32(w1 << cutlass.Uint32(16))
    fa[3] = f32_of_u32(w1 & cutlass.Uint32(0xFFFF0000))
    fb[0] = f32_of_u32(w2 << cutlass.Uint32(16))
    fb[1] = f32_of_u32(w2 & cutlass.Uint32(0xFFFF0000))
    fb[2] = f32_of_u32(w3 << cutlass.Uint32(16))
    fb[3] = f32_of_u32(w3 & cutlass.Uint32(0xFFFF0000))
...
def ldg_bf16(base_addr, idx):
    """Native scalar bf16 gather widened exactly to fp32 in registers."""
    bits = _ld_g_nc_u16(base_addr + cutlass.Int64(idx) * cutlass.Int64(2))
    return f32_of_u32(bits << cutlass.Uint32(16))
...
def _ldg_bf16_rs(base_addr, idx, sc2):
    """Native-bf16 twin of _ldg_f32_rs with a caller-held byte stride."""
    bits = C._ld_g_nc_u16(base_addr + cutlass.Int64(idx) * cutlass.Int64(sc2))
    return C.f32_of_u32(bits << cutlass.Uint32(16))
...
class GvrMainKernel:
    ...
    def __init__(
        ...
        self.pf = self.pfd > 0
        # Exact engine used by the two measured 1024K/B16 wins.  Keeping the
        # packed-P3 carrier experiment on this full compile-time key excludes
        # R=8 512K splits, unsplit U=4/PFD=2 routes, and every kbig route.
        self.p3_raw = (
    ...
    def kern(
        ...
        CMPB = self.cmpb
        PFD = self.pfd
        if cutlass.const_expr(self.dtype == cutlass.BFloat16 and self.pfd > 2):
            PFD = 2
        P3RAW = self.p3_raw
        NATT = self.natt
        NW = BLK // 32
            ...
            if short == cutlass.Int32(0):
                ...
                if cutlass.const_expr(self.split):
                    ...
                    if cutlass.const_expr(self.dtype == cutlass.BFloat16):
                        Q = (
                            (n >> cutlass.Int32(3)) + cutlass.Int32(self.r_const - 1)
                        ) // cutlass.Int32(self.r_const)
                else:
                    Q = cutlass.Int32(0)
        ...
        if cutlass.const_expr(self.dtype == cutlass.BFloat16):
            pf = [cute.make_rmem_tensor((8,), cutlass.Float32) for _ in range(max(PFD, 1))]
            if cutlass.const_expr(P3RAW):
            ...
            if fullsl != cutlass.Int32(0):  # prime, full slice
                ...
                for uu in cutlass.range_constexpr(PFD):
                    ...
                    if cutlass.const_expr(self.dtype == cutlass.Float32):
                        ...
                        if cutlass.const_expr(P3RAW):
                            C.ld_g_bf16x8_raw(x_addr, ic, pf[uu])
                        else:
                            C.ld_g_bf16x8(atom128, x_addr, ic, pf[uu])
        ...
        if cutlass.const_expr(self.dtype == cutlass.BFloat16):
            fr = [
                cute.make_rmem_tensor((8,), cutlass.Float32) for _ in range(max(U - PFD, 1))
            ]
            if cutlass.const_expr(P3RAW):
                fr = [
                    cute.make_rmem_tensor((4,), cutlass.Uint32)
                        ...
                        if cutlass.const_expr(self.dtype == cutlass.Float32):
                            ...
                            if cutlass.const_expr(P3RAW):
                                for q in cutlass.range_constexpr(8):
                                    vq = C.bf16x8_raw_lane(vv, q)
                                    M = M | (
                                        cutlass.Int32(vq >= TF)
                                        << cutlass.Int32(uu * 8 + q)
                                    )
                            else:
                                for q in cutlass.range_constexpr(8):
                                    M = M | (
                                        cutlass.Int32(vv[q] >= TF)
                                        << cutlass.Int32(uu * 8 + q)
                                    )
                else:  # partial body
                    for uu in cutlass.range_constexpr(PFD, U):
                        ...
                        if cutlass.const_expr(self.dtype == cutlass.Float32):
                            ...
                            if cutlass.const_expr(P3RAW):
                                C.ld_g_bf16x8_raw(x_addr, ic, fr[uu - PFD])
                            else:
                                C.ld_g_bf16x8(atom128, x_addr, ic, fr[uu - PFD])
                    for uu in cutlass.range_constexpr(U):
                        if cutlass.const_expr(uu < PFD):
                            vv = pf[uu]
                        else:
                        ...
                        if infull != cutlass.Int32(0):
                            ...
                            for uu in cutlass.range_constexpr(PFD):
                                ...
                                if cutlass.const_expr(self.dtype == cutlass.Float32):
                                    ...
                                    if cutlass.const_expr(P3RAW):
                                        C.ld_g_bf16x8_raw(x_addr, jc, pf[uu])
                                    else:
                                        C.ld_g_bf16x8(atom128, x_addr, jc, pf[uu])
...
def get_compiled(
    ...
    key = (tuple(tpl), options_extra, bool(hint_free), bool(prefill), dtype)
...
class GvrTopkRegKernel:
    ...
    def kern(
        ...
        if short == cutlass.Int32(0):
            ...
            n4 = n >> cutlass.Int32(2)
            # Native-bf16 route_bf16 admits one narrow envelope fringe above
            # the float4 register tile.  Keep every full vector in registers
            # and feed the final <=64 scalar elements through the existing
            # per-thread tail path.  The fp32 arm retains its original n4.
            if cutlass.const_expr(self.dtype == cutlass.BFloat16):
