...
def _device():
    """Lazy import of the native-bf16 device module (first routed shape compiles;
    a broken/absent device module only fails when actually reached)."""
    global _dev_mod
    if _dev_mod is None:
        try:
            from . import gvr_device_bf16_wide as _m  # in-tree
        except ImportError:  # standalone dir
            import gvr_device_bf16_wide as _m
        _dev_mod = _m
    return _dev_mod
...
def route_bf16(b: int, n: int, npad: int, k: int) -> dict[str, object]:
    """Native-bf16 route table.

    Plan-B starts from a fresh fp32 route result; bf16-only tuning is applied
    here without mutating the production fp32 tables or their caches.
    """
    n4 = n >> 2
    if b <= 16 and k == 2048 and 5120 < n4 <= 9216:
        smc = (3 * NB + 2 * CMPC) * 4
        return {
            "kernel": "reg_clus",
            "tpl": (BLKC, 3, 4),
    ...
    if b == 1 and k == 2048 and 9216 < n4 <= 20480:
        smc = (3 * NB + 2 * CMPC) * 4
        return {
            "kernel": "reg_clus",
            "tpl": (BLKC, 5, 4),
    ...
    if b == 1 and k == 2048 and 20480 < n4 <= 40960:
        smc = (3 * NB + 2 * CMPC) * 4
        return {
            "kernel": "reg_clus",
            "tpl": (BLKC, 5, 8),
    ...
    for reg_elems in (1024, 2048, 4096, 8192, 16384, 32768, 65536):
        if reg_elems < n <= reg_elems + 64:
            if reg_elems <= 4096 or b <= (148 if reg_elems <= 16384 else 16):
                plan = route(b, reg_elems, npad, k)
                if reg_elems == 8192 and plan["kernel"] in ("reg", "regimg"):
                    tpl = list(plan["tpl"])
                    tpl[1] = 2
                    plan["tpl"] = tuple(tpl)
                return plan
    if b <= 148 and k == 2048 and 4096 < n4 <= 5120:
        # The native path can keep a fifth packed fragment resident, extending
        # the single-CTA register family across the padded v32 16K envelope.
        cmp_ = n if n < 2560 else 2560
        nbh = 2 * NB
        return {
            "kernel": "reg",
            "tpl": (1024, 5, 1, 2, True, False, False, nbh),
            "rt": {
                "n": n,
                "npad": npad,
                "k": k,
    ...
    if b == 1 and k <= 1024 and n <= 131136:
        # Flash/pro envelopes through 512K carry at least 61 columns of
        # guaranteed decode padding.  Route on that conservative valid-width
        # bound while the device still reads and gates the exact kv_lens row.
        return route(b, max(n - 61, k + 1), npad, k)
    return route(b, n, npad, k)


def route_streaming_bf16(
    b: int, n: int, npad: int, k: int, force_main: bool = False
) -> dict[str, object]:
    """Native-bf16 streaming table with eight-element main-family vectors.

    Halving U preserves the production tile's element count and live-value
    footprint while each bf16 load grows from four to eight elements.
    """
...
_BF16_VARLEN_CACHE = {}
_VARLEN_CACHE = _BF16_VARLEN_CACHE
...
def _varlen_launcher(
    ...
    # bf16 k>=1024 candidate-volume retune (R6) — see the compact host's
    # _varlen_launcher_bf16 for the measurement notes.  Host-only runtime
    # launch args; identical gates so both hosts agree on any shape.
    ...
    if big and 2 < r_const <= 9 and k > 1024 and tpl[5]:
        ...
        # k=1024 SPLIT big twin: amin 3.5k->2k with SFAC 48->128.
        amin = 2 * k
        sfac = 128
...
def run_varlen_bf16(
    ...
    if logits.dtype is not torch.bfloat16:
        raise RuntimeError(f"logits must be bfloat16 (got {logits.dtype})")
    ...
    if not (logits.is_cuda and indices.is_cuda):
        raise RuntimeError("all tensors must be CUDA")
    if logits.dtype is not torch.bfloat16 or indices.dtype is not _I32:
        raise RuntimeError("logits must be bfloat16; indices must be int32")
...
# Explicit native entry name; keep the legacy alias within this bf16-only
# host module so its optional warmup helpers remain internally consistent.
run_varlen = run_varlen_bf16
