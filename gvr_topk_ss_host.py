...
def _device():
    ...
    if _dev_mod is None:
        try:
            from . import gvr_topk_ss as _m  # in-tree
        except ImportError:  # standalone dir
            import gvr_topk_ss as _m
        _dev_mod = _m
    return _dev_mod
...
def _device_v4():
    ...
    if _dev_mod_v4 is None:
        try:
            from . import gvr_topk_ss_v4 as _m4  # in-tree
        except ImportError:  # standalone dir
            import gvr_topk_ss_v4 as _m4
        _dev_mod_v4 = _m4
    return _dev_mod_v4
    ...
    if (
        ...
        if amax >= 2:
            for v in (1, 2):
                ...
                # B=1 keeps its split-main arm (measured slower under CS16
                # in-suite: a single 16-CTA cluster gang-schedules on one GPC
                # with no co-scheduled work to overlap its rendezvous). v=2
                # only: the cs16-v1 shape probed 11.1us vs 9.84us (pro_512k_bs4)
                # and 21.3us vs 12.4us (v32_128k_bs8) against the CS8-v2
                # incumbents -- the doubled DSMEM merge fan-in loses at 131K.
...
def _varlen_launcher_bf16(
    ...
    if plan_free["kernel"] == "reg_clus":
        fn = dev.get_compiled__regclus(
            ...
            quadc=(
                96
                if (k > 1024 and num_rows > 1 and plan_free["tpl"][2] >= 8)
                else 384
            ),
        )
        lc = ("reg_clus", fn, n_kernel)
