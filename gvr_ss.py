...
# ---------------------------------------------------------------------------
# bf16 load primitives (native bf16 arm; 4 elements per 8-byte vector).
# bf16 -> fp32 widening is EXACT: value bits = bf16 bits << 16, so every
# downstream consumer (fkey ladder, histograms, staged u32 value bits) is
# dtype-agnostic once the loads widen in registers.
# ---------------------------------------------------------------------------
@dsl_user_op
    ...
    def kern(
        ...
        if shas != cutlass.Int32(0):
            ...
            if cutlass.const_expr(self.dtype == cutlass.Float32):
                ...
                C.ld_g_bf16x8_pair(atom128, x_addr, tidx * SS2, fsa, fsb)
            if cutlass.const_expr(self.prefill):
        ...
        while j < SMP:
            ...
            if cutlass.const_expr(self.dtype == cutlass.Float32):
                ...
                C.ld_g_bf16x8_pair(atom128, x_addr, j * SS2, fma_, fmb_)
            for t in cutlass.range_constexpr(4):
        ...
        if cutlass.const_expr(self.pf):
            ...
            if fullsl != cutlass.Int32(0):  # prime, full slice
                if cutlass.const_expr(self.dtype == cutlass.Float32):
                    ...
                    for uu in cutlass.range_constexpr(PFD):
                        C.ld_g_bf16x8_pk(atom128, x_addr, c0 + tidx + cutlass.Int32(uu * BLK), pf[uu])
            else:  # clamped prime
                if cutlass.const_expr(self.dtype == cutlass.Float32):
                    for uu in cutlass.range_constexpr(PFD):
                    ...
                    for uu in cutlass.range_constexpr(PFD):
                        ...
                        if ic >= c1:
                            ic = lim4
                        C.ld_g_bf16x8_pk(atom128, x_addr, ic, pf[uu])
            ...
            if g1 != cutlass.Int32(0):
                if cutlass.const_expr(self.dtype == cutlass.Float32):
                    ...
                    for uu in cutlass.range_constexpr(PFD, U):
                        C._prefetch_l2(
                            x_addr
                            + cutlass.Int64(c0 + tidx + cutlass.Int32(uu * BLK))
                            * cutlass.Int64(16)
                        )
        ...
        while running != cutlass.Int32(0):
            ...
            if cutlass.const_expr(self.dtype == cutlass.BFloat16):
                tf_bf16x2 = C.bf16_ceilx2_from_f32(TF)
            ...
            while it < nIt:
                ...
                if isfull != cutlass.Int32(0):  # full body
                    if cutlass.const_expr(self.dtype == cutlass.Float32):
                        ...
                        for uu in cutlass.range_constexpr(PFD, U):
                            C.ld_g_bf16x8_pk(atom128, x_addr, i0 + cutlass.Int32(uu * BLK), fr[uu - PFD])
                        for uu in cutlass.range_constexpr(U):
                            if cutlass.const_expr(uu < PFD):
                                vv = pf[uu]
                            else:
                                vv = fr[uu - PFD]
                            for q in cutlass.range_constexpr(4):
                                qm = C.bf16x2_ge_mask(vv[q], tf_bf16x2)
                    ...
                    if cutlass.const_expr(self.dtype == cutlass.Float32):
                        ...
                        for uu in cutlass.range_constexpr(PFD, U):
                            ...
                            if ic >= c1:
                                ic = lim4  # clamped address
                            C.ld_g_bf16x8_pk(atom128, x_addr, ic, fr[uu - PFD])
                        for uu in cutlass.range_constexpr(U):
                            if cutlass.const_expr(uu < PFD):
                                vv = pf[uu]
                            else:
                            ...
                            if okq != cutlass.Int32(0):  # ok-gated (+inf-pad escape)
                                for q in cutlass.range_constexpr(4):
                                    qm = C.bf16x2_ge_mask(vv[q], tf_bf16x2)
                                    M = M | (
                                        cutlass.Int32(qm) << cutlass.Int32(uu * 8 + q * 2)
                                    )
                if cutlass.const_expr(self.prefill):
                    ...
                    if hasnext != cutlass.Int32(0):
                        ...
                        if infull != cutlass.Int32(0):
                            if cutlass.const_expr(self.dtype == cutlass.Float32):
                                ...
                                for uu in cutlass.range_constexpr(PFD):
                                    C.ld_g_bf16x8_pk(atom128, x_addr, j0 + cutlass.Int32(uu * BLK), pf[uu])
                        else:
                            if cutlass.const_expr(self.dtype == cutlass.Float32):
                                for uu in cutlass.range_constexpr(PFD):
                                ...
                                for uu in cutlass.range_constexpr(PFD):
                                    ...
                                    if jc >= c1:
                                        jc = lim4
                                    C.ld_g_bf16x8_pk(atom128, x_addr, jc, pf[uu])
                ...
                if M != cutlass.Int32(0):
                    ...
                    if cutlass.const_expr(self.dtype == cutlass.Float32):
                        ...
                        if cutlass.const_expr(self.vstg and self.blk == 512):
                            xv = C.ldg_bf16_rs(x_addr, idx, x4_pin)
                        else:
                    ...
                    while M != cutlass.Int32(0):
                        ...
                        if cutlass.const_expr(self.dtype == cutlass.Float32):
                            ...
                            if cutlass.const_expr(self.vstg and self.blk == 512):
                                xv2 = C.ldg_bf16_rs(x_addr, idx2, x4_pin)
                            else:
        ...
        if alive != cutlass.Int32(0):
            ...
            if degen == cutlass.Int32(0):
                    ...
                    while i < listN:
                        ...
                        if cutlass.const_expr(self.vstg):
                            ...
                            if cutlass.const_expr(self.v16):
                                ...
                                xv = C.f32_of_u32(wpk & cutlass.Uint32(0xFFFF0000))
                                bq = C.f2s_rz((xv - TF) * SC)
                                if bq > cutlass.Int32(NBS - 1):
                                    bq = cutlass.Int32(NBS - 1)
                            else:
                ...
                if dga != cutlass.Int32(0):
                    ...
                    rng16 = cutlass.Int32(0)
                    if tidx < cutlass.Int32(NBS):
                        s_hist[tidx] = cutlass.Int32(0)
                    cute.arch.barrier()  # ---- barrier (degen A init) ----
                    brk = cutlass.Int32(0)
                    lev = cutlass.Int32(0)
                    while brk == cutlass.Int32(0):  # <=8 levels (bf16: see rng16)
                        if need2 == m2:
                            ethr = cutlass.Int64(cutlass.Uint32(rlo)) - cutlass.Int64(1)
                            above2 = above2 + m2
                            ...
                            ethr = cutlass.Int64(cutlass.Uint32(rlo))
                            brk = cutlass.Int32(1)
                        elif lev >= cutlass.Int32(4 if self.dtype != cutlass.Float32 else 8):
                            # bf16 level cap: adjacent distinct bf16 keys are
                            # exactly 0x10000 apart, so 256-bin narrowing from
                            # the full 32-bit bracket converges in <= 4 levels
                            # (24/16/8/0-shift sequence); fp32 keeps 8.
                            ethr = cutlass.Int64(cutlass.Uint32(rlo))
                            brk = cutlass.Int32(1)
                        else:
                            # bf16 one-class exit: a bracket narrower than the
                            # 0x10000 bf16 key spacing holds exactly ONE value
                            # class — every remaining level would re-walk the
                            # candidates only to pin a key we already know is
                            # unique. Stop here; the emit selects ties by key
                            # RANGE [rlo, rhi] instead of key == ethr (rng16).
                            hit16 = cutlass.Int32(0)
                            if cutlass.const_expr(self.dtype != cutlass.Float32):
                                if (cutlass.Uint32(rhi) - cutlass.Uint32(rlo)) < cutlass.Uint32(
                                    65536
                                ):
                                    ethr = cutlass.Int64(cutlass.Uint32(rhi))
                                    rng16 = cutlass.Int32(1)
                                    hit16 = cutlass.Int32(1)
                                    brk = cutlass.Int32(1)
                            if hit16 == cutlass.Int32(0):
                                d2 = cutlass.Uint32(rhi) - cutlass.Uint32(rlo)
                                b2_ = cutlass.Int32(32) - C.clz_i32(
                                    cutlass.Int32(d2 | cutlass.Uint32(1))
                                )
                        ...
                        if i < listN:
                            ...
                            if tie_m != cutlass.Int32(0):
                                if iu == ethr:
                                    p2 = cutlass.Int32(1)
                            if cutlass.const_expr(self.dtype != cutlass.Float32):
                                if rng16 != cutlass.Int32(0):
                                    # one-class bracket exit: ties are the whole
                                    # key RANGE [rlo, ethr==rhi] (single bf16
                                    # value class); winners stay iu > ethr.
                                    p2 = cutlass.Int32(0)
                                    if iu >= cutlass.Int64(cutlass.Uint32(rlo)):
                                        if iu <= ethr:
                                            p2 = cutlass.Int32(1)
                    ...
                    while brk == cutlass.Int32(0):  # <=8 levels
                        if need2 == m2:
                            ...
                            # bf16 one-class exit: a bracket narrower than the
                            # 0x10000 bf16 key spacing holds exactly ONE value
                            # class — remaining levels would re-walk candidates
                            # only to pin a key already known unique. Stop and
                            # emit ties by key RANGE [rlo, rhi] (rng16).
                            hit16 = cutlass.Int32(0)
                            if cutlass.const_expr(self.dtype != cutlass.Float32):
                                if (cutlass.Uint32(rhi) - cutlass.Uint32(rlo)) < cutlass.Uint32(
                                    65536
...
def get_compiled(
    ...
    key = (tuple(tpl), options_extra, bool(hint_free), bool(prefill), str(dtype), bool(v16))
...
def run(logits, pre_idx, n: int, out, ws):
    ...
    try:
        from . import gvr_ss_host as ct_dispatch
    except ImportError:
        import gvr_ss_host as ct_dispatch
    b, npad = logits.shape
    k = pre_idx.shape[1]
...
class GvrTopkRegKernel:
    ...
    def __init__(
        ...
        # bf16 varlen head-latency hoist gate (see the kernel-head comment):
        # the default excludes minb >= 8 (32-register wall) and the
        # (1024,·,deg) rungs whose deg bracket consumes every lane at once —
        # both exclusions were measured on multi-row grids where warp
        # co-residency hides the head. hoist_b1 re-admits the (deg, blk=1024)
        # rungs for SINGLE-ROW launches: with one CTA on the GPU there is no
        # co-resident warp supply to hide the kv_lens round trip, so
        # overlapping it with the batch load is a pure critical-path cut. The
        # minb >= 8 register-wall exclusion stays.
        self.bf16_hoist = bool(varlen) and self.minb < 8 and (
            (not (self.deg and self.blk == 1024)) or bool(hoist_b1)
        )
    ...
    def kern(
        ...
        # ---- bf16 varlen head-latency hoist: the row vector batch is
        # address-independent of kv_lens (the envelope launch arg n bounds it
        # inside the padded row), so it is issued BEFORE the per-row varlen
        # prologue reads kv_lens -- the two global round trips overlap instead
        # of serializing at the kernel head. Lanes at/beyond the per-row
        # vector window are overwritten with -inf once n is known (register
        # state identical to the unhoisted batch; pad contents are never
        # consulted). fp32 arm: constexpr-eliminated, codegen untouched.
        # minb >= 8 tuples run at a 32-register launch bound where the
        # hoist-lengthened batch lifetimes spill (measured: the b>148
        # one-wave (256,V,8) rungs regressed 1.4-5.6%); the (1024,1,·,deg)
        # rung also measured slower hoisted (its deg bracket consumes every
        # lane immediately, so the hoist only adds WAW waits at 1024-thread
        # width), while the (512,2,4,deg) rung measured faster and stays on.
        # Head latency is not the excluded classes' bottleneck.
        hfrags = [cute.make_rmem_tensor((4,), cutlass.Float32) for _ in range(VPT)]
        if cutlass.const_expr(self.dtype != cutlass.Float32 and self.bf16_hoist):
        ...
        if short == cutlass.Int32(0):
            ...
            if cutlass.const_expr(self.dtype == cutlass.Float32):
                ...
                if cutlass.const_expr(self.bf16_hoist):
                    ...
                    # the unhoisted predicated batch below).
                    frags = hfrags
                    for u in cutlass.range_constexpr(VPT):
                    ...
                    if n4 >= cutlass.Int32(self.blk * self.vpt):  # block-uniform peel
                        for u in cutlass.range_constexpr(VPT):
                            ld_g_bf16x4(atom128, x_addr, tid + cutlass.Int32(u * self.blk), frags[u])
                    else:  # predicated flat batch
                        for u in cutlass.range_constexpr(VPT):
                            i = tid + cutlass.Int32(u * self.blk)
                            if i < n4:
            ...
            if cutlass.const_expr(self.dtype != cutlass.Float32):
                # staging-time crossing-key range (see the one-class fast
                # path in Phase 3): collect sites atomically fold every
                # staged crossing key into [s_kmin, s_kmax].
                if tid == cutlass.Int32(0):
                    s_kmm[0] = cutlass.Uint32(0xFFFFFFFF)
                    s_kmm[1] = cutlass.Uint32(0)
            ...
            if esc == cutlass.Int32(1):
                ...
                while state == cutlass.Int32(0):
                    ...
                    if state == cutlass.Int32(0):
                        ...
                        if cutlass.const_expr(self.dtype != cutlass.Float32):
                            if state == cutlass.Int32(0):
                                if (rhi - rlo) < cutlass.Uint32(65536):
                                    # bf16 one-class exit: adjacent distinct bf16 keys
                                    # are 0x10000 apart, so this bracket holds ONE value
                                    # class -- emit ties by key range (rng16e).
                                    rng16e = cutlass.Int32(1)
                                    state = cutlass.Int32(1)
                    if state == cutlass.Int32(0):
                        d2 = rhi - rlo
                ...
                if cutlass.const_expr(self.dtype != cutlass.Float32):
                    if rng16e != cutlass.Int32(0):
                        ethr = cutlass.Int64(rhi)
                ...
                # ---- emit
                kmn_st = cutlass.Uint32(0xFFFFFFFF)  # bf16 staged-key range
                kmx_st = cutlass.Uint32(0)  # (register-folded; see s_kmm)
                kq_ = cutlass.Uint32(0)
                                ...
                                if whole == cutlass.Int32(0):
                                    ...
                                    if q2i < cmp_:  # escape-made-safe guard
                                        if cutlass.const_expr(self.pk16):
                                            ...
                                            ci[q2i] = idx
                                        if cutlass.const_expr(self.dtype != cutlass.Float32):
                                            kq_ = fkey(_val(frags, s))
                                            if cutlass.const_expr(self.pk16):
                                                kq_ = kq_ & cutlass.Uint32(0xFFFF0000)
                                            if kq_ < kmn_st:
                                                kmn_st = kq_
                                            if kq_ > kmx_st:
                                                kmx_st = kq_
                ...
                # bf16 staged-key range publish: two shared atomics per WARP
                # (warp-aggregated; the emit-done barrier below orders them
                # before the Phase-3 reads). Feeds the one-class quad fast
                # path and the narrowing arm's bracket reuse.
                if cutlass.const_expr(self.dtype != cutlass.Float32):
                    kmn_w = warp_min_u32(kmn_st)
                    kmx_w = warp_max_u32(kmx_st)
                    if lane == cutlass.Int32(0):
                        atomic_min_cta(s_kmm.iterator, kmn_w)
                        atomic_max_cta(s_kmm.iterator + 1, kmx_w)
                ...
                if whole == cutlass.Int32(0):
                    ...
                    if quad == cutlass.Int32(1):
                        ...
                        if cutlass.const_expr(self.dtype != cutlass.Float32):
                            # one-class fast path: the collect sites folded
                            # every staged crossing key into [s_kmin, s_kmax];
                            # equal bounds mean the crossing bin is a single
                            # bf16 tie class, so its members are checker-
                            # interchangeable — emit the first `need` by slot
                            # order and skip the O(mc^2) rank walk.
                            if cutlass.Uint32(s_kmm[0]) == cutlass.Uint32(s_kmm[1]):
                                if mc > cutlass.Int32(0):
                                    oneq = cutlass.Int32(1)
                                    i = tid
                                    while i < need:
                                        if cutlass.const_expr(self.pk16):
                                            out_row[above + i] = cutlass.Int32(
                                                cutlass.Uint32(ck[i]) & cutlass.Uint32(0xFFFF)
                                            )
                                        else:
                                            out_row[above + i] = ci[i]
                                        i = i + cutlass.Int32(BLK)
                        if oneq == cutlass.Int32(0):
                        ...
                        if cutlass.const_expr(self.dtype != cutlass.Float32):
                            # the collect sites already folded every written
                            # crossing key into s_kmm (walked set == written
                            # set, both cmp_-capped): reuse it and skip the
                            # recompute pass and its two barriers.
                        ...
                        while done == cutlass.Int32(0):
                            ...
                            if done == cutlass.Int32(0):
                                ...
                                while i < mc:
                                    if cutlass.const_expr(self.pk16):
                                        unar = cutlass.Uint32(ck[i]) & cutlass.Uint32(
                                            0xFFFF0000
                                        )
                                    else:
                        ...
                        while it < it2:
                            ...
                            if i < mc:
                                ...
                                if cutlass.Int64(uke) == ethr:
                                    q2f = cutlass.Int32(1)
                                if cutlass.const_expr(self.dtype != cutlass.Float32):
                                    if rng16p != cutlass.Int32(0):
                                        q2f = cutlass.Int32(0)
                                        if cutlass.Int64(uke) >= cutlass.Int64(cutlass.Uint32(rlo)):
                                            if cutlass.Int64(uke) <= ethr:
                                                q2f = cutlass.Int32(1)
...
def reg_topk(logits, pre_idx, n, out, rd=None):
    ...
    try:
        from .gvr_ss_host import _dummy_kv
    except ImportError:
        from gvr_ss_host import _dummy_kv
        ...
        if short == cutlass.Int32(0):
            ...
            if shas != cutlass.Int32(0):
                ...
                if cutlass.const_expr(self.dtype == cutlass.Float32):
                    ...
                    C.ld_g_bf16x8_pair(atom128, x_addr, p8, fsa, fsb)
            ...
            while j < smp2:
                ...
                if cutlass.const_expr(self.dtype == cutlass.Float32):
                    ...
                    C.ld_g_bf16x8_pair(atom128, x_addr, p8, fma_, fmb_)
                for t in cutlass.range_constexpr(4):
                    smn = C.fmin_f32(smn, fma_[t])
                    smx = C.fmax_f32(smx, fma_[t])
                for t in cutlass.range_constexpr(4):
                            ...
                            for uu in cutlass.range_constexpr(U):
                                ...
                                for q in cutlass.range_constexpr(8):
                                    if cutlass.const_expr(q % 2 == 0):
                                        xq = C.f32_of_u32(vv[q // 2] << cutlass.Uint32(16))
                                    else:
                                        xq = C.f32_of_u32(vv[q // 2] & cutlass.Uint32(0xFFFF0000))
                                    M = M | (
                                ...
                                if okq != cutlass.Int32(0):  # +inf-pad escape, ok-gated
                                    for q in cutlass.range_constexpr(8):
                                        if cutlass.const_expr(q % 2 == 0):
                                            ...
                                            xq = C.f32_of_u32(
                                                vv[q // 2] & cutlass.Uint32(0xFFFF0000)
                                            )
                                        M = M | (
                                            cutlass.Int32(xq >= TF) << cutlass.Int32(uu * 8 + q)
                                        )
...
class GvrRegClusKernel:
    ...
    def __init__(
        ...
        # CS=10/16 are bf16-route-only widenings: the merge/striping phases are
        # CS-parametric (range_constexpr(CS) mapa loops, rank slabs q2i >>
        # LCMPC with CS*CMPC capacity) and both cluster widths are within the
        # SM100 nonportable limit. fp32 route() never emits cs > 8.
        assert vpt in (1, 2, 4) and cs in (2, 4, 8, 10, 16)
        # constexpr logits dtype (Float32 verbatim arm / BFloat16 native arm)
        self.dtype = dtype
        assert dtype in (cutlass.Float32, cutlass.BFloat16)
    ...
    def kern(
        ...
        if cutlass.const_expr(self.dtype != cutlass.Float32 and self.varlen):
            ...
            if cutlass.const_expr(self.hint_free):
                if cutlass.const_expr(self.cs >= 10):
        ...
        if short == cutlass.Int32(0):
            ...
            if cutlass.const_expr(self.dtype == cutlass.Float32):
                ...
                if rank == cutlass.Int32(0):
                    if tid < ntail:
                        qv = _fmaf__regclus(tval, SC, CQ)
                        bn = _umin_u32__regclus(
                            f2u_rz(qv), cutlass.Uint32(NB__regclus - 1)
                        )
                        atomic_add_cta(
                            s_hist.iterator + cutlass.Int32(bn), cutlass.Int32(1)
                        )
                    ...
                    if degen == cutlass.Int32(0):
                        if mc <= cutlass.Int32(QUADC__regclus):
                            if cutlass.const_expr(
                                ...
                                and (
                                    (
                                        (QUADC__regclus == 96 or QUADC__regclus == 192)
                                        and VPT == 2
                                    )
                                    or (
                                ...
                                if i < mc:
                                    ...
                                    if cutlass.const_expr(self.dtype != cutlass.Float32):
                                        if rng16d != cutlass.Int32(0):
                                            q2f = cutlass.Int32(0)
                                            if cutlass.Int64(cutlass.Uint32(uke)) >= cutlass.Int64(cutlass.Uint32(rlo)):
                                                if cutlass.Int64(cutlass.Uint32(uke)) <= ethr:
                                                    q2f = cutlass.Int32(1)
                                n1 = ballot(q1f == cutlass.Int32(1))
                                n2 = ballot(q2f == cutlass.Int32(1))
            ...
            # ---- P10: keep peers resident only while rank 0 can address a
            # remote crossing slab.  P7 stripes q2i by CMPC, so mc <= CMPC is
            # entirely local to rank 0; P8's ordered cluster barrier has
            # already completed every peer write before peers may exit.
            # Preserve the fp32 specialization verbatim/SASS-identical.
            if cutlass.const_expr(self.dtype == cutlass.Float32):
                _cluster_sync_aligned()
            else:
                if m > cutlass.Int32(CMPC):
                    _cluster_sync_aligned()
